...
Burgerstroom, Bolt Dynamisch, energie.be) or the hourly-aggregated curve (Frank default, Luminus, Mega,
...
(`pricing.py:70`, `const.py:152-140`). A digital (SMR3) meter registers
...
`const.py:170-135`) is orthogonal to the supplier meter: it is the billing mode
set on the user's grid connection, and the coordinator falls back automatically
when the DSO does not publish Impact rates (`const.py:165-135`).
