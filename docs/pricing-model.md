...
`vat_rate` is non-zero (`pricing.py:525-534`, same reasoning at
`pricing.py:517-560` for `static_breakdown`).
...
- `dso.data_management_per_year`, `capacity_eur_per_kw_year`, the Brussels OSP
  ...
  levies (`pricing.py:672-687`); `energy_fund_eur_per_month` is defined on the
  `TaxOverlay` (`providers/base.py:651`) but is not touched here.
...
- The Wallonia `region_connection_fee` is a per-kWh term and IS billed, but
  through `taxes_vat_exempt_eur_per_kwh` (`pricing.py:737`), not
...
(`providers/base.py:473`), and `resolve_excise_band` (`providers/base.py:903`)
...
2. `bi_capable` (meter is `bi` or `dynamic`) and both `peak` and `offpeak` are
   ...
   - Otherwise: `is_offpeak(when, region)` picks off-peak vs peak
     (`pricing.py:269`).
3. Fall back to the single/current `base` rate (`pricing.py:376`).
...
(`providers/base.py:339-342`). Fields: `pic`, `medium`, `eco`
(`providers/base.py:354-356`). `dso_impact_band` (`pricing.py:593-609`):
...
1. **Exclusive night** (`pricing.py:520-535`), resolved BEFORE the Impact band so
   ...
   `distribution_single` (`pricing.py:677-682`). Each step is closer to the real
...
`DsoTariffMode` (`"simple" | "bi_horaire" | "impact"`, `pricing.py:596`,
...
(`pricing.py:449-463`). Three rate shapes carry the dedicated field: `FixedRates`
...
rule as `compute_breakdown` (`pricing.py:723-754`). It returns `None` when the
...
(`pricing.py:545-552`). A missing `dso_key` raises `KeyError` with the available
keys (`pricing.py:723-754`, same guard in `compute_breakdown` at
`pricing.py:401-570`).
...
`peak`/`transition`/`offpeak`, and a `formula` string (`providers/base.py:388-403`).
...
snapshot's `vat_rate` (`providers/base.py:673-673`). None of the injection code
