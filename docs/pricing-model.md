...
`vat_rate` is non-zero (`pricing.py:422-431`, same reasoning at
`pricing.py:422-431` for `static_breakdown`).
...
(`providers/base.py:250-252`). `dso_impact_band` (`pricing.py:478-494`):
...
1. **Exclusive night** (`pricing.py:520-535`), resolved BEFORE the Impact band so
   ...
   `distribution_single` (`pricing.py:529-534`). Each step is closer to the real
...
`DsoTariffMode` (`"simple" | "bi_horaire" | "impact"`, `pricing.py:474`,
...
(`pricing.py:368-382`). Three rate shapes carry the dedicated field: `FixedRates`
...
rule as `compute_breakdown` (`pricing.py:422-431`). It returns `None` when the
...
(`pricing.py:461-468`). A missing `dso_key` raises `KeyError` with the available
keys (`pricing.py:402-407`, same guard in `compute_breakdown` at
`pricing.py:401-570`).
...
`_injection_needs_spot` detects it (`coordinator.py:2644-2666`):

```python
def _injection_needs_spot(snapshot, entry) -> bool:   # coordinator.py:2644
...
unavailable (`coordinator.py:1150-1163`, `coordinator.py:1252`). This is the
...
data (`coordinator.py:2789-2812`). Priority:

1. **Per-slot TOU** via `_tou_injection_rate` (`coordinator.py:2746-2748`).
2. **Spot formula** `factor * spot + base` when either the energy is
   ...
   (`coordinator.py:2749-2756`). The spot is looked up on the contract's own grid
   ...
   slot (900 s quarter-hourly, 3600 s hourly) (`coordinator.py:2760-2786`).
3. **Monthly indicative** `inj.current` otherwise, including static-energy cards
   ...
   Flexy, EBEM Groen Variabel / B@sic+) (`coordinator.py:2736-2744`,
   `coordinator.py:2752-2757`).
...
(`coordinator.py:2689-2708`, fields at `providers/base.py:304-306`). It reuses the
energy contract's own `weekend_rule` via `tou_slot` so injection and consumption
agree on the slot for a given hour (`coordinator.py:2703`). Returns `None`
...
formula and a historical spot exist, then `current` (`coordinator.py:2831-2862`).
...
user-facing numbers diverged (`coordinator.py:2844-2850`).
...
`_injection_needs_spot` (`coordinator.py:2662`) and `_compute_injection_price`
(`coordinator.py:2752`): when a card prints a monthly `current`, the spot branch
is skipped and the realized rate is used, keeping the live sensor consistent with
the YTD credit for the same hour (`coordinator.py:2736-2744`). A latent mis-price
here is masked whenever the indicative prints, which is why it was fixed
explicitly rather than left to fall through.

### YTD injection paths

Past-month YTD billing routes injection per regime (`coordinator.py:3562-3703`,
context):
...
- `injection`: per-hour `cons * all_in - inj * inj_rate`, where `inj_rate` comes
  from `_historical_injection_rate` (`coordinator.py:3683-3701`).

Shape (c) has a dedicated YTD helper `_ytd_spot_injection_credit`
(`coordinator.py:3718`) that credits a static-energy contract whose injection is a
...
cached spot (`coordinator.py:3740-3758`).
...
Fluvius actually charges rather than the energy side alone. Monthly cost (`_compute_capacity`, `coordinator.py:2437-2449`):
...
`capacity_eur_per_kw_year is None` (`coordinator.py:2440-2448`). The rate lives on
...
(`_compute_prosumer`, `coordinator.py:2884-2911`):
...
`CONF_SOLAR_KVA > 0` (`coordinator.py:2895-2909`). The Wallonia gate is deliberate:
...
(`coordinator.py:2865-2881`, `providers/base.py:497`).
...
overlay, gated the same Walloon-only way (`coordinator.py:3375-3411`).
...
(`_brussels_osp_fee`, `coordinator.py:2452-2461`):

```python
def _brussels_osp_fee(overlay, entry) -> float:      # coordinator.py:2452
...
(`coordinator.py:2481`), not to the per-kWh all-in.
