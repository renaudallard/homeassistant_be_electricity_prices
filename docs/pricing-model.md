...
| (c) Spot-indexed on a static-energy card | `factor` + `base` set, energy NOT dynamic, and either `current is None` or the card flags `slot_indexed` | Yes | Cociter Variable and Variable Trihoraire, every Bolt fixed and variable card. The compare estimate and the projection, which multiply one rate by a whole year's export, price the formula per slot over the closed days of day-ahead the coordinator holds for the past year (`_credit_spots`, `compare_inputs.py`), weighted by the household's export shape (`_compare_injection_credit`, `compare_weighting.py`). Not the live table's day or two, which moved the recorded projection by a median 76 EUR a day. Below 30 closed days the projection leaves the credit out and says so, and the page prices it on the day-ahead window in front of it |
