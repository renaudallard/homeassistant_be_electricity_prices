...
- a PER-SLOT formula floors each slot, because that is what the contract bills.
  ...
  (`_export_weighted_credit`, `compare_quote.py`), which is the basis the
...
| (e) Register pair | `current` + `peak` + `offpeak`, flagged `bi_hourly` | No | Trevion Groene Energie Vast: one feed-in rate per meter register beside the consumption rates. A bi-hourly or digital meter is credited by register on the region's day/night schedule, a single-register meter at `current`; a pair without the flag is shape (a). Every path asks per register, the per-day year-to-date walk included, which takes one hour inside each block |
