...
4. Build the slug and query URL (`_document_url`, `engie.py:349`), download and
   extract the PDF text with `fetch_pdf_text` (`_pdf.py:283`), and delegate to
...
| `valid_until` | `parse_valid_until` (`_pdf.py:1097`) | Best-effort validity date. |
...
the base and the factor and routes each through `parse_sign` (`_pdf.py:810`) so a
...
`parse_brussels_osp` (`_pdf.py:840`) into `brussels_osp_by_tier`.
