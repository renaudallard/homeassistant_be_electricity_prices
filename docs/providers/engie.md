...
4. Build the slug and query URL (`_document_url`, `engie.py:346`), download and
   extract the PDF text with `fetch_pdf_text` (`_pdf.py:234`), and delegate to
...
| `valid_until` | `parse_valid_until` (`_pdf.py:1004`) | Best-effort validity date. |
...
the base and the factor and routes each through `parse_sign` (`_pdf.py:717`) so a
