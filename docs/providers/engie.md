...
| `valid_until` | `parse_valid_until` (`_pdf.py:1165`) | Best-effort validity date. |
...
the base and the factor and routes each through `parse_sign` (`_pdf.py:878`) so a
...
`parse_brussels_osp` (`_pdf.py:908`) into `brussels_osp_by_tier`.
