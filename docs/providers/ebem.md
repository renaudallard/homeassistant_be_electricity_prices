...
| --- | --- | --- |
| Extractor id | `ebem` | `ebem.py:712` |
| Label | `EBEM` | `ebem.py:713` |
| Region(s) served | Flanders only | `_EBEM_REGIONS`, `ebem.py:709` |
| Publication form | Monthly PDF cards linked from one HTML listing page | `ebem.py:36` |
| Listing URL | `https://www.ebem.be/tarieven/` | `_LISTING_URL`, `ebem.py:90` |
| PDF base | `https://www.ebem.be` | `_PDF_BASE`, `ebem.py:91` |
| Archive | Yes, on the listing page (>= 6 months at last check) | `ebem.py:40` |
| Probe | HEAD the listing page (`Last-Modified` / `ETag`) | `probe`, `ebem.py:194` |
...
fetch would 404 (`tests/test_ebem.py:79`).
...
captured by `_PDF_RE` (`ebem.py:100`):
...
month would silently fall through to the coordinator proxy (`ebem.py:95`,
`tests/test_ebem.py:353`). The `kind` group is open-ended (`[a-z]+`) so a
...
Declared in `_CONTRACTS` (`ebem.py:119`) and turned into `Contract` objects in
the registry (`ebem.py:714`). None of the three sets `spot_indexed_injection`,
...
- **Groen Dyn@mic**: 15-minute Belpex spot dynamic, SMR3 (smart meter) only,
  with its own `dynamic` card. `quarter_hourly=True` (`ebem.py:388`) keeps the
  native 15-minute slots like Engie / Cociter rather than aggregating to hourly;
  see the `DynamicRates.quarter_hourly` docstring (`providers/base.py:128`).

Retired products: the variable card explicitly notes EBEM stopped selling fixed
contracts for now (`ebem.py:97`, `ebem.py:219`). No fixed contract is declared.
If EBEM revives one, its PDF kind (e.g. `fix`) surfaces verbatim from
`discover()` so live_check files a tracking issue (`ebem.py:236`).
...
2. `_find_latest(session, contract.pdf_kind)` (`ebem.py:241`) GETs the listing,
...
3. `fetch_pdf_text_layout` (`_pdf.py:316`) downloads the PDF and extracts
   layout-preserving text. This variant rejects CDNs that return HTTP 200 with
   `text/html` for a missing PDF (`_pdf.py:319`).
4. `parse_snapshot` (`ebem.py:258`) does the parsing.
...
HEADs the listing page via `head_freshness_key` (`_pdf.py:329`) and returns its
...
4. Resolve the URL, set `label = "YYYY-MM"`, download + parse; return `None` on
   `ExtractorError`.
5. Pass the result through `archive_validity_check` (`_pdf.py:638`).
...
accepted on the strength of the URL resolver alone (`_pdf.py:670`). This is why
`_extract_validity` failing silently degrades the safety check, not the fetch.

`test_fetch_for_month_handles_underscore_separator` (`tests/test_ebem.py:353`)
...
- `gas` / `aardgas` kinds (`_NON_ELEC_KINDS`, `ebem.py:108`) are dropped
...
`parse_snapshot` (`ebem.py:258`) assembles a `SupplierSnapshot` from six
...
| Field | Parser | Notes |
| --- | --- | --- |
| `energy` | `_extract_energy` (`ebem.py:358`) | branches on `contract_id` |
| `injection` | `_extract_injection` (`ebem.py:520`) | branches dynamic vs variable |
| `dsos` | `_extract_dsos` (`ebem.py:650`) | digital-meter table, optional analog prosumer |
| `taxes.federal_excise` + `energy_contribution` | `_extract_federal_taxes` (`ebem.py:589`) | |
| `taxes.flanders_renewables` | `_extract_flanders_renewables` (`ebem.py:610`) | |
| `valid_until` | `_extract_validity` (`ebem.py:310`) | printed Dutch month + year |

### Validity: `_extract_validity` (`ebem.py:310`)
...
directly via `_DUTCH_MONTHS` (`ebem.py:294`) and returned as the last day of
...
- It scans the first 600 chars for the first `word + year` token that is
  *actually a month*, rather than aborting on the first `word + 20\d\d`
  ...
  (`ebem.py:322`, `tests/test_ebem.py:59`).

### VAT convention

The registry convention is that stored snapshot values are VAT-inclusive
(`TaxOverlay.vat_rate=0.0` means "already VAT-incl"; `providers/base.py:307`).
EBEM cards print both ex-VAT and incl-VAT columns:

- `_vat_multiplier` (`ebem.py:344`) reads the percentage from the card header
  (`INCL. BTW N%` or `BTW N%`) via the shared `vat_multiplier` helper
  (`_pdf.py:369`), defaulting to 1.06.
- Dynamic energy factor / base are converted from the ex-VAT formula:
  `_formula_to_dynamic` (`ebem.py:352`) does `factor * vat * 10` and
...
### Groen Dyn@mic (`DynamicRates`, `ebem.py:359`)
...
(offset illustrative, from the card / test at `ebem.py:361`,
`tests/test_ebem.py:253`). The regex anchors on a line starting with
...
and `1.625 * 1.06 / 100 = 0.017225` (`tests/test_ebem.py:255`).

Yearly fee: `_extract_yearly_fee_abonnement` (`ebem.py:509`), the
`Abonnement ... €/jaar ... €/jaar` row (incl-VAT second column). Illustrative
`70` (`tests/test_ebem.py:259`).

### Groen B@sic+ (`VariableRates`, `ebem.py:391`)

Single rate for all hours. The formula row is
`Verbruik alle uren <factor> Belpex <sign> <base>` (`ebem.py:392`). The stored
...
`tests/test_ebem.py:221`); `yearly_fixed_fee_exclusive_night` still comes from
...
(`ebem.py:400`). `peak` / `offpeak` / `exclusive_night` energy are left `None`.

### Groen Variabel (`VariableRates`, `ebem.py:411`)
...
`ExtractorError` per label (`ebem.py:425`). The stored rates (`current`, `peak`,
...
### `_indicative_from_row` (`ebem.py:449`)
...
snapshot on an otherwise valid card (`ebem.py:461`).

Illustrative indicatives from the test (`tests/test_ebem.py:92`): mono
`0.123363`, peak `0.132458`, off-peak / excl-night `0.113359` EUR/kWh; B@sic+
`0.121243`.

## DSO overlay: `_extract_dsos` (`ebem.py:650`)

Maps the eight Fluvius sub-areas via `_FLANDERS_LABELS` (`ebem.py:638`). Note
...
The parser anchors on the `DIGITALE METER` heading (`ebem.py:663`) and reads
...
`prosumer_eur_per_kva_year` (`ebem.py:670`). The dynamic card is SMR3-only, so
it has no analog table and the prosumer column stays `None`
(`tests/test_ebem.py:272`). A DSO row that fails to match is skipped, not
fatal (`ebem.py:689`). Illustrative for `fluvius_iveka`: capacity `59.58`,
`distribution_single 0.0634`, `distribution_exclusive_night 0.0566`,
`data_management_per_year 18.92`, prosumer `67.79` (`tests/test_ebem.py:155`).

## Tax overlay

`_extract_federal_taxes` (`ebem.py:589`) returns
...
`_extract_flanders_renewables` (`ebem.py:610`) combines
...
(`tests/test_ebem.py:291`), guarding against silently zeroing ~1.6 c€/kWh of
every bill.
...
`energy_fund_eur_per_month`, and `vat_rate` are all `0.0` (`ebem.py:280`,
`tests/test_ebem.py:126`). The residential energy-fund tariff is EUR 0; the
non-residential EUR 10.07/month tier is not modelled (`tests/test_ebem.py:130`).

## Injection: `_extract_injection` (`ebem.py:520`)

EBEM spans two of the three injection shapes in the taxonomy (see
[../pricing-model.md](../pricing-model.md)):

### Dynamic: hourly factor*spot+base (`ebem.py:522`)
...
(`tests/test_ebem.py:262`). Negative base is expected: a low spot can make a
producer pay to inject, and the pricing engine respects that.

### Variable / B@sic+: monthly indicative only (`ebem.py:550`)
...
(`tests/test_ebem.py:172`). Emitting factor / base would make the pricing engine
apply monthly coefficients to the hourly spot; this mirrors Ecofix Flexy's
BELPEX-SPP-M handling (`ebem.py:556`). The regex captures an optional fourth
group (the indicative), and a missing indicative is fatal
(`ebem.py:571`, `tests/test_ebem.py:179`).
...
feed-in credit (`ebem.py:531`, `ebem.py:564`, `tests/test_ebem.py:193`).
...
- **Two products, one PDF**: `ebem_variable` and `ebem_basic_plus` share the
  ...
  (`tests/test_ebem.py:227`).
- **Underscore separator**: the 2026-01 dynamic file uses `dynamic_01-2026`
  (underscore), so `_PDF_RE` accepts `[-_]` between kind and MM (`ebem.py:95`).
...
- **Sign flexibility everywhere**: every formula and indicative regex accepts
  ...
  flip silently between them on re-render (`_pdf.py:466`). A literal `+` in
...
- **Exclusive-night yearly fee**: both variable products surface a dedicated
  ...
  exclusive-night meter, even B@sic+ which has no energy split (`ebem.py:400`).
...
- **`fetch_for_month` validity cross-check**: a resolved past-month URL whose
  ...
  and falls back to the coordinator proxy (`ebem.py:191`).
...
Test constants `_VARIABLE` / `_DYNAMIC` (`tests/test_ebem.py:51`); the listing
HTML is read at `tests/test_ebem.py:305`. Fixture text is loaded with
`layout=True` (`tests/test_ebem.py:55`) so it matches the production
...
| --- | --- | --- |
| Wrong / missing month, or archive proxy silently used | `_extract_validity` (`ebem.py:310`) | title format or a new colliding `word + year` token |
| Every fetch 404s or picks wrong month | `_PDF_RE` (`ebem.py:100`) / `_find_latest` (`ebem.py:241`) | filename pattern, separator, or kind token changed |
| Dynamic energy / injection missing | `_extract_energy` dynamic branch (`ebem.py:359`) / `_extract_injection` dynamic (`ebem.py:522`) | `alle uren` / `Belpex15'` label or spacing drift |
| Variable rate / indicative wrong | `_indicative_from_row` (`ebem.py:449`) | column order / count or sign changed |
| Variable formula row not found | row regexes (`ebem.py:415`) | meter-type label wording changed |
| Yearly fees wrong | `_extract_yearly_fee_variable` / `_excl_night` / `_abonnement` (`ebem.py:478`) | `Vaste vergoeding` / `Abonnement` label changed |
| A DSO drops out or a rate shifts | `_extract_dsos` (`ebem.py:650`) / `_FLANDERS_LABELS` (`ebem.py:638`) | Fluvius label rename, table heading, or column order |
| Taxes zeroed or wrong | `_extract_federal_taxes` (`ebem.py:589`) / `_extract_flanders_renewables` (`ebem.py:610`) | `0-3 MWH` casing, `Beschermende klanten`, or `Totale bijdrage` row drift |
| Monthly injection indicative missing (fatal) | `_extract_injection` variable branch (`ebem.py:558`) | card stopped printing the realized indicative column |
