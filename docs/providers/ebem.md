...
| Listing URL | `https://www.ebem.be/tarieven/` | `_LISTING_URL`, `ebem.py:93` |
| PDF base | `https://www.ebem.be` | `_PDF_BASE`, `ebem.py:94` |
| Archive | Yes, on the listing page (>= 6 months at last check) | `ebem.py:40` |
| Probe | HEAD the listing page (`Last-Modified` / `ETag`) | `probe`, `ebem.py:197` |
...
captured by `_PDF_RE` (`ebem.py:103`):
...
Declared in `_CONTRACTS` (`ebem.py:122`) and turned into `Contract` objects in
...
3. `fetch_pdf_text_layout` (`_pdf.py:465`) downloads the PDF and extracts
   layout-preserving text. This variant rejects CDNs that return HTTP 200 with
   `text/html` for a missing PDF (`_pdf.py:403`).
4. `parse_snapshot` (`ebem.py:261`) does the parsing.
...
HEADs the listing page via `head_freshness_key` (`_pdf.py:479`) and returns its
...
4. Resolve the URL, set `label = "YYYY-MM"`, download + parse; return `None` on
   `ExtractorError`.
5. Pass the result through `archive_validity_check` (`_pdf.py:1139`).
...
`test_fetch_for_month_handles_underscore_separator` (`tests/test_ebem.py:330`)
...
- `gas` / `aardgas` kinds (`_NON_ELEC_KINDS`, `ebem.py:111`) are dropped
...
`parse_snapshot` (`ebem.py:261`) assembles a `SupplierSnapshot` from six
...
| Field | Parser | Notes |
| --- | --- | --- |
| `energy` | `_extract_energy` (`ebem.py:338`) | branches on `contract_id` |
| `injection` | `_extract_injection` (`ebem.py:523`) | branches dynamic vs variable |
| `dsos` | `_extract_dsos` (`ebem.py:663`) | digital-meter table, optional analog prosumer |
| `taxes.federal_excise` + `energy_contribution` | `_extract_federal_taxes` (`ebem.py:602`) | |
| `taxes.flanders_renewables` | `_extract_flanders_renewables` (`ebem.py:623`) | |
| `valid_until` | `_extract_validity` (`ebem.py:300`) | printed Dutch month + year |

### Validity: `_extract_validity` (`ebem.py:300`)
...
directly via `_DUTCH_MONTHS` (`ebem.py:297`) and returned as the last day of
...
- `_vat_multiplier` (`ebem.py:324`) reads the percentage from the card header
...
- Dynamic energy factor / base are converted from the ex-VAT formula:
  `_formula_to_dynamic` (`ebem.py:332`) does `factor * vat * 10` and
...
Yearly fee: `_extract_yearly_fee_abonnement` (`ebem.py:515`), the
...
### `_indicative_from_row` (`ebem.py:464`)
...
## DSO overlay: `_extract_dsos` (`ebem.py:663`)

Maps the eight Fluvius sub-areas via `_FLANDERS_LABELS` (`ebem.py:651`). Note
...
`_extract_federal_taxes` (`ebem.py:602`) returns
...
`_extract_flanders_renewables` (`ebem.py:623`) combines
...
## Injection: `_extract_injection` (`ebem.py:523`)
...
### Variable / B@sic+: month-indexed on Belpex-SPP0 (`ebem.py:548`)
...
(`tests/test_ebem.py:171`), which is what makes that safe: the engine resolves
...
a missing indicative is still fatal (`ebem.py:569`, `tests/test_ebem.py:180`).
...
- **Underscore separator**: the 2026-01 dynamic file uses `dynamic_01-2026`
  (underscore), so `_PDF_RE` accepts `[-_]` between kind and MM (`ebem.py:103`).
...
| --- | --- | --- |
| Wrong / missing month, or archive proxy silently used | `_extract_validity` (`ebem.py:300`) | title format or a new colliding `word + year` token |
| Every fetch 404s or picks wrong month | `_PDF_RE` (`ebem.py:103`) / `_find_latest` (`ebem.py:244`) | filename pattern, separator, or kind token changed |
| Dynamic energy / injection missing | `_extract_energy` dynamic branch (`ebem.py:338`) / `_extract_injection` dynamic (`ebem.py:523`) | `alle uren` / `Belpex15'` label or spacing drift |
| Variable rate / indicative wrong | `_indicative_from_row` (`ebem.py:464`) | column order / count or sign changed |
| Variable formula row not found | row regexes (`ebem.py:403`) | meter-type label wording changed |
| Yearly fees wrong | `_extract_yearly_fee_variable` / `_excl_night` / `_abonnement` (`ebem.py:490`) | `Vaste vergoeding` / `Abonnement` label changed |
| A DSO drops out or a rate shifts | `_extract_dsos` (`ebem.py:663`) / `_FLANDERS_LABELS` (`ebem.py:651`) | Fluvius label rename, table heading, or column order |
| Taxes zeroed or wrong | `_extract_federal_taxes` (`ebem.py:602`) / `_extract_flanders_renewables` (`ebem.py:623`) | `0-3 MWH` casing, `Beschermende klanten`, or `Totale bijdrage` row drift |
| Monthly injection indicative missing (fatal) | `_extract_injection` variable branch (`ebem.py:523`) | card stopped printing the realized indicative column |
