...
- Both dynamic products set `quarter_hourly=True` (`octaplus.py:348-349`),
  ...
  long-term statistics). See `DynamicRates` docs in `base.py:150-163`.
...
| `valid_until` | `parse_valid_until` (`_pdf.py:945`) | shared helper |
...
`vat_rate` convention in `base.py:569-569`.
