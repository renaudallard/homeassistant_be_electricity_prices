...
transient group-buy SKU. `DISCOVER_IDS` (`providers/energyvision.py:153`) lists all of
...
header via `_publication_label` (`providers/energyvision.py:433`).
...
`fetch` (`providers/energyvision.py:315`) validates the contract id and region, then
`_resolve_card_url` (`providers/energyvision.py:363`) GETs the listing HTML and regexes the
...
`EXTRACTOR.probe` = `probe` (`providers/energyvision.py:333`): a cheap
...
`discover` (`providers/energyvision.py:348`) returns the residential NL electricity product
...
`parse_snapshot` (`providers/energyvision.py:381`) dispatches on the contract kind
...
`_extract_dynamic` (`providers/energyvision.py:452`) does one `findall` with
`_DYN_FORMULA_RE` (`providers/energyvision.py:179`), which matches both the `afnametarief`
...
`_extract_fixed` (`providers/energyvision.py:489`) parses the "Groene stroom - vast tarief
13,57 €cent/kWh" row with `_FIXED_ENERGY_RE` (`providers/energyvision.py:187`). The fixed
...
`_extract_taxes` (`providers/energyvision.py:507`) parses the shared Flanders levy block
...
`_extract_dsos` (`providers/energyvision.py:532`) covers all eight Fluvius sub-areas via
`_DSO_ROWS` (`providers/energyvision.py:300`). EnergyVision prints the area names in **upper
...
`_DIGITAL_MARKER` and `_ANALOG_MARKER` (`providers/energyvision.py:295`) - a modern SMR3
...
- **Upper-case Fluvius labels.** EnergyVision prints them in caps, so it needs its own
  `_DSO_ROWS` map, not `FLUVIUS_CARD_LABELS` (`providers/energyvision.py:300`).
