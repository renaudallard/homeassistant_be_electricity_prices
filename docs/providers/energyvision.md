...
transient group-buy SKU. `DISCOVER_IDS` (`providers/energyvision.py:154`) lists all of
...
header via `_publication_label` (`providers/energyvision.py:434`).
...
`fetch` (`providers/energyvision.py:316`) validates the contract id and region, then
`_resolve_card_url` (`providers/energyvision.py:364`) GETs the listing HTML and regexes the
...
`EXTRACTOR.probe` = `probe` (`providers/energyvision.py:334`): a cheap
...
`discover` (`providers/energyvision.py:349`) returns the residential NL electricity product
...
`parse_snapshot` (`providers/energyvision.py:382`) dispatches on the contract kind
...
`_extract_dynamic` (`providers/energyvision.py:453`) does one `findall` with
`_DYN_FORMULA_RE` (`providers/energyvision.py:180`), which matches both the `afnametarief`
...
`_extract_fixed` (`providers/energyvision.py:490`) parses the "Groene stroom - vast tarief
13,57 €cent/kWh" row with `_FIXED_ENERGY_RE` (`providers/energyvision.py:188`). The fixed
...
`_extract_taxes` (`providers/energyvision.py:508`) passes this card's anchors to the shared
...
`_extract_dsos` (`providers/energyvision.py:525`) covers all eight Fluvius sub-areas via
`_DSO_ROWS` (`providers/energyvision.py:301`). EnergyVision prints the area names in **upper
...
`_DIGITAL_MARKER` and `_ANALOG_MARKER` (`providers/energyvision.py:296`) - a modern SMR3
...
- **Upper-case Fluvius labels.** EnergyVision prints them in caps, so it needs its own
  `_DSO_ROWS` map, not `FLUVIUS_CARD_LABELS` (`providers/energyvision.py:301`).
