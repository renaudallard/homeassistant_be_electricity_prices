...
| Property | Value |
| --- | --- |
| Registry id | `mega` (`mega.py:947`) |
| Label | `Mega` (`mega.py:948`) |
...
`<REGION>` is one of `VL` / `WL` / `BX` (`_REGION_TO_CODE`, `mega.py:108`). The
...
`mega.py:106`), where every product card exposes an
`<a data-product-element="<Product Name>" ... href="<PDF URL>">` anchor, and matches
the anchor to its PDF with a regex (`_find_pdf_url`, `mega.py:184`).

The `source_url` recorded on the snapshot is the resolved PDF URL for the live
`fetch` (`mega.py:269`); the pure `parse_snapshot` defaults it to `_LISTING_URL`
when called without one (`mega.py:353`).

## Contracts

Eleven residential electricity products are registered (`_CONTRACTS`,
`mega.py:132`; the test pins `len(contract_ids) == 11` at `test_mega.py:61`). The
...
- Off-peak Impact is Wallonia-only (`regions=frozenset({REGION_WALLONIA})`,
  `mega.py:155`) because it needs the CWaPE Tarif réseau IMPACT plus an SMR3 smart
...
(`_KNOWN_UNSUPPORTED_PRODUCTS`, `mega.py:173`).

## Fetch strategy

### Live fetch

`fetch(session, contract_id, region)` (`mega.py:249`):

1. Validate the contract id and resolve the region to its `VL` / `WL` / `BX` code.
2. GET the listing HTML (`_fetch_listing_html`, `mega.py:202`).
3. Regex the current PDF URL with `_find_pdf_url` (`mega.py:184`), which pins the
   ...
   other-region links from matching (`mega.py:189`). The test
   ...
   `NG`.
4. Download the PDF text (`fetch_pdf_text`, `_pdf.py:160`) and hand it to
   `parse_snapshot`.

`fetch` raises `ExtractorError` on an unknown contract, an unknown region, or a
listing that has no entry for the (product, region) pair (`mega.py:255`).

### Probe

`probe(session, contract_id, region)` returns the resolved PDF URL for the pair
(`mega.py:227`). Mega's listing has neither `Last-Modified` nor `ETag`, so the
...
`fetch_for_month(session, contract_id, region, year_month)` (`mega.py:272`) uses
...
2. Match the `-MMYYYY-` segment (`mmyyyy_re`, `mega.py:311`) and capture the current
...
3. In the filename tail after the `-MMYYYY-` segment, rewrite the two-digit
   ...
   `Online0106-Fixed -> Online0105-Fixed`, `Cosy1306 -> Cosy1305`) (`mega.py:326`).
...
4. Download, parse, then cross-check the parsed card against the requested month
   with `archive_validity_check` (`_pdf.py:734`), passing `_FR_MONTH_NAMES`
   (`mega.py:349`). If validity or the month text does not match, return `None` so
...
`_is_pdf_payload`, `_pdf.py:109`, rejects it), when the parse fails, or when the
...
`discover(session)` (`mega.py:206`) returns every `data-product-element` value on
...
`parse_snapshot(contract_id, text, region, source_url)` (`mega.py:352`) is the pure,
...
| --- | --- | --- |
| Energy rates (per kind) | `_extract_energy` | `mega.py:500` |
| Injection | `_extract_injection` | `mega.py:650` |
| Publication label | `_extract_publication_month` | `mega.py:619` |
| Valid until | `parse_valid_until` then `_extract_valid_until` | `_pdf.py:773`, `mega.py:638` |
| Federal excise | `_extract_federal_excise` | `mega.py:689` |
| Energy contribution | `_extract_energy_contribution` | `mega.py:705` |
| Wallonia connection fee | `_extract_connection_fee` (Wallonia only) | `mega.py:720` |
| Regional renewables | `_extract_flanders_renewables` / `_extract_renewables` | `mega.py:732`, `mega.py:751` |
| DSO overlay | `_extract_flanders_dsos` / `_extract_wallonia_dsos` / `_extract_brussels_dsos` | `mega.py:775`, `mega.py:848`, `mega.py:906` |
| Supplier PV forfait | `_extract_supplier_prosumer` | `mega.py:407` |

### Energy block

`_extract_energy` (`mega.py:500`) always reads the yearly standing charge first
(`_extract_yearly_fee`, `mega.py:604`), which accepts both the split dynamic layout
...
- `tou_impact`: parse three CWaPE bands (`_extract_impact_tier` for `PIC`,
  `MEDIUM`, `ECO`, `mega.py:566`) plus the footnote formula text into `ImpactRates`.
...
- `fixed` / `variable`: read `Compteur mono-horaire` (mono), plus `Tarif jour`
  ...
  `_extract_meter_value` (`mega.py:579`). The bi-hourly labels are only read inside
...
Mega prints two distinct formulas in every Dynamic PDF (`mega.py:442`):
...
`mega.py:455`, `_INJECTION_FORMULA_RE` `mega.py:458`) sharing `_FORMULA_TAIL`
(`mega.py:447`). This is critical because Mega prints the injection formula BEFORE
...
guards against exactly that. `_parse_formula` (`mega.py:463`) converts factor and
...
base cents are converted to EUR (`mega.py:507`). Illustrative: factor 1.05, base
0.0135 EUR, fee 42.4 EUR/yr (`test_mega.py:115`); injection factor 1.0, base -0.04
EUR (`test_mega.py:128`).

### DSO overlays

The region dispatch in `parse_snapshot` (`mega.py:372`) selects exactly one DSO
parser and one renewables levy per snapshot.

Flanders (`_extract_flanders_dsos`, `mega.py:775`, `_FLANDERS_LABELS` `mega.py:772`)
...
Wallonia (`_extract_wallonia_dsos`, `mega.py:848`, `_WALLONIA_LABELS` `mega.py:839`)
...
Brussels (`_extract_brussels_dsos`, `mega.py:906`) maps the single Sibelga row, an
...
shared `parse_brussels_osp` (`_pdf.py:532`) and keyed by connection-power tier. The
...
`_extract_publication_month` (`mega.py:619`) first tries the versioned Smart Fixed
...
`_FR_MONTH_NAMES` (`mega.py:616`). `valid_until` prefers the shared
`parse_valid_until` keyword scan and falls back to `_extract_valid_until`
(`mega.py:638`), which reads `mois MM/YYYY` and returns the last calendar day of that
month (Mega cards are valid for the printed month).

## Taxes

`TaxOverlay` (`mega.py:387`) is assembled from:

- Federal excise: first tier `Consommation entre 0 et 3000 kWh`
  (`_extract_federal_excise`, `mega.py:689`), uniform across regions. Mandatory;
  raises on a miss because dropping it would silently undercount the bill by roughly
  5 c€/kWh (about 50 EUR/year at 1000 kWh, `mega.py:694`, illustrative).
- Energy contribution: the next number on the same `0 et 3000 kWh` row
  (`_extract_energy_contribution`, `mega.py:705`). Mandatory, raises on a miss.
- Regional renewables: exactly one of `flanders_renewables` (a single
  ...
  cogénération row appears, `mega.py:732`), `wallonia_renewables`, or
  `brussels_renewables`, per region. Each raises on a miss in its own region.
- `region_connection_fee`: the Wallonia raccordement
  (`Redevance de raccordement`, `mega.py:720`), 0.0 outside Wallonia. Mandatory in
  Wallonia, raises on a miss.
- `energy_fund_eur_per_month`: 0.0.
- `vat_rate`: **0.0**, meaning the snapshot's prices are already VAT-incl (`mega.py:395`).
...
`_extract_injection` (`mega.py:650`) produces an `InjectionRates` from the same energy
...
when both `current` and `factor` are absent (`mega.py:681`).

### Supplier-side PV forfait

`_extract_supplier_prosumer` (`mega.py:407`) parses the compensation-regime
...
`Tarif prosumer` column, exactly like the Cociter Variable forfait (`mega.py:413`).
...
(`mega.py:434`). `test_supplier_pv_forfait_absent_on_brussels_and_flanders_dynamic`
...
- **Consumption / injection formulas are printed in reverse order.** Injection comes
  ...
  (`mega.py:442`, `test_mega.py:141`).
- **The injection base can be an en-dash, not an ASCII hyphen.** `SIGN_CHARS` /
  `parse_sign` handle every Unicode dash variant (`_pdf.py:506`); do not narrow the
...
- **VAT convention is TVAC (`vat_rate=0.0`).** Both energy and taxes are already
  ...
  (`mega.py:395`, `mega.py:413`).
- **`Compteur bi-horaire` is split across a newline by pypdf**, so a literal `find`
  never matched; the anchor uses `Compteur\s+bi-horaire` (`mega.py:594`). The same
...
- **`Redevance fixe` heading splits differently on dynamic vs fixed cards**; accept
  both the joined and the split layout (`mega.py:604`).
...
- **DSO label-to-key is not one-to-one:** `Fluvius Kempen` maps to IVEKA,
  `Fluvius Midden-Vlaanderen` to INTERGEM (`mega.py:772`).
- **Prosumer rates live in a separate table** from the main DSO row in both Flanders
  ...
  distribution rate (`mega.py:797`, `mega.py:856`).
- **Dynamic cards carry no compensation regime:** no supplier forfait, no
  ...
  is broken out into a separate paragraph (`mega.py:434`, `mega.py:789`).
...
- **Mandatory-line policy:** the yearly fee, federal excise, energy contribution,
  ...
  fails loudly instead of mis-billing (`mega.py:436`, `mega.py:612`, `mega.py:701`,
  `mega.py:728`, `mega.py:747`).
- **`fetch_for_month` must rotate two month placeholders and preserve the effective
  ...
  `archive_validity_check` (`mega.py:280`).
...
1. `_find_pdf_url` (`mega.py:184`) and the listing HTML shape. If Mega changes the
...
2. `_extract_meter_value` / `_extract_impact_tier` / `_extract_yearly_fee`
   (`mega.py:579`, `mega.py:566`, `mega.py:604`). Label wording or the label / value
   newline split is the most common drift; these anchor on French labels
   (`Compteur mono-horaire`, `Tarif jour`, `Redevance fixe`).
3. `_CONSUMPTION_FORMULA_RE` / `_INJECTION_FORMULA_RE` (`mega.py:455`). A reworded
...
4. The DSO row parsers (`mega.py:775`, `mega.py:848`, `mega.py:906`). A changed
   ...
   one.
5. `_extract_supplier_prosumer` (`mega.py:407`). A reworded `Forfait panneaux` line,
...
6. `fetch_for_month` (`mega.py:272`). If Mega changes the effective-date suffix
