...
`<REGION>` is one of `VL` / `WL` / `BX` (`_REGION_TO_CODE`, `mega.py`). The
...
`mega.py`), where every product card exposes an
`<a data-product-element="<Product Name>" ... href="<PDF URL>">` anchor, and matches
the anchor to its PDF with a regex (`_find_pdf_url`, `mega.py`).

The `source_url` recorded on the snapshot is the resolved PDF URL for the live
`fetch` (`mega.py`); the pure `parse_snapshot` defaults it to `_LISTING_URL`
when called without one (`mega.py`).
...
| `mega_zen_fixed` | Mega Zen Fixed (3 years) | `fixed` | all three | off the listing for the August 2026 card only |
...
- Mega Cap ("prix variable plafonne") was discontinued with the September 2026
  ...
  edition with its HTML stub, while August's cards are still served; only the
  `data-filter-tarif="CAP"` button survives on the listing, with no product
  behind it. The B2B half is the one that had to go with it: `fetch` rolls a
  professional card back one month when the current one is missing, so the
  contract kept pricing off August's variable index instead of failing
  (`mega.py`). The `_energy_price_ceiling` parser and the `ceiling_*`
  fields stay: they key off the card's own "plafond" sentence, not off the
  product, so they cost nothing and work again the day a card prints one.
...
`mega.py`) -- SME included, since the built filename is the same one the
listing links:
...
Fixed kept its own through the month its residential edition was off the
listing -- which is why the catalog diff never flagged the return: `known` was
built from every contract's `product_name`, professional ones included, so the
B2B entry covered the listing name for a residential product that was gone.
...
`fetch(session, contract_id, region)` (`mega.py`):

1. Validate the contract id and resolve the region to its `VL` / `WL` / `BX` code.
2. GET the listing HTML (`_fetch_listing_html`, `mega.py`).
3. Resolve the current PDF URL with `_resolve_pdf_url` (`mega.py`). It first
   regexes the listing with `_find_pdf_url` (`mega.py`), which pins the pattern
   ...
   matching (`mega.py`). When that regional block is missing but the card is
   ...
   (`_REGION_SEGMENT_RE`, `mega.py`). The test
   `test_listing_url_finder_picks_electricity_for_region` (`test_mega.py`) confirms
   a Smart Fixed / WL match ends `Smart2204-Fixed.pdf` and never contains `NG`;
   `test_resolver_falls_back_to_sibling_region_when_block_missing` (`test_mega.py`)
   covers the fallback.
4. Download the PDF text (`fetch_pdf_text`, `_pdf.py`) and hand it to
   `parse_snapshot`, which asserts the card's own `Client résidentiel - <Region>`
   header matches the requested region (`_assert_card_region`, `mega.py`) so a
...
(`mega.py`). Mega's listing has neither `Last-Modified` nor `ETag`, so the
...
`fetch_for_month(session, contract_id, region, year_month)` (`mega.py`) uses
...
2. Match the `-MMYYYY-` segment (`mmyyyy_re`, `mega.py`) and capture the current
...
3. In the filename tail after the `-MMYYYY-` segment, rewrite the two-digit
   ...
   Online0105-Fixed`, `Cosy1306 -> Cosy1305`) (`mega.py`).
...
(`test_mega.py`) pins the `-B2B-` segment, the variant suffix and the single
fetch attempt.

### Catalog discovery

`discover(session)` (`mega.py`) returns every `data-product-element` value on
...
2026-05-05, `test_discover_filters_known_unsupported_products`, `test_mega.py`).
On a listing fetch failure it returns an empty set rather than raising.

## Parsing

`parse_snapshot(contract_id, text, region, source_url)` (`mega.py`) is the pure,
unit-tested parser. It first asserts the card is the requested region's edition
(`_assert_card_region`, `mega.py`), anchoring on the `Client résidentiel -
...
| Regional renewables | `_extract_flanders_renewables` / `_extract_renewables` | `_mega_overlays.py` |
| DSO overlay | `_extract_flanders_dsos` / `_extract_wallonia_dsos` / `_extract_brussels_dsos` | `_mega_overlays.py`, `_mega_overlays.py` |
| Supplier PV forfait | `_extract_supplier_prosumer` | `_mega_overlays.py` |

### Energy block

`_extract_energy` (`_mega_cards.py`) always reads the yearly standing charge first
(`_extract_yearly_fee`, `_mega_cards.py`), which accepts both the split dynamic layout
(`Redevance fixe\n(€/an)\n42.4`) and the joined fixed layout
(`Redevance fixe (€/an)\n111.3`). A missing standing charge raises (it is on every
card); `test_missing_yearly_fee_is_fatal` (`test_mega.py`) enforces it.
...
- `fixed` / `variable`: read `Compteur mono-horaire` (mono), plus `Tarif jour`
  ...
  `_extract_meter_value` (`_mega_cards.py`). The bi-hourly labels are only read inside
...
> in c€/kWh. `_realized_rates` (`_mega_cards.py`) parses that sentence and both
...
The region dispatch in `parse_snapshot` (`mega.py`) selects exactly one DSO
parser and one renewables levy per snapshot.

Flanders (`_extract_flanders_dsos`, `_mega_overlays.py`, `_FLANDERS_LABELS` `_mega_overlays.py`)
...
Brussels (`_extract_brussels_dsos`, `_mega_overlays.py`) maps the single Sibelga row, an
...
`_extract_publication_month` (`_mega_cards.py`) first tries the versioned Smart Fixed
prefix `V<n> <month> <year>` (the token class includes `é` and `û` so `août` keeps
its version, `test_publication_month_keeps_version_for_august`, `test_mega.py`),
then falls back to `Prix du mois MM/YYYY` rendered as `<month-name> YYYY` from
`_FR_MONTH_NAMES` (`_mega_cards.py`). `valid_until` prefers the shared
`parse_valid_until` keyword scan and falls back to `_extract_valid_until`
(`mega.py`), which reads `mois MM/YYYY` and returns the last calendar day of that
...
- `vat_rate`: **0.0**, meaning the snapshot's prices are already VAT-incl (`mega.py`).
  This is the same convention as Eneco and Cociter; do not add a VAT multiplier when
  parsing.

The test `test_taxes_split_correctly_per_region` (`test_mega.py`) pins the
...
- **`fetch_for_month` must rotate two month placeholders and preserve the effective
  ...
  `archive_validity_check` (`mega.py`).
...
1. `_find_pdf_url` (`mega.py`) and the listing HTML shape. If Mega changes the
...
4. The DSO row parsers (`_mega_cards.py`, `_mega_cards.py`). A changed
   ...
   one.
5. `_extract_supplier_prosumer` (`_mega_overlays.py`). A reworded `Forfait panneaux` line,
...
6. `fetch_for_month` (`mega.py`). If Mega changes the effective-date suffix
