...
3. Resolve the current PDF URL with `_resolve_pdf_url` (`mega.py:377`). It first
   ...
   covers the fallback.
4. Download the PDF text (`fetch_pdf_text`, `_pdf.py:284`) and hand it to
...
4. Download, parse, then cross-check the parsed card against the requested month
   with `archive_validity_check` (`_pdf.py:1126`), passing `_FR_MONTH_NAMES`
...
`_is_pdf_payload`, `_pdf.py:140`, rejects it), when the parse fails, or when the
...
| --- | --- | --- |
| Energy rates (per kind) | `_extract_energy` | `_mega_cards.py:257` |
| Injection | `_extract_injection` | `_mega_cards.py:574` |
| Publication label | `_extract_publication_month` | `_mega_cards.py:535` |
| Valid until | `parse_valid_until` then `_extract_valid_until` | `_pdf.py:1165`, `_mega_cards.py:554` |
...
`_extract_energy` (`_mega_cards.py:257`) always reads the yearly standing charge first
(`_extract_yearly_fee`, `_mega_cards.py:520`), which accepts both the split dynamic layout
...
- `tou_impact`: parse three CWaPE bands (`_extract_impact_tier` for `PIC`,
  ...
  `_impact_band_coefficients` (`_mega_cards.py:222`) also parses the footnote's three
...
- `fixed` / `variable`: read `Compteur mono-horaire` (mono), plus `Tarif jour`
  ...
  `_extract_meter_value` (`_mega_cards.py:495`). The bi-hourly labels are only read inside
...
> in c€/kWh. `_realized_rates` (`_mega_cards.py:400`) parses that sentence and both
...
shared `parse_brussels_osp` (`_pdf.py:908`) and keyed by connection-power tier. The
...
`_extract_publication_month` (`_mega_cards.py:535`) first tries the versioned Smart Fixed
...
`_FR_MONTH_NAMES` (`_mega_cards.py:532`). `valid_until` prefers the shared
...
`_extract_injection` (`_mega_cards.py:574`) produces an `InjectionRates` from the same energy
...
- **The injection base can be an en-dash, not an ASCII hyphen.** `SIGN_CHARS` /
  `parse_sign` handle every Unicode dash variant (`_pdf.py:878`); do not narrow the
