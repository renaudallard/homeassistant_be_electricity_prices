...
3. Resolve the current PDF URL with `_resolve_pdf_url` (`mega.py:334`). It first
   ...
   `test_listing_url_finder_picks_electricity_for_region` (`test_mega.py:76`) confirms
...
2026-05-05, `test_discover_filters_known_unsupported_products`, `test_mega.py:700`).
...
| Valid until | `parse_valid_until` then `_extract_valid_until` | `_pdf.py:1004`, `_mega_cards.py:528` |
...
(`_extract_yearly_fee`, `_mega_cards.py:494`), which accepts both the split dynamic layout
(`Redevance fixe\n(€/an)\n42.4`) and the joined fixed layout
(`Redevance fixe (€/an)\n111.3`). A missing standing charge raises (it is on every
card); `test_missing_yearly_fee_is_fatal` (`test_mega.py:412`) enforces it.
...
- `fixed` / `variable`: read `Compteur mono-horaire` (mono), plus `Tarif jour`
  ...
  `_extract_meter_value` (`_mega_cards.py:469`). The bi-hourly labels are only read inside
...
> in c€/kWh. `_realized_rates` (`_mega_cards.py:374`) parses that sentence and both
...
test `test_dynamic_consumption_and_injection_are_not_swapped` (`test_mega.py:227`)
...
(`test_wallonia_dso_carries_prosumer_rate_from_separate_table`, `test_mega.py:287`).
...
`_extract_publication_month` (`_mega_cards.py:509`) first tries the versioned Smart Fixed
prefix `V<n> <month> <year>` (the token class includes `é` and `û` so `août` keeps
its version, `test_publication_month_keeps_version_for_august`, `test_mega.py:405`),
then falls back to `Prix du mois MM/YYYY` rendered as `<month-name> YYYY` from
`_FR_MONTH_NAMES` (`_mega_cards.py:506`). `valid_until` prefers the shared
...
The test `test_taxes_split_correctly_per_region` (`test_mega.py:380`) pins the
cross-region excise (0.0503288) and the per-region renewables split.

## Injection

`_extract_injection` (`_mega_cards.py:548`) produces an `InjectionRates` from the same energy
