...
3. Resolve the current PDF URL with `_resolve_pdf_url` (`mega.py:377`). It first
   ...
   covers the fallback.
4. Download the PDF text (`fetch_pdf_text`, `_pdf.py:283`) and hand it to
...
4. Download, parse, then cross-check the parsed card against the requested month
   with `archive_validity_check` (`_pdf.py:1058`), passing `_FR_MONTH_NAMES`
...
| Valid until | `parse_valid_until` then `_extract_valid_until` | `_pdf.py:1097`, `_mega_cards.py:554` |
...
shared `parse_brussels_osp` (`_pdf.py:840`) and keyed by connection-power tier. The
...
- **The injection base can be an en-dash, not an ASCII hyphen.** `SIGN_CHARS` /
  `parse_sign` handle every Unicode dash variant (`_pdf.py:810`); do not narrow the
