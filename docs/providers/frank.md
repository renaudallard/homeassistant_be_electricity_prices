...
(`providers/frank.py:135`) fix the region to `REGION_FLANDERS`; `fetch` rejects any other
region with "Frank Energie only operates in Flanders" (`providers/frank.py:238`). All
...
(`providers/frank.py:84`); it is public and needs no auth.
...
`_resolve_pdf_url` (`providers/frank.py:220`).

## Contracts

Five tiers are declared in `_TIERS` (`providers/frank.py:104`) and turned into `Contract`
objects by the `EXTRACTOR` comprehension (`providers/frank.py:506`). Every one is
...
`base.py:69`). None of them sets `quarter_hourly`, so all bill per clock hour: Frank is
listed in the `DynamicRates` docstring as an hourly-billing supplier
(`base.py:132`). The integration aggregates the ENTSO-E 15-minute curve to hourly for
these contracts.
...
(`providers/frank.py:122`). `test_matches_suffix_slim_accepts_both_sl_and_full_word`
(`tests/test_frank.py:335`) pins this behaviour.

No tier is retired. `discover()` (`providers/frank.py:285`) surfaces any unrecognised
...
`fetch` (`providers/frank.py:231`) validates the contract id and region, calls
...
`_resolve_pdf_url` (`providers/frank.py:177`) builds a GROQ query. With no target month it
...
(`providers/frank.py:141`), which extracts the word after "Dynamisch" and checks: for the
...
(`providers/frank.py:216`). A no-match raises `ExtractorError`
(`providers/frank.py:211`).

### Probe (freshness key)

`probe` (`providers/frank.py:264`) returns the `_createdAt` of the single newest
...
(`providers/frank.py:152`) normalises this: after `json.loads(...).get("result", [])` it
checks `isinstance(result, dict)` and wraps a lone dict in a list
(`providers/frank.py:170`). Do not remove that branch or `probe` and any future `[0]`
...
`fetch_for_month` (`providers/frank.py:245`) passes `target_month=year_month` into
...
(`providers/frank.py:192`). The Dutch month title comes from `_NL_MONTHS_TITLE`
(`providers/frank.py:86`) indexed by `target_month.month - 1`.

After parsing, the result is passed through `archive_validity_check(snap, text,
year_month, month_names=_NL_MONTHS)` (`providers/frank.py:261`). Because Frank cards carry
...
the wrong card (`_pdf.py:664`). Because `month_names` is supplied, a snapshot with no
...
(`providers/frank.py:259`), letting the coordinator fall back to the current snapshot as a
proxy.

## Parsing

`parse_snapshot` (`providers/frank.py:323`) assembles the `SupplierSnapshot` from five
...
| field | parser | source |
| --- | --- | --- |
| `energy` (`DynamicRates`) | `_extract_dynamic` | `providers/frank.py:362` |
| `injection` (`InjectionRates`) | `_extract_injection` | `providers/frank.py:403` |
| `taxes` (`TaxOverlay`) | `_extract_taxes` | `providers/frank.py:440` |
| `dsos` (`dict[str, DsoOverlay]`) | `_extract_dsos` | `providers/frank.py:468` |
| `valid_until` | `parse_valid_until` (shared) | `_pdf.py:677` |

### Number format

`_NUM = r"([\d]+(?:[.,][\d]+)?)"` (`providers/frank.py:349`) accepts both decimal
separators and `to_float` normalises either. The comment (`providers/frank.py:344`)
...
`test_dot_decimal_render_matches_comma` (`tests/test_frank.py:140`) pins that a comma card
and its dot-replaced twin parse identically.

## Energy formula

`_extract_dynamic` (`providers/frank.py:362`) parses the PDF formula row with `_FORMULA_RE`
(`providers/frank.py:351`), which matches:
...
(`tests/test_frank.py:63`) pins the standard April card at factor `0.1068 * 1.06 * 10`
...
`Abonnementskost (EUR/maand) <num>` (`providers/frank.py:357`) and multiplied by 12 to
`yearly_fixed_fee` (`providers/frank.py:384`). A missing row is fatal: the comment notes
the ~35 EUR/yr charge is mandatory, so a miss raises "monthly fixed fee row not found"
rather than silently billing zero (`providers/frank.py:383`).
`test_missing_monthly_fee_is_fatal` (`tests/test_frank.py:158`) locks this. The April
fixture's 2,92 EUR/month resolves to 35.04 EUR/year (illustrative,
`tests/test_frank.py:77`).
...
spot-indexed-variable shape. `_extract_injection` (`providers/frank.py:403`) parses a
`terugleveringsvergoeding` row with `_INJECTION_RE` (`providers/frank.py:395`):
...
`base.py:216`), so no `vat_mult` is applied (`providers/frank.py:417`):
...
EUR/kWh (`providers/frank.py:406`). `test_missing_injection_is_fatal`
(`tests/test_frank.py:173`) pins this. The sign between BELPEX and the base is mandatory
in the regex; a sign-less or reworded formula misses and raises rather than defaulting to
minus (`providers/frank.py:412`).

The April fixture yields factor `0.1 * 10 = 1.0` and base `-1.150 / 100` (illustrative,
`tests/test_frank.py:198`). Watch the JN tier: it carries a different injection base
(-0,02 vs -0,0115 on the other four tiers), pinned by the parametrized tier test
(`tests/test_frank.py:231` and the comment at `tests/test_frank.py:253`).
...
`_extract_taxes` (`providers/frank.py:440`) parses five levy rows and builds a `TaxOverlay`.
All card values are VAT-inclusive (6% BTW), so `vat_rate=0.0` is set explicitly
(`providers/frank.py:461`, comment at :453) and pinned by `test_taxes_vat_rate_zero`
(`tests/test_frank.py:192`).
...
(cogeneration) surcharges (`providers/frank.py:457`). Because Frank is Flanders-only, both
are mandatory renewables levies on every card; a miss raises "could not parse Frank
Energie GSC/WKK levies" rather than under-billing (`providers/frank.py:448`).
`test_missing_gsc_wkk_is_fatal` (`tests/test_frank.py:165`) locks this, and
`test_taxes_flanders_renewables_gsc_plus_wkk` pins GSC 1,166 + WKK 0,371 = 1,537 EURct/kWh
(illustrative, `tests/test_frank.py:181`).

The federal excise and energy contribution are also mandatory; a miss on either raises
"could not parse Frank Energie tax block" (`providers/frank.py:443`). The energy fund is
optional and defaults to 0.0; the April fixture has no residential fund row, pinned by
`test_taxes_energy_fund_residential_zero` (`tests/test_frank.py:187`). All EURct/kWh values
are divided by 100 to reach EUR/kWh.

## DSO overlay

`_extract_dsos` (`providers/frank.py:468`) covers all eight Fluvius sub-areas via
`_FLUVIUS_LABELS` (`providers/frank.py:124`), which maps the card's human label to the
canonical DSO key:
...
(`tests/test_frank.py:84`) asserts all eight are present.

The parser first narrows to the digital-meter section between the "Digitale meter" and
"Klassieke meter" markers (`providers/frank.py:469`); a missing "Digitale meter" marker
...
by `test_dso_transport_is_zero` (`tests/test_frank.py:114`). A label that does not match is
...
- The bracket around the label is character-class-tolerant: `[\[\(]...[\]\)]`
  (`providers/frank.py:479`) accepts a mismatched open/close bracket. Frank's PDF renders
  Kempen as `Fluvius [Kempen)` with mismatched brackets;
  `test_dso_kempen_despite_bracket_artifact` (`tests/test_frank.py:121`) pins that it still
  parses.
- The hyphen in labels like "Halle-Vilvoorde" is loosened to `[\s\-]*` so a space or
  missing hyphen in the rendered text still matches (`providers/frank.py:477`).

`test_dso_antwerpen_distribution` and `test_dso_antwerpen_capacity_and_databeheer`
(`tests/test_frank.py:99`) pin the Antwerpen row (illustrative: normaal 5,35 ct/kWh, excl
nacht 4,81 ct/kWh, capacity 52,37 EUR/kW/yr, databeheer 18,92 EUR/yr).

## valid_until

`parse_valid_until` (`_pdf.py:677`) is the shared best-effort validity parser; Frank cards
resolve to the last day of the pricing month. `test_valid_until_is_end_of_april`
(`tests/test_frank.py:273`) pins the April fixture to a date in month 4, year 2026. This
...
- GROQ `[0]` returns a dict, not a list. `_sanity_query` wraps a lone dict
  (`providers/frank.py:170`); removing that branch breaks `probe`.
- Suffix is not the tier name. Korting's filename token is `VT`, Slim's is `SL` or the
  full word `Slim` (aliased in `_SUFFIX_ALIASES`, `providers/frank.py:122`). Both Slim
  spellings are live simultaneously.
- Both decimal separators must be accepted (`_NUM`, `providers/frank.py:349`); a
...
- VAT applied only to energy. The `x 1,06` multiplier scales the energy factor and base
  (`providers/frank.py:375`) but not injection (VAT-exempt, `providers/frank.py:417`).
  Card levy values are already VAT-incl, so `TaxOverlay.vat_rate=0.0`
  (`providers/frank.py:461`).
- Fail-loud on missing mandatory rows: energy formula, monthly fee, injection formula,
  ...
  Four fatal-miss tests guard these (`tests/test_frank.py:158`-179, :165, :173).
- Mismatched bracket artifact in the DSO table (`Fluvius [Kempen)`) is tolerated by the
  character-class brackets (`providers/frank.py:479`).
...
`test_non_default_tiers_extract_energy_and_injection` (`tests/test_frank.py:212`) was added
with the other four fixtures to catch a tier-specific card regression. Tests load fixtures
through `fixture_text(name, layout=True)` (`tests/test_frank.py:42`), matching the
...
| --- | --- | --- |
| A tier stops fetching, or a new sixth tier is ignored | `_TIERS`, `_TIER_SUFFIX`, `_SUFFIX_ALIASES`, `_matches_suffix` (`providers/frank.py:104`-149) | filename token renamed or a new suffix appears; `discover` will surface `frank_dynamic_<suffix>` |
