...
Flanders. The module docstring (`providers/frank.py`) and `_FRANK_REGIONS`
(`providers/frank.py`) fix the region to `REGION_FLANDERS`; `fetch` rejects any other
region with "Frank Energie only operates in Flanders" (`providers/frank.py`). All
...
(`providers/frank.py`); it is public and needs no auth.

```
config (contract_id) ->  probe(): GROQ newest _createdAt  -> freshness key
                     \-> fetch():  GROQ "*Dynamisch*" (gas excluded)
                                     |
                                     v
...
`_resolve_pdf_url` (`providers/frank.py`).
...
`fetch` (`providers/frank.py`) validates the contract id and region, calls
...
`_resolve_pdf_url` (`providers/frank.py`) builds a GROQ query. With no target month it
asks for every "Dynamisch" file asset ordered newest first (`[0..29]`):

```groq
*[_type=="sanity.fileAsset" && originalFilename match "*Dynamisch*"
  && !(originalFilename match "*Gas*")]
  {originalFilename,url,_createdAt} | order(_createdAt desc)[0..29]
```

The selector is shared by all four lookups as `_CARD_SELECT`, and the two halves
are each load-bearing. It does **not** key on "Elektriciteit": Frank dropped that
token in September 2026, renaming
`... Tariefkaart Elektriciteit Dynamisch HV  Augustus 2026.pdf` to
`... Tariefkaart Dynamisch HV September 2026.pdf`. A filter carrying the old token
matched nothing newer than August, so every tier silently served the previous
month's card, and the probe could not see it either because its key came from the
same query. Gas is then excluded explicitly rather than left to its own naming,
because the same release also renamed "Gas ZTP" to "Gas" - matching "Dynamisch"
alone would adopt a future "Gas Dynamisch" card as an electricity tier.

The returned rows are filtered through `_matches_suffix(filename, suffix)`
(`providers/frank.py`), which extracts the word after "Dynamisch" and checks: for the
...
(`providers/frank.py`). A no-match raises `ExtractorError`
(`providers/frank.py`).

### Probe (freshness key)

`probe` (`providers/frank.py`) returns the `_createdAt` of the single newest
...
(`providers/frank.py`) normalises this: after `json.loads(...).get("result", [])` it
checks `isinstance(result, dict)` and wraps a lone dict in a list
(`providers/frank.py`). Do not remove that branch or `probe` and any future `[0]`
...
`fetch_for_month` (`providers/frank.py`) passes `target_month=year_month` into
...
(`providers/frank.py`). The Dutch month title comes from `_NL_MONTHS_TITLE`
(`providers/frank.py`) indexed by `target_month.month - 1`.
...
`parse_snapshot` (`providers/frank.py`) assembles the `SupplierSnapshot` from five
...
| field | parser | source |
| --- | --- | --- |
| `energy` (`DynamicRates`) | `_extract_dynamic` | `providers/frank.py` |
...
`_NUM = r"([\d]+(?:[.,][\d]+)?)"` (`providers/frank.py`) accepts both decimal
separators and `to_float` normalises either. The comment (`providers/frank.py`)
...
`test_dot_decimal_render_matches_comma` (`tests/test_frank.py`) pins that a comma card
and its dot-replaced twin parse identically.

## Energy formula

`_extract_dynamic` (`providers/frank.py`) parses the PDF formula row with `_FORMULA_RE`
(`providers/frank.py`), which matches:
...
(`tests/test_frank.py`) pins the standard April card at factor `0.1068 * 1.06 * 10`
...
`Abonnementskost (EUR/maand) <num>` (`providers/frank.py`) and multiplied by 12 to
`yearly_fixed_fee` (`providers/frank.py`). A missing row is fatal: the comment notes
the ~35 EUR/yr charge is mandatory, so a miss raises "monthly fixed fee row not found"
rather than silently billing zero (`providers/frank.py`).
`test_missing_monthly_fee_is_fatal` (`tests/test_frank.py`) locks this. The April
...
spot-indexed-variable shape. `_extract_injection` (`providers/frank.py`) parses a
`terugleveringsvergoeding` row with `_INJECTION_RE` (`providers/frank.py`):
...
EUR/kWh (`providers/frank.py`). `test_missing_injection_is_fatal`
(`tests/test_frank.py`) pins this. The sign between BELPEX and the base is mandatory
in the regex; a sign-less or reworded formula misses and raises rather than defaulting to
minus (`providers/frank.py`).
...
`_extract_taxes` (`providers/frank.py`) parses five levy rows and builds a `TaxOverlay`.
All card values are VAT-inclusive (6% BTW), so `vat_rate=0.0` is set explicitly
(`providers/frank.py`, comment at :439) and pinned by `test_taxes_vat_rate_zero`
(`tests/test_frank.py`).
...
(cogeneration) surcharges (`providers/frank.py`). Because Frank is Flanders-only, both
are mandatory renewables levies on every card; a miss raises "could not parse Frank
Energie GSC/WKK levies" rather than under-billing (`providers/frank.py`).
`test_missing_gsc_wkk_is_fatal` (`tests/test_frank.py`) locks this, and
`test_taxes_flanders_renewables_gsc_plus_wkk` pins GSC 1,166 + WKK 0,371 = 1,537 EURct/kWh
(illustrative, `tests/test_frank.py`).

The federal excise is mandatory; a miss raises "could not parse Frank Energie tax block"
(`providers/frank.py`), pinned by `test_missing_federal_excise_is_fatal`
(`tests/test_frank.py`).
...
levy being abolished and defaults to 0.0 (`providers/frank.py`, comment at :423);
`test_august_card_drops_the_energy_contribution_row` (`tests/test_frank.py`) pins it
...
`test_taxes_energy_fund_residential_zero` (`tests/test_frank.py`). All EURct/kWh values
are divided by 100 to reach EUR/kWh.

## DSO overlay

`_extract_dsos` (`providers/frank.py`) covers all eight Fluvius sub-areas via
`_FLUVIUS_LABELS` (`providers/frank.py`), which maps the card's human label to the
canonical DSO key:
...
by `test_dso_transport_is_zero` (`tests/test_frank.py`). A label that does not match is
...
- The bracket around the label is character-class-tolerant: `[\[\(]...[\]\)]`
  (`providers/frank.py`) accepts a mismatched open/close bracket. Frank's PDF renders
  Kempen as `Fluvius [Kempen)` with mismatched brackets;
  `test_dso_kempen_despite_bracket_artifact` (`tests/test_frank.py`) pins that it still
  parses.
- The hyphen in labels like "Halle-Vilvoorde" is loosened to `[\s\-]*` so a space or
  missing hyphen in the rendered text still matches (`providers/frank.py`).

`test_dso_antwerpen_distribution` and `test_dso_antwerpen_capacity_and_databeheer`
(`tests/test_frank.py`) pin the Antwerpen row (illustrative: normaal 5,35 ct/kWh, excl
nacht 4,81 ct/kWh, capacity 52,37 EUR/kW/yr, databeheer 18,92 EUR/yr).

## valid_until

`parse_valid_until` (`_pdf.py`) is the shared best-effort validity parser; Frank cards
...
- GROQ `[0]` returns a dict, not a list. `_sanity_query` wraps a lone dict
  (`providers/frank.py`); removing that branch breaks `probe`.
- Suffix is not the tier name. Korting's filename token is `VT`, Slim's is `SL` or the
  full word `Slim` (aliased in `_SUFFIX_ALIASES`, `providers/frank.py`). Both Slim
  spellings are live simultaneously.
- Both decimal separators must be accepted (`_NUM`, `providers/frank.py`); a
...
- VAT applied only to energy. The `x 1,06` multiplier scales the energy factor and base
  (`providers/frank.py`) but not injection (VAT-exempt, `providers/frank.py`).
...
- Fail-loud on missing mandatory rows: energy formula, monthly fee, injection formula,
  ...
  Taxes below. Five fatal-miss tests guard the rest (`tests/test_frank.py`-197,
...
- Mismatched bracket artifact in the DSO table (`Fluvius [Kempen)`) is tolerated by the
  character-class brackets (`providers/frank.py`).
