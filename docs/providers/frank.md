...
objects by the `EXTRACTOR` comprehension (`providers/frank.py:485`). Every one is
...
(`tests/test_frank.py:327`) pins this behaviour.
...
`test_missing_monthly_fee_is_fatal` (`tests/test_frank.py:150`) locks this. The April
...
`base.py:216`), so no `vat_mult` is applied (`providers/frank.py:396`):
...
(`tests/test_frank.py:165`) pins this. The sign between BELPEX and the base is mandatory
...
`tests/test_frank.py:190`). Watch the JN tier: it carries a different injection base
(-0,02 vs -0,0115 on the other four tiers), pinned by the parametrized tier test
(`tests/test_frank.py:223` and the comment at `tests/test_frank.py:245`).
...
Energie GSC/WKK levies" rather than under-billing (`providers/frank.py:427`).
`test_missing_gsc_wkk_is_fatal` (`tests/test_frank.py:157`) locks this, and
`test_taxes_flanders_renewables_gsc_plus_wkk` pins GSC 1,166 + WKK 0,371 = 1,537 EURct/kWh
(illustrative, `tests/test_frank.py:173`).

The federal excise and energy contribution are also mandatory; a miss on either raises
"could not parse Frank Energie tax block" (`providers/frank.py:422`). The energy fund is
optional and defaults to 0.0; the April fixture has no residential fund row, pinned by
`test_taxes_energy_fund_residential_zero` (`tests/test_frank.py:179`). All EURct/kWh values
are divided by 100 to reach EUR/kWh.

## DSO overlay

`_extract_dsos` (`providers/frank.py:447`) covers all eight Fluvius sub-areas via
...
"Klassieke meter" markers (`providers/frank.py:448`); a missing "Digitale meter" marker
...
- The bracket around the label is character-class-tolerant: `[\[\(]...[\]\)]`
  (`providers/frank.py:458`) accepts a mismatched open/close bracket. Frank's PDF renders
...
- The hyphen in labels like "Halle-Vilvoorde" is loosened to `[\s\-]*` so a space or
  missing hyphen in the rendered text still matches (`providers/frank.py:456`).
