...
| Field | Value | Source |
| --- | --- | --- |
| Extractor id | `eneco` | `eneco.py:635` |
| Label | `Eneco` | `eneco.py:636` |
| Regions served | Flanders and Wallonia (no Brussels) | `eneco.py:632`, module docstring `eneco.py:38` |
| Publication | one PDF per contract on the Eneco CDN | `eneco.py:28-36` |
| VAT convention | prices are VAT-inclusive (6 %), so `TaxOverlay.vat_rate = 0.0` | `eneco.py:36`, `eneco.py:550` |
| Probe | listing scrape returning the resolved PDF URL | `eneco.py:221-239` |
| Archive | per-month issues kept on the CDN, resolved by volume walk | `eneco.py:175-218` |

`EXTRACTOR.regions()` (the union over its contracts, `base.py:383`) is
...
(`eneco.py:30-34`, `eneco.py:89`):

```
https://cdn.eneco.be/downloads/nl/general/tk/BC_032_<ISSUE>_NL_ENECO_POWER_<NAME>.pdf
```

`<NAME>` is the product slug (`FIX`, `FLEX`, `DYNAMIC`, `eneco.py:112-116`).
...
is never hardcoded; it is resolved from the listing page (`eneco.py:31-35`):
...
| --- | --- | --- | --- | --- |
| `power_fix` | Eneco Zon & Wind Vast | `fixed` | flanders, wallonia | `eneco.py:638-643` |
| `power_flex` | Eneco Zon & Wind Flex | `variable` | flanders, wallonia | `eneco.py:644-649` |
| `power_dynamic` | Eneco Zon & Wind Dynamisch | `dynamic` | flanders only | `eneco.py:650-660` |
...
- **Power Flex** is a variable (monthly-indexed) contract: the card prints the
  ...
  (`_extract_variable`, `eneco.py:341-367`).
- **Power Dynamic** is an hourly dynamic contract indexed on the hourly spot
  ...
  reference and must not be offered in Wallonia (`eneco.py:651-660`, enforced by
  `test_power_dynamic_offered_in_flanders_only`, `tests/test_eneco.py:54-61`).
- `DynamicRates.quarter_hourly` is left at its default `False` (`_extract_dynamic`
  returns a `DynamicRates` without setting it, `eneco.py:399-403`). Eneco Dynamic
  bills per clock hour, so the integration aggregates the ENTSO-E 15-minute curve
  to hourly (`base.py:128-142`).
- No contract sets `spot_indexed_injection`; it stays `False` on all three
  (`Contract` default, `base.py:75`). See the injection section for why Fix / Flex
...
`fetch` (`eneco.py:157-172`) validates the contract id, scrapes the listing,
...
every region the contract is sold in (`eneco.py:160`).

```
fetch(session, contract_id, region)
  |-> _fetch_listing            GET the tariefkaarten HTML  (eneco.py:259-260)
  |-> _resolve_url              regex the live PDF URL      (eneco.py:263-285)
  |-> fetch_pdf_text            download + pypdf text       (_pdf.py:158-165)
  '-> parse_snapshot            build the SupplierSnapshot  (eneco.py:288-302)
```

`_resolve_url` (`eneco.py:263-285`) accepts either a full href
...
`probe` (`eneco.py:221-239`) returns the resolved current PDF URL as the freshness
key. A header-only (HEAD) probe is unusable here because the listing returns a
per-request ETag under `Cache-Control: no-store` (`eneco.py:228-232`). Because the
...
`fetch_for_month` (`eneco.py:175-218`) supports the time-correct yearly-cost flow
...
For the requested month it walks volumes `01`..`05` (`eneco.py:196`). For each
candidate:

1. `head_freshness_key` HEAD-probes the URL first (`eneco.py:205`). A missing
...
3. `parse_snapshot` parses; an `ExtractorError` skips the volume
   (`eneco.py:211-214`).
4. `archive_validity_check` (`_pdf.py:638-674`) confirms the snapshot actually
   covers `year_month`, passing `month_names=_NL_MONTHS` (`eneco.py:215`). This
...
(`eneco.py:218`) or when the contract id is unknown (`eneco.py:192-194`); the
coordinator then falls back to the current snapshot as a proxy. This behaviour is
pinned by `test_fetch_for_month_*` (`tests/test_eneco.py:317-375`): a match
...
`discover` (`eneco.py:242-256`) returns the set of `power_<name>` slugs advertised
on the listing, extracted from every `BC_..._NL_ENECO_POWER_<NAME>.pdf` link and
lower-cased to match the registry contract ids.

## Parsing

`parse_snapshot` (`eneco.py:288-302`) assembles the snapshot from six extractors.
...
SupplierSnapshot(
    ...
    energy=_extract_energy(text, contract_id),      # eneco.py:310-317
    dsos=_extract_dsos(text),                        # eneco.py:406-418
    taxes=_extract_taxes(text),                      # eneco.py:494-551
    source_url=source_url,
    publication_label=_extract_publication_month(text),  # eneco.py:305-307
    valid_until=parse_valid_until(text),             # _pdf.py:677
    injection=_extract_injection(text, contract_id), # eneco.py:554-629
)
```

### The `_NUM` numeric token

The shared numeric token (`eneco.py:153`) is the parsing linchpin. It matches
...
(`tests/test_eneco.py:202-215`) locks both the NBSP-grouped and ungrouped
four-digit round-trips. `_WS` (`eneco.py:154`) matches ASCII whitespace or NBSP
and is used to span line wraps in the tax block. All numeric values are parsed via
`to_float` (`_pdf.py:452-463`), which strips every Unicode space variant before
swapping comma for dot.

### Publication label and validity

`_extract_publication_month` (`eneco.py:305-307`) captures `Tariefkaart <month>
<year>` (for example `mei 2026`). `valid_until` comes from the shared
`parse_valid_until` (`_pdf.py:677`), which reads the "Geldig van ... t.e.m. ..."
line. `test_extracts_valid_until_from_geldig_line` (`tests/test_eneco.py:239-251`)
...
`_extract_energy` (`eneco.py:310-317`) dispatches on the contract id.

| kind | helper | fields returned | key anchors |
| --- | --- | --- | --- |
| fixed | `_extract_fixed` (`eneco.py:320-338`) | `single`, `peak` (day), `offpeak` (night), `exclusive_night`, `yearly_fixed_fee` | `DAG NACHT` header then five `_NUM` |
| variable | `_extract_variable` (`eneco.py:341-367`) | `current`, `yearly_fixed_fee`, `formula` | `(€/jaar)` + `Geschatte jaarprijs`; `Maandprijs`; Belpex formula |
| dynamic | `_extract_dynamic` (`eneco.py:370-403`) | `factor`, `base`, `yearly_fixed_fee` | `Enkelvoudige meter`; `(f X BELPEX-H +- base) X vat` |
...
values from `test_fix_extracts_energy_block` (`tests/test_eneco.py:64-71`):
...
the reason (`eneco.py:342-347`): a single extra header line on a future card broke
the earlier rigid four-newline skip and took Power Flex offline.
`test_flex_yearly_fee_survives_extra_header_line` (`tests/test_eneco.py:190-199`)
...
between the Belpex factor and the base (`SIGN_CHARS`, `_pdf.py:473`) so a polarity
...
multiplier the card actually prints (`eneco.py:380-383`): `(f X BELPEX-H +- base)
...
conversion (`eneco.py:391-398`) turns the PDF's cents-per-kWh-from-EUR-per-MWh form
...
(`tests/test_eneco.py:218-231`): PDF `(0.102 X BELPEX-H + 1) X 1.06` yields
...
`_extract_dsos` (`eneco.py:406-418`) walks two label maps and emits a `DsoOverlay`
per matched row.

Wallonia (`_WALLONIA_LABELS`, `eneco.py:121-127`), via `_find_wallonia_row`
(`eneco.py:421-457`):
...
kept as the canonical `ores` (`eneco.py:118-120`, dedup guard at `eneco.py:409`).
...
code maps groups 4/5/6 to `medium`/`pic`/`eco` accordingly (`eneco.py:438-445`).
...
(`tests/test_eneco.py:74-89`): `distribution_single = 0.1087`,
...
Flanders (`_FLUVIUS_LABELS`, `eneco.py:135-144`), via `_find_fluvius_row`
(`eneco.py:460-491`):
...
(`eneco.py:473-477`) so it does not pick up the analogue-meter row further down.
Key modelling decisions (`eneco.py:460-491`):
...
- `prosumer_eur_per_kva_year` stays `None`: SMR3 connections do not sit under the
  ...
  `tests/test_eneco.py:92-96`).

`test_fix_extracts_all_fluvius_sub_areas` and
`test_fix_fluvius_sub_areas_have_distinct_rates` (`tests/test_eneco.py:99-141`)
...
`_extract_taxes` (`eneco.py:494-551`) builds a `TaxOverlay` with
`vat_rate = 0.0` (prices are already VAT-incl, `eneco.py:550`). Fields:

| field | source token | line |
| --- | --- | --- |
| `federal_excise` | first number in the "Verbruik tussen 0 en 3.000 kWh" tier | `eneco.py:495-504` |
| `energy_contribution` | second number in that tier | `eneco.py:498-505` |
| `flanders_renewables` | "Bijdrage groene stroom en WKK ... (€cent/kWh)" | `eneco.py:511-515`, `542` |
| `wallonia_renewables` | "Bijdrage groene stroom Wallonie ... (€cent/kWh)" | `eneco.py:516-520`, `543-547` |
| `region_connection_fee` | "Aansluitingsvergoeding elektriciteit ... (€cent/kWh)" | `eneco.py:521-534`, `548` |
| `energy_fund_eur_per_month` | "Standaard tarief (domicilieadres)" | `eneco.py:535-538`, `549` |
...
footnote markers that a lazy `_NUM` would otherwise capture (`eneco.py:507-510`).
Both regional renewables are populated from every card; the pricing engine selects
the right one per region (`tests/test_eneco.py:158-162`). Illustrative values from
`test_fix_extracts_taxes` (`tests/test_eneco.py:154-163`):
...
(`eneco.py:502-503`, `eneco.py:527-534`). The connection fee is a mandatory all-in
...
(`tests/test_eneco.py:144-151`) removes the label and asserts the raise.

### Injection

`_extract_injection` (`eneco.py:554-629`) is the most delicate block. Layout on
every contract:
...
Steps (`eneco.py:574-629`):

1. Anchor on the stable `AFNAME EN INJECTIE` prefix (`eneco.py:574`), then cut the
   section at the next ALL-CAPS heading (`ENERGIEDELEN`, `BELASTINGEN`, ...) so
   unrelated blocks do not pollute the matches (`eneco.py:580-584`).
2. Read the monthly indicative (`Maandprijs`), falling back to the yearly estimate
   (`Geschatte jaarprijs`) when no `Maandprijs` prints (`eneco.py:588-603`). Both
...
3. Only for `power_dynamic`, parse the Belpex-H formula into hourly-spot
   coefficients (`eneco.py:612-620`): `factor = factor_pdf * 10`,
   `base = base_cents / 100` (VAT-exempt for residential, so no VAT scaling).

Injection taxonomy (the three-shape rule, `base.py:214-234`):
...
- **Power Fix and Power Flex are monthly-indicative-only**: the extractor surfaces
  ...
  (`eneco.py:605-620`, the `contract_id == "power_dynamic"` guard). The card's
  ...
  to the hourly spot if the `Maandprijs` ever stopped printing (`eneco.py:607-611`).
  Illustrative: `current = 0.0476`, `factor = None`, `base = None`
  (`tests/test_eneco.py:254-277`).
- **Power Dynamic is hourly `factor * spot + base`**: it indexes on Belpex-H, so it
  ...
  `test_dynamic_extracts_injection_rates` (`tests/test_eneco.py:279-287`): formula
...
- **VAT-inclusive cards**: all prices are 6 % VAT-incl, so `vat_rate = 0.0` and the
  pricing engine does not rescale (`eneco.py:36`, `eneco.py:550`, `base.py:305-308`).
  Dynamic is the exception where a VAT multiplier is read from the card and folded
  into `factor` / `base` (`eneco.py:376-398`).
- **`AFNAME EN INJECTIE / VALORISATIE` rename (issue #35)**: the July 2026 cards
  ...
  keys off the stable `AFNAME EN INJECTIE` prefix (`eneco.py:570-574`), guarded by
  `test_injection_survives_valorisatie_suffix_drop` (`tests/test_eneco.py:290-307`).
- **`_NUM` four-digit truncation**: the previous integer-part cap of three digits
  ...
  and ungrouped four-digit values (`eneco.py:147-153`, guarded at
  `tests/test_eneco.py:202-215`).
- **Rigid newline skip broke Power Flex**: the variable yearly-fee parser used to
  ...
  `Geschatte jaarprijs` tokens (`eneco.py:342-352`, guarded at
  `tests/test_eneco.py:190-199`).
- **Fail-loud excise and connection fee**: unlike the gated renewables, these two
  raise on a regex miss to avoid silent under-billing (`eneco.py:502-503`,
  `527-534`).
...
- **Fluvius transport is 0**: the Flemish Afnametarief bundles Elia transmission;
  do not add a transport term (`eneco.py:132-134`, `468-469`).
- **Flemish digital meters have no peak / offpeak split and no prosumer rate**
  (`eneco.py:483-491`, `tests/test_eneco.py:92-96`).
...
- **Archive substitution guard**: `archive_validity_check` rejects a snapshot whose
  ...
  (`_pdf.py:638-674`, `tests/test_eneco.py:337-355`).
- **Sign flexibility**: every Belpex formula match (consumption and injection)
  ...
  not silently drop the formula or the base (`eneco.py:358-359`, `380-383`,
  `590-593`).
- **Power Dynamic is Flanders-only**: its Walloon DSO rows are vestigial reference;
  do not offer the product in Wallonia (`eneco.py:651-660`,
  `tests/test_eneco.py:54-61`).
...
| --- | --- | --- |
| every snapshot fails to fetch | `_resolve_url` / `_fetch_listing` (listing HTML or URL pattern changed) | `eneco.py:259-285` |
| fixed energy parse error | `_extract_fixed` (`DAG NACHT` header or column count changed) | `eneco.py:320-338` |
| variable energy or yearly fee wrong | `_extract_variable` anchors (`(€/jaar)`, `Geschatte jaarprijs`, `Maandprijs`) | `eneco.py:341-367` |
| dynamic factor / base wrong | `_extract_dynamic` formula regex or VAT multiplier | `eneco.py:370-403` |
| a DSO row missing | its label string in `_WALLONIA_LABELS` / `_FLUVIUS_LABELS`, or the row column count | `eneco.py:121-144`, `421-491` |
| Impact bands swapped | column-order mapping in `_find_wallonia_row` | `eneco.py:438-445` |
| tax value wrong or fatal error | `_extract_taxes` anchors (tier label, `(€cent/kWh)`, `Aansluitingsvergoeding`) | `eneco.py:494-551` |
| injection credit zeroed | `_extract_injection` heading anchor / section cutoff | `eneco.py:574-593` |
| four-digit fee truncated | `_NUM` token | `eneco.py:153` |
| historical month mis-billed | `fetch_for_month` volume walk or `archive_validity_check` | `eneco.py:175-218` |
