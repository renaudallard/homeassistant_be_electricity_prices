...
`EXTRACTOR.regions()` (the union over its contracts, `base.py:561`) is
...
`parse_valid_until` (`_pdf.py:858`), which reads the "Geldig van ... t.e.m. ..."
...
between the Belpex factor and the base (`SIGN_CHARS`, `_pdf.py:592`) so a polarity
