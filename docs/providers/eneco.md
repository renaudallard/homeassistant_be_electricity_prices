...
| Field | Value | Source |
| --- | --- | --- |
| Extractor id | `eneco` | `eneco.py` |
| Label | `Eneco` | `eneco.py` |
| Regions served | Flanders and Wallonia (no Brussels) | `eneco.py`, module docstring `eneco.py` |
| Publication | one PDF per contract on the Eneco CDN | `eneco.py` |
| VAT convention | prices are VAT-inclusive (6 %), so `TaxOverlay.vat_rate = 0.0` | `eneco.py` |
| Probe | listing scrape returning the resolved PDF URL | `eneco.py` |
...
`fetch` (`eneco.py`) validates the contract id, scrapes the listing,
...
(`eneco.py`) and reaches the tax overlay alone (`eneco.py`), where it
gates the Flemish energy fund out of Wallonia (`eneco.py`).

```
fetch(session, contract_id, region)
  |-> _fetch_listing            GET the tariefkaarten HTML  (eneco.py)
  |-> _resolve_url              regex the live PDF URL      (eneco.py)
  |-> fetch_pdf_text            download + pypdf text       (_pdf.py)
  '-> parse_snapshot            build the SupplierSnapshot  (eneco.py)
```

`_resolve_url` (`eneco.py`) accepts either a full href
...
`probe` (`eneco.py`) returns the resolved current PDF URL as the freshness
key. A header-only (HEAD) probe is unusable here because the listing returns a
per-request ETag under `Cache-Control: no-store` (`eneco.py`). Because the
...
`fetch_for_month` (`eneco.py`) supports the time-correct yearly-cost flow
...
(`eneco.py`) or when the contract id is unknown (`eneco.py`); the
coordinator then falls back to the current snapshot as a proxy. This behaviour is
pinned by `test_fetch_for_month_*` (`tests/test_eneco.py`): a match
...
`discover` (`eneco.py`) returns the set of `power_<name>` slugs advertised
on the listing, extracted from every `BC_..._NL_ENECO_POWER_<NAME>.pdf` link and
lower-cased to match the registry contract ids.

## Parsing

`parse_snapshot` (`eneco.py`) assembles the snapshot from six extractors.
...
SupplierSnapshot(
    ...
    energy=_extract_energy(text, contract_id),      # eneco.py
    dsos=_extract_dsos(text),                        # eneco.py
    taxes=_extract_taxes(text, region),              # eneco.py
    source_url=source_url,
    publication_label=_extract_publication_month(text),  # eneco.py
    valid_until=parse_valid_until(text),             # _pdf.py
    injection=_extract_injection(text, contract_id), # eneco.py
)
```

### The `_NUM` numeric token

The shared numeric token (`eneco.py`) is the parsing linchpin. It matches
...
`_extract_energy` (`eneco.py`) dispatches on the contract id.

| kind | helper | fields returned | key anchors |
| --- | --- | --- | --- |
| fixed | `_extract_fixed` (`eneco.py`) | `single`, `peak` (day), `offpeak` (night), `exclusive_night`, `yearly_fixed_fee` | `DAG NACHT` header then five `_NUM` |
| variable | `_extract_variable` (`eneco.py`) | `current`, `yearly_fixed_fee`, `formula`, `formula_factor` / `formula_base`, `month_indexed`, `rlp_indexed` | `(€/jaar)` + `Geschatte jaarprijs`; `Maandprijs`; Belpex formula |
| dynamic | `_extract_dynamic` (`eneco.py`) | `factor`, `base`, `yearly_fixed_fee` | `Enkelvoudige meter`; `(f X BELPEX-H +- base) X vat` |
...
values from `test_fix_extracts_energy_block` (`tests/test_eneco.py`):
...
the reason (`eneco.py`): a single extra header line on a future card broke
...
multiplier the card actually prints (`eneco.py`): `(f X BELPEX-H +- base)
...
conversion (`eneco.py`) turns the PDF's cents-per-kWh-from-EUR-per-MWh form
...
(`tests/test_eneco.py`): PDF `(0.102 X BELPEX-H + 1) X 1.06` yields
...
`_extract_dsos` (`eneco.py`) walks two label maps and emits a `DsoOverlay`
per matched row.

Wallonia (`_WALLONIA_LABELS`, `eneco.py`), via `_find_wallonia_row`
(`eneco.py`):
...
kept as the canonical `ores` (`eneco.py`, dedup guard at `eneco.py`).
...
code maps groups 4/5/6 to `medium`/`pic`/`eco` accordingly (`eneco.py`).
...
(`tests/test_eneco.py`): `distribution_single = 0.1087`,
...
Flanders (`_FLUVIUS_LABELS`, `eneco.py`), via `_find_fluvius_row`
(`eneco.py`):
...
section (`eneco.py`): the analogue-meter table below repeats every label
and is the same five figures wide, so nothing else tells the two apart.
Key modelling decisions (`eneco.py`):
...
`test_fix_fluvius_sub_areas_have_distinct_rates` (`tests/test_eneco.py`)
...
`_extract_taxes` (`eneco.py`) builds a `TaxOverlay` with
`vat_rate = 0.0` (prices are already VAT-incl, `eneco.py`). Fields:
...
footnote markers that a lazy `_NUM` would otherwise capture (`eneco.py`).
Both regional renewables are populated from every card; the pricing engine selects
the right one per region (`tests/test_eneco.py`). Illustrative values from
`test_fix_extracts_taxes` (`tests/test_eneco.py`):
...
(`eneco.py`). The connection fee is a mandatory all-in
...
(`tests/test_eneco.py`) removes the label and asserts the raise.

### Injection

`_extract_injection` (`eneco.py`) is the most delicate block. Layout on
every contract:
...
1. Anchor on the stable `AFNAME EN INJECTIE` prefix (`eneco.py`), then cut the
   section at the next ALL-CAPS heading (`ENERGIEDELEN`, `BELASTINGEN`, ...) so
   unrelated blocks do not pollute the matches (`eneco.py`).
2. Read the monthly indicative (`Maandprijs`), falling back to the yearly estimate
   (`Geschatte jaarprijs`) when no `Maandprijs` prints (`eneco.py`). Both
...
- **Power Fix and Power Flex are MONTH-indexed**: the extractor surfaces the
  ...
  `month_indexed=True` (`eneco.py`). The flag is what makes surfacing
...
- **Power Dynamic is hourly `factor * spot + base`**: it indexes on Belpex-H, so it
  ...
  `test_dynamic_extracts_injection_rates` (`tests/test_eneco.py`): formula
...
- **`AFNAME EN INJECTIE / VALORISATIE` rename (issue #35)**: the July 2026 cards
  ...
  keys off the stable `AFNAME EN INJECTIE` prefix (`eneco.py`), guarded by
  `test_injection_survives_valorisatie_suffix_drop` (`tests/test_eneco.py`).
- **`>` bullet before every row label until June 2025**: the cards printed
  ...
  take the optional `_BULLET` (`eneco.py`), guarded by
  `test_flex_reads_the_bullet_prefixed_archive_card`
  (`tests/test_eneco.py`). 26 of the 27 Flex issues come back: the
  ...
  `_extract_variable` now reads the consumption side alone (`eneco.py`),
  ...
  `parse_snapshot` raises (`tests/test_eneco.py`).
- **An injection row we cannot read is fatal**: a numeric row ending in
  ...
  than storing a snapshot with an empty leg (`eneco.py`, guarded at
  `tests/test_eneco.py`). The bullet is why: the archive check passes a
...
- **`_NUM` four-digit truncation**: the previous integer-part cap of three digits
  ...
  and ungrouped four-digit values (`eneco.py`, guarded at
  `tests/test_eneco.py`).
- **Rigid newline skip broke Power Flex**: the variable yearly-fee parser used to
  ...
  `Geschatte jaarprijs` tokens (`eneco.py`, guarded at
  `tests/test_eneco.py`).
- **Energiefonds is billed in Flanders only**: the table is headed "Bijdrage
  ...
  (`fees.py`). `region` is therefore forwarded to `_extract_taxes`
  (`eneco.py`), which zeroes the field outside Flanders (`eneco.py`);
  both callers pass it, the live card (`eneco.py`) and the archive walk
  (`eneco.py`). Every issue since January 2025 prints 0,00 in the domiciled
  ...
  `test_energy_fund_is_flanders_only` (`tests/test_eneco.py`) on the live
  path and `test_fetch_for_month_gates_the_energy_fund_by_region`
  (`tests/test_eneco.py`) on the archive path, both of which patch a
...
| --- | --- | --- |
| every snapshot fails to fetch | `_resolve_url` / `_fetch_listing` (listing HTML or URL pattern changed) | `eneco.py` |
...
| four-digit fee truncated | `_NUM` token | `eneco.py` |
| historical month mis-billed | `fetch_for_month` volume walk or `archive_validity_check` | `eneco.py` |
