...
`ecopower.py:618-646`; `_extract_taxes` `ecopower.py:601-639`). Residential injection is VAT-exempt,
...
| --- | --- | --- |
| Groene burgerstroom (gbs) | `ecopower.be/groene-stroom/prijs-nieuw` (`_PRICE_PAGE`, `ecopower.py:111`) | `<YYYYMM>_gbs_tariefkaart.pdf` |
| Dynamische burgerstroom (dbs) | `ecopower.be/groene-stroom/dynamische-burgerstroom` (`_DBS_PAGE`, `ecopower.py:147`) | `<YYYYMM|YYYYMMDD>[a-z]?_dbs_tariefkaart*.pdf` |
...
Constants: `_CONTRACT_ID` / `_CONTRACT_LABEL` (`ecopower.py:142-143`), `_DBS_CONTRACT_ID` /
`_DBS_CONTRACT_LABEL` (`ecopower.py:145-146`). The `EXTRACTOR` registration is at
`ecopower.py:882-902`.
...
- **gbs** (`_resolve_latest_pdf`, `ecopower.py:851`): GET the price page HTML, run `_CARD_RE`
...
- **dbs** (`_resolve_latest_dbs_pdf`, `ecopower.py:880`): GET the dynamic product page, run
  `_DBS_CARD_RE` (`ecopower.py:156`), sort and take the highest. The dynamic formula is
...
`probe()` (`ecopower.py:240-255`) HEADs the relevant price page and returns its `Last-Modified`
header via `head_freshness_key` (`_pdf.py:479`). The gbs contract probes `_PRICE_PAGE`, the dbs
...
`fetch_for_month` (`ecopower.py:190-239`) supports the time-correct yearly-cost flow. Ecopower's
...
- **gbs** (`ecopower.py:202`): scrape `_PRICE_PAGE`, find the `_CARD_RE` matches whose month
  ...
  `parse_snapshot`. Then `archive_validity_check` (`_pdf.py:1139`) cross-checks that the parsed card
  actually covers the requested month, using Dutch month names (`_NL_MONTHS`, `ecopower.py:108-108`)
...
- **dbs** (`_fetch_dbs_for_month`, `ecopower.py:900`): dynamic cards do not rotate monthly, so
...
`discover()` (`ecopower.py:258-302`) is a CI / live-check helper, not part of runtime pricing. It
...
- `parse_snapshot(text, source_url, publication_label)` (`ecopower.py:303-317`): gbs card.
- `parse_dbs_snapshot(text, source_url, publication_label)` (`ecopower.py:320-339`): dbs card. The
  tax block layout is identical, so it reuses `_extract_taxes`; only energy, DSO and injection
  parsers differ.

Both operate on layout-preserving pdfplumber text (`extract_pdf_text_layout`, `_pdf.py:370`;
fetched via `fetch_pdf_text_layout`, `_pdf.py:465`). pdfplumber is required because the cards use
...
| --- | --- | --- |
| `energy` | `_extract_energy` (`ecopower.py:387`) -> `VariableRates.current` | `_extract_dbs_energy` (`ecopower.py:425`) -> `DynamicRates` |
| `dsos` | `_extract_dsos` (`ecopower.py:471`) | `_extract_dbs_dsos` (`ecopower.py:554`) |
| `taxes` | `_extract_taxes` (`ecopower.py:618`) | same helper reused |
| `injection` | `_extract_injection` (`ecopower.py:753`) | `_extract_dbs_injection` (`ecopower.py:830`) |
| `valid_until` | `parse_valid_until` (`_pdf.py:1178`) | same |
| `publication_label` | passed in (`YYYY-MM`) | passed in |

### Energy parsing

**gbs** (`_extract_energy`, `ecopower.py:387-403`): the card prints a formula breakdown
...
- `_ENERGY_RE` (`ecopower.py:355`): same-line `Groene burgerstroom ... <rate> euro/kWh`.
- `_ENERGY_VARIABEL_RE` (`ecopower.py:379-384`): the July 2026 layout, which broke the 50/50 split
...
- `_ENERGY_SPLIT_RE` (`ecopower.py:364-367`): fallback for mid-2026 cards that moved the resolved
...
**dbs** (`_extract_dbs_energy`, `ecopower.py:425-451`): the card prints
`Dynamische burgerstroom elk kwartier 0,00102 × EPEX DA +0,004 euro/kWh` (illustrative,
`test_dbs_card_energy_is_dynamic_formula_htva`, `test_ecopower.py:414-422`). `_DBS_ENERGY_RE` (`ecopower.py:415-420`) captures factor, sign, base.
...
- The multiplication glyph is `×` (U+00D7); the regex accepts `[×xX*]` in case a re-render swaps
  it.
- The additive base sign is parsed through `SIGN_CHARS` / `parse_sign` (`_pdf.py:891`,
...
The monthly subscription `Abonnementskost <n> euro/maand` (`_ABONNEMENT_RE`, `ecopower.py:422`)
maps to `yearly_fixed_fee` via `_extract_dbs_abonnement` (`ecopower.py:454-464`). Because
...
Ecopower maps all eight Fluvius sub-areas via `_DSO_LABELS` (`ecopower.py:139-139`). Note the two
...
**gbs** (`_extract_dsos`, `ecopower.py:471-528`): the card lists two networks per sub-area, a
...
Digital row layout (comment `ecopower.py:488-498`):
...
**dbs** (`_extract_dbs_dsos`, `ecopower.py:554-583`): the dynamic card has only a digital block (a
...
section header matches but no DSO row does (`ecopower.py:524-527`, `534-537`). Returning `{}`
...
`_extract_taxes` (`ecopower.py:601-639`), shared by both cards. Regexes at `ecopower.py:618-646`:
...
quota, docstring `ecopower.py:609-614`). **Both are mandatory**: a missing GSC or WKK raises,
because treating them as optional would let a relabel silently drop a per-kWh charge
(the `ExtractorError` raise at `ecopower.py:644-645`; `test_missing_gsc_or_wkk_surcharge_is_fatal`, `test_ecopower.py:75-81`).

### Injection parsing

**gbs** injection is a **monthly-indicative-only** shape (taxonomy: `current` set, no
`factor`/`base`). `_extract_injection` (`ecopower.py:728-761`). The terugleververgoeding is a
...
1. `_INJECTION_FIXED_RE` (`ecopower.py:703-706`): an authoritative `OPGELET t.e.m. <date> is de
   terugleververgoeding <value> euro/kWh en 100% vast` note. When present **and still in effect**
   (`_fixed_note_in_effect`, `ecopower.py:697-715`), this fixed value wins.
2. `_INJECTION_RE` (`ecopower.py:655-665`): the label line, matching both the pre-May-2026 label
   `Terugleververgoeding (digitale meter)` and the post-May-2026 label
   `Injectie Groene Burgerstroom (terugleververgoeding)`.
3. `_INJECTION_VARIABEL_RE` (`ecopower.py:682-687`): the July 2026 `VAST` / `VARIABEL` layout,
...
4. `_INJECTION_SPLIT_RE` (`ecopower.py:677-681`): split-layout fallback where the resolved value
   is on the line below the label.

All four use `SIGN_CHARS` for the leading sign, and non-ASCII minus glyphs are normalised to `-`
before `to_float` (`ecopower.py:768-776`). Returns `None` when nothing matches (injection is
nullable).

**dbs** injection is an **hourly `factor*spot+base`** shape. `_extract_dbs_injection`
(`ecopower.py:756-771`) parses `Terugleververgoeding elk kwartier 0,00098 × EPEX DA - 0,015
euro/kWh` via `_DBS_INJECTION_RE` (`ecopower.py:822-827`). Same MWh->kWh factor scaling (`× 1000`)
...
(`DsoOverlay.prosumer_eur_per_kva_year` docstring, `base.py:681-681`), so the extractor never sets
...
- **`inschatting` next-month preview.** Around month-end Ecopower publishes an estimation card
  ...
  definitive form (`ecopower.py:110-120`, `191-197`, `733-739`). Test:
...
- **Issue #31, May 2026 injection relabel.** The injection row was renamed from
  ...
  both labels (`ecopower.py:662-672`; `test_may_card_injection_label_is_matched`,
...
- **Split-layout cards (mid-2026).** The resolved energy and injection values moved onto the line
  **below** their label. `_ENERGY_SPLIT_RE` and `_INJECTION_SPLIT_RE` are the fallbacks
  (`ecopower.py:364-367`, `633-637`; `test_split_layout_card_parses_energy_and_injection`,
...
- **`100% vast` injection note vs the 50/50 variable formula.** On split-layout cards the label
  ...
  (illustrative `0,020`) they actually receive (`ecopower.py:686-689`, `668-676`;
...
- **Stale carried-over note.** A later month's card can still carry the old note while already
  printing the variable formula. `_fixed_note_in_effect` (`ecopower.py:697-715`) compares the
...
- **`Maximumtarief` optional 7th column.** Slides into a DSO row where Fluvius publishes a maximum
  ...
  (`ecopower.py:488-498`; `test_april_card_extracts_imewo_with_optional_max_column`,
...
- **Digital-meter-only model.** Only the DIGITALE METER block is parsed; the ANALOGE METER block
  is ignored (same energy rate, different network cost, `ecopower.py:471-482`).
...
- **Fail-loud on empty DSO / missing GSC/WKK.** Both are deliberate guards against a silent
  backfill skip / silently-dropped mandatory charge (`ecopower.py:524-527`, `579-580`).
- **`discover` logs unreachable pages.** A partial page failure is logged, not swallowed, so a
  dropped family is not masked by a still-non-empty result (`ecopower.py:240-257`).
...
1. **Energy rate moved or relabelled** -> `_ENERGY_RE` / `_ENERGY_VARIABEL_RE` /
   `_ENERGY_SPLIT_RE` (`ecopower.py:355-384`)
   for gbs, `_DBS_ENERGY_RE` (`ecopower.py:415-420`) for dbs. A `raise ExtractorError("could not
...
5. **Card filename family or price-page structure changed** -> `_CARD_RE`, `_DBS_CARD_RE`
   (`ecopower.py:113-159`), `_resolve_latest_pdf` / `_resolve_latest_dbs_pdf`
   (`ecopower.py:258-300`), and `discover` (`ecopower.py:258-302`). Symptom: `no Ecopower
