...
header via `head_freshness_key` (`_pdf.py:350`). The gbs contract probes `_PRICE_PAGE`, the dbs
...
- **gbs** (`ecopower.py:185-210`): scrape `_PRICE_PAGE`, find the `_CARD_RE` match whose YYYYMM
  ...
  `parse_snapshot`. Then `archive_validity_check` (`_pdf.py:819`) cross-checks that the parsed card
...
Both operate on layout-preserving pdfplumber text (`extract_pdf_text_layout`, `_pdf.py:247`;
fetched via `fetch_pdf_text_layout`, `_pdf.py:337`). pdfplumber is required because the cards use
...
| `valid_until` | `parse_valid_until` (`_pdf.py:858`) | same |
...
- The multiplication glyph is `×` (U+00D7); the regex accepts `[×xX*]` in case a re-render swaps
  it.
- The additive base sign is parsed through `SIGN_CHARS` / `parse_sign` (`_pdf.py:596`,
...
Ecopower maps all eight Fluvius sub-areas via `_DSO_LABELS` (`ecopower.py:118-119`). Note the two
...
- **HTVA cards, `vat_rate=0.06`.** Ecopower is the cooperative outlier. Do not blindly copy a
  TVAC-publishing supplier's `vat_rate=0.0` convention here (`ecopower.py:56-60`, `581-591`).
- **`inschatting` next-month preview.** Around month-end Ecopower publishes an estimation card
  ...
  definitive form (`ecopower.py:109-116`, `191-197`, `733-739`). Test:
