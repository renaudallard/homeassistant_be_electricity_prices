...
header via `head_freshness_key` (`_pdf.py:424`). The gbs contract probes `_PRICE_PAGE`, the dbs
...
- **gbs** (`ecopower.py:202`): scrape `_PRICE_PAGE`, find the `_CARD_RE` matches whose month
  ...
  `parse_snapshot`. Then `archive_validity_check` (`_pdf.py:965`) cross-checks that the parsed card
...
Both operate on layout-preserving pdfplumber text (`extract_pdf_text_layout`, `_pdf.py:321`;
fetched via `fetch_pdf_text_layout`, `_pdf.py:411`). pdfplumber is required because the cards use
...
| `valid_until` | `parse_valid_until` (`_pdf.py:1004`) | same |
...
- The multiplication glyph is `×` (U+00D7); the regex accepts `[×xX*]` in case a re-render swaps
  it.
- The additive base sign is parsed through `SIGN_CHARS` / `parse_sign` (`_pdf.py:717`,
...
(`DsoOverlay.prosumer_eur_per_kva_year` docstring, `base.py:575-575`), so the extractor never sets
