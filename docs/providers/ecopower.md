...
frozenset({REGION_FLANDERS})`, `ecopower.py`). `fetch()` rejects any other region up front
(`ecopower.py`), and `EXTRACTOR.regions()` returns just Flanders because both contracts
declare `regions=_ECOPOWER_REGIONS` (`ecopower.py`).

The two products (module docstring, `ecopower.py`):
...
| --- | --- | --- |
| Groene burgerstroom (gbs) | `ecopower.be/groene-stroom/prijs-nieuw` (`_PRICE_PAGE`, `ecopower.py`) | `<YYYYMM>_gbs_tariefkaart.pdf` |
| Dynamische burgerstroom (dbs) | `ecopower.be/groene-stroom/dynamische-burgerstroom` (`_DBS_PAGE`, `ecopower.py`) | `<YYYYMM|YYYYMMDD>[a-z]?_dbs_tariefkaart*.pdf` |
...
Constants: `_CONTRACT_ID` / `_CONTRACT_LABEL` (`ecopower.py`), `_DBS_CONTRACT_ID` /
`_DBS_CONTRACT_LABEL` (`ecopower.py`). The `EXTRACTOR` registration is at
`ecopower.py`.
...
```
fetch(session, contract_id, region)              ecopower.py
...
(`ecopower.py`), which pads the month-only form to `YYYYMM00` so a dated reissue outranks a
...
captured so ordering stays numeric (comment `ecopower.py`).
...
`fetch_for_month` (`ecopower.py`) supports the time-correct yearly-cost flow. Ecopower's
...
- **dbs** (`_fetch_dbs_for_month`, `ecopower.py`): dynamic cards do not rotate monthly, so
...
`discover()` (`ecopower.py`) is a CI / live-check helper, not part of runtime pricing. It
scrapes both pages, emits `_CONTRACT_ID` when a gbs card is seen and `_DBS_DISCOVER_ID`
(`ecopower_dbs`, `ecopower.py`) when a dbs card is seen, and surfaces any **new**
...
(`ecopower.py`). A page that fails to fetch is logged, not swallowed, so a partial failure
...
- `parse_snapshot(text, source_url, publication_label)` (`ecopower.py`): gbs card.
- `parse_dbs_snapshot(text, source_url, publication_label)` (`ecopower.py`): dbs card. The
  tax block layout is identical, so it reuses `_extract_taxes`; only energy, DSO and injection
  parsers differ.

Both operate on layout-preserving pdfplumber text (`extract_pdf_text_layout`, `_pdf.py`;
fetched via `fetch_pdf_text_layout`, `_pdf.py`). pdfplumber is required because the cards use
rotated multi-column DSO and tax tables that a plain text extractor drops. Fixture tests read the
same layout text through `fixture_text(name, layout=True)` (`test_ecopower.py`).
...
(`DsoOverlay.prosumer_eur_per_kva_year` docstring, `base.py`), so the extractor never sets
...
| `ecopower_burgerstroom_jul.pdf` | July 2026 gbs, `VAST` / `VARIABEL` layout, injection gone 50% variable | VARIABEL-layout energy + injection, and the WKK-capture regression guard (`test_ecopower.py`) |
...
`fetch_for_month` tests also use inline HTML listings `_LISTING_HTML` (`test_ecopower.py`)
and `_DBS_LISTING_HTML` (`test_ecopower.py`) with a stub `_Session` / `_Resp`, patching
