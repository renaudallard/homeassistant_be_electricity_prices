...
frozenset({REGION_FLANDERS})`, `ecopower.py:830`). `fetch()` rejects any other region up front
(`ecopower.py:182`), and `EXTRACTOR.regions()` returns just Flanders because both contracts
declare `regions=_ECOPOWER_REGIONS` (`ecopower.py:840`, `ecopower.py:846`).
...
`ecopower.py:56-60`; `_extract_taxes` `ecopower.py:569-607`). Residential injection is VAT-exempt,
...
| --- | --- | --- |
| Groene burgerstroom (gbs) | `ecopower.be/groene-stroom/prijs-nieuw` (`_PRICE_PAGE`, `ecopower.py:126`) | `<YYYYMM>_gbs_tariefkaart.pdf` |
| Dynamische burgerstroom (dbs) | `ecopower.be/groene-stroom/dynamische-burgerstroom` (`_DBS_PAGE`, `ecopower.py:154`) | `<YYYYMM>[a-z]?_dbs_tariefkaart*.pdf` |
...
Constants: `_CONTRACT_ID` / `_CONTRACT_LABEL` (`ecopower.py:149-150`), `_DBS_CONTRACT_ID` /
`_DBS_CONTRACT_LABEL` (`ecopower.py:152-153`). The `EXTRACTOR` registration is at
`ecopower.py:832-852`.
...
- **Groene burgerstroom** is a `variable` contract even though it is 50% fixed. The card resolves
  ...
  `ecopower.py:353-366`).
- **Dynamische burgerstroom** sets `quarter_hourly=True` (`ecopower.py:412`). Ecopower's card
...
```
fetch(session, contract_id, region)              ecopower.py:177
...
- **gbs** (`_resolve_latest_pdf`, `ecopower.py:740-765`): GET the price page HTML, run `_CARD_RE`
  (`ecopower.py:132-135`) over it to collect every `(<YYYYMM>, url)` pair, **drop any URL
...
- **dbs** (`_resolve_latest_dbs_pdf`, `ecopower.py:768-784`): GET the dynamic product page, run
  `_DBS_CARD_RE` (`ecopower.py:161-164`), sort and take the highest YYYYMM. The dynamic formula is
...
captured so month ordering stays numeric (comment `ecopower.py:156-160`).

### Probe (freshness key)

`probe()` (`ecopower.py:241-256`) HEADs the relevant price page and returns its `Last-Modified`
header via `head_freshness_key` (`_pdf.py:329`). The gbs contract probes `_PRICE_PAGE`, the dbs
...
`fetch_for_month` (`ecopower.py:195-238`) supports the time-correct yearly-cost flow. Ecopower's
...
- **gbs** (`ecopower.py:213-238`): scrape `_PRICE_PAGE`, find the `_CARD_RE` match whose YYYYMM
  equals the requested month and whose URL is not an `inschatting` preview, download and
  `parse_snapshot`. Then `archive_validity_check` (`_pdf.py:638`) cross-checks that the parsed card
  actually covers the requested month, using Dutch month names (`_NL_MONTHS`, `ecopower.py:110-123`)
...
- **dbs** (`_fetch_dbs_for_month`, `ecopower.py:787-816`): dynamic cards do not rotate monthly, so
...
`discover()` (`ecopower.py:259-295`) is a CI / live-check helper, not part of runtime pricing. It
scrapes both pages, emits `_CONTRACT_ID` when a gbs card is seen and `_DBS_DISCOVER_ID`
(`ecopower_dbs`, `ecopower.py:169`) when a dbs card is seen, and surfaces any **new**
...
(`ecopower.py:174`). A page that fails to fetch is logged, not swallowed, so a partial failure
...
- `parse_snapshot(text, source_url, publication_label)` (`ecopower.py:298-312`): gbs card.
- `parse_dbs_snapshot(text, source_url, publication_label)` (`ecopower.py:315-335`): dbs card. The
  tax block layout is identical, so it reuses `_extract_taxes`; only energy, DSO and injection
  parsers differ.

Both operate on layout-preserving pdfplumber text (`extract_pdf_text_layout`, `_pdf.py:196`;
fetched via `fetch_pdf_text_layout`, `_pdf.py:316`). pdfplumber is required because the cards use
rotated multi-column DSO and tax tables that a plain text extractor drops. Fixture tests read the
same layout text through `fixture_text(name, layout=True)` (`test_ecopower.py:52-53`).

### Fields extracted per card

| Snapshot field | gbs source | dbs source |
| --- | --- | --- |
| `energy` | `_extract_energy` (`ecopower.py:353`) -> `VariableRates.current` | `_extract_dbs_energy` (`ecopower.py:387`) -> `DynamicRates` |
| `dsos` | `_extract_dsos` (`ecopower.py:433`) | `_extract_dbs_dsos` (`ecopower.py:512`) |
| `taxes` | `_extract_taxes` (`ecopower.py:569`) | same helper reused |
| `injection` | `_extract_injection` (`ecopower.py:673`) | `_extract_dbs_injection` (`ecopower.py:719`) |
| `valid_until` | `parse_valid_until` (`_pdf.py:677`) | same |
| `publication_label` | passed in (`YYYY-MM`) | passed in |

### Energy parsing

**gbs** (`_extract_energy`, `ecopower.py:353-366`): the card prints a formula breakdown
`(50% vast aan 0,17 euro + 50% variabel aan 0,08472117 euro)` followed by the resolved figure
(illustrative `0,1274 euro/kWh` in the April fixture, `test_ecopower.py:89-94`). Two regexes:

- `_ENERGY_RE` (`ecopower.py:341`): same-line `Groene burgerstroom ... <rate> euro/kWh`.
- `_ENERGY_SPLIT_RE` (`ecopower.py:347-350`): fallback for mid-2026 cards that moved the resolved
...
**dbs** (`_extract_dbs_energy`, `ecopower.py:387-413`): the card prints
`Dynamische burgerstroom elk kwartier 0,00102 × EPEX DA +0,004 euro/kWh` (illustrative,
`test_ecopower.py:248-252`). `_DBS_ENERGY_RE` (`ecopower.py:377-382`) captures factor, sign, base.
...
- The multiplication glyph is `×` (U+00D7); the regex accepts `[×xX*]` in case a re-render swaps
  it.
- The additive base sign is parsed through `SIGN_CHARS` / `parse_sign` (`_pdf.py:473`,
  `_pdf.py:477`) so a punctuation drift (hyphen vs en-dash vs U+2212) never flips the sign
  silently.
- Values stay HTVA; `vat_rate=0.06` scales them later. They are NOT pre-scaled.

The monthly subscription `Abonnementskost <n> euro/maand` (`_ABONNEMENT_RE`, `ecopower.py:384`)
maps to `yearly_fixed_fee` via `_extract_dbs_abonnement` (`ecopower.py:416-427`). Because
...
`test_ecopower.py:260-266`).

### DSO parsing

Ecopower maps all eight Fluvius sub-areas via `_DSO_LABELS` (`ecopower.py:137-146`). Note the two
...
(`test_ecopower.py:97-109`, `281-294`).

**gbs** (`_extract_dsos`, `ecopower.py:433-493`): the card lists two networks per sub-area, a
DIGITAL METER block and an ANALOG METER block. The integration only models the **digital** path
(`_slice_between(text, "DIGITALE METER", "ANALOGE METER")`, `ecopower.py:445`), which is where the
...
Digital row layout (comment `ecopower.py:450-456`):

```
<label> | databeheer EUR/yr | capacity EUR/kW/yr | - | enkelvoudig EUR/kWh | uitsluitend_nacht EUR/kWh | [maximumtarief] | -
```

Row regex `ecopower.py:457-462`. The optional 7th `Maximumtarief` column slides in between the
exclusive-night rate and the trailing dash on rows where Fluvius publishes a maximum (the Imewo
April 2026 card has one, `test_ecopower.py:130-140`); the `(?:\s+[\d,]+)?` group skips it without
mis-aligning the distribution rate.

Captured columns map to `DsoOverlay` (`ecopower.py:481-487`):
...
- `transport = 0.0` (Elia transport is rolled into distribution on Ecopower's card; there is no
  ...
  `test_ecopower.py:124-127`)

**dbs** (`_extract_dbs_dsos`, `ecopower.py:512-554`): the dynamic card has only a digital block (a
dynamic contract requires a smart meter), sliced `_slice_between(text, "Nettarieven",
"Heffingen")` (`ecopower.py:525`). The row layout differs (no separating dashes):

```
databeheer | capacity | afname enkelvoudig | afname uitsluitend-nacht | [maximumtarief] | injectietarief
```

Row regex reads the first four numeric columns (`ecopower.py:533-537`) and ignores the optional
maximumtarief and the trailing injection network tariff (`DsoOverlay` does not model them). Column
mapping (`ecopower.py:540-549`): `distribution_single` = group 3, `distribution_exclusive_night` =
...
`<numbers>` / `Vlaanderen`). `_DBS_WRAPPED_LABEL_RE` (`ecopower.py:509`) plus the `.sub`
(`ecopower.py:528-530`) stitches the two label fragments back around the rate row so the per-DSO
row regex sees one line. Tests assert the stitched row keeps its real rates
(`test_ecopower.py:312-319`).

Both DSO parsers **fail loud** with `ExtractorError("Ecopower: no DSO rows parsed ...")` if the
section header matches but no DSO row does (`ecopower.py:488-492`, `550-553`). Returning `{}`
...
(`test_ecopower.py:64-69`).

### Tax parsing

`_extract_taxes` (`ecopower.py:569-607`), shared by both cards. Regexes at `ecopower.py:560-566`:
...
Wallonia and Brussels renewables stay 0 (Ecopower is Flanders-only, `test_ecopower.py:154-156`).
...
quota, docstring `ecopower.py:578-583`). **Both are mandatory**: a missing GSC or WKK raises,
because treating them as optional would let a relabel silently drop a per-kWh charge
(`ecopower.py:595-596`; `test_missing_gsc_or_wkk_surcharge_is_fatal`, `test_ecopower.py:72-78`).

### Injection parsing

**gbs** injection is a **monthly-indicative-only** shape (taxonomy: `current` set, no
`factor`/`base`). `_extract_injection` (`ecopower.py:673-706`). The terugleververgoeding is a
...
matching every other supplier's sign (`test_ecopower.py:159-166`).

Three matching strategies, in priority order:

1. `_INJECTION_FIXED_RE` (`ecopower.py:641-644`): an authoritative `OPGELET t.e.m. <date> is de
   terugleververgoeding <value> euro/kWh en 100% vast` note. When present **and still in effect**
   (`_fixed_note_in_effect`, `ecopower.py:652-670`), this fixed value wins.
2. `_INJECTION_RE` (`ecopower.py:613-623`): the label line, matching both the pre-May-2026 label
   `Terugleververgoeding (digitale meter)` and the post-May-2026 label
   `Injectie Groene Burgerstroom (terugleververgoeding)`.
3. `_INJECTION_SPLIT_RE` (`ecopower.py:628-632`): split-layout fallback where the resolved value
   is on the line below the label.

All three use `SIGN_CHARS` for the leading sign, and non-ASCII minus glyphs are normalised to `-`
before `to_float` (`ecopower.py:696-701`). Returns `None` when nothing matches (injection is
nullable).

**dbs** injection is an **hourly `factor*spot+base`** shape. `_extract_dbs_injection`
(`ecopower.py:719-734`) parses `Terugleververgoeding elk kwartier 0,00098 × EPEX DA - 0,015
euro/kWh` via `_DBS_INJECTION_RE` (`ecopower.py:711-716`). Same MWh->kWh factor scaling (`× 1000`)
...
(`DsoOverlay.prosumer_eur_per_kva_year` docstring, `base.py:270-273`), so the extractor never sets
...
(`17,85 × 1,06 = 18,92`, `ecopower.py:470-472`; `test_ecopower.py:112-123`). If you touch any of
...
- **`inschatting` next-month preview.** Around month-end Ecopower publishes an estimation card
  ...
  definitive form (`ecopower.py:128-135`, `219-223`, `753-759`). Test:
  `test_fetch_for_month_skips_inschatting_preview` (`test_ecopower.py:396-408`).
- **Issue #31, May 2026 injection relabel.** The injection row was renamed from
  ...
  both labels (`ecopower.py:613-623`; `test_may_card_injection_label_is_matched`,
  `test_ecopower.py:219-227`).
- **Split-layout cards (mid-2026).** The resolved energy and injection values moved onto the line
  **below** their label. `_ENERGY_SPLIT_RE` and `_INJECTION_SPLIT_RE` are the fallbacks
  (`ecopower.py:347-350`, `628-632`; `test_split_layout_card_parses_energy_and_injection`,
  `test_ecopower.py:177-200`).
- **`100% vast` injection note vs the 50/50 variable formula.** On split-layout cards the label
  ...
  (illustrative `0,020`) they actually receive (`ecopower.py:641-644`, `684-692`;
...
- **Stale carried-over note.** A later month's card can still carry the old note while already
  printing the variable formula. `_fixed_note_in_effect` (`ecopower.py:652-670`) compares the
  ...
  `test_stale_fixed_injection_note_is_ignored_on_a_later_card` (`test_ecopower.py:203-216`).
...
- **`Maximumtarief` optional 7th column.** Slides into a DSO row where Fluvius publishes a maximum
  ...
  (`ecopower.py:450-462`; `test_april_card_extracts_imewo_with_optional_max_column`,
  `test_ecopower.py:130-140`).
...
- **Fail-loud on empty DSO / missing GSC/WKK.** Both are deliberate guards against a silent
  backfill skip / silently-dropped mandatory charge (`ecopower.py:488-492`, `595-596`).
- **`discover` logs unreachable pages.** A partial page failure is logged, not swallowed, so a
  dropped family is not masked by a still-non-empty result (`ecopower.py:276-295`).
...
`fetch_for_month` tests also use inline HTML listings `_LISTING_HTML` (`test_ecopower.py:342-348`)
and `_DBS_LISTING_HTML` (`test_ecopower.py:438-443`) with a stub `_Session` / `_Resp`, patching
...
1. **Energy rate moved or relabelled** -> `_ENERGY_RE` / `_ENERGY_SPLIT_RE` (`ecopower.py:341-350`)
   for gbs, `_DBS_ENERGY_RE` (`ecopower.py:377-382`) for dbs. A `raise ExtractorError("could not
   parse ... rate/formula")` is the symptom.
2. **Injection label / layout / note changed** -> `_INJECTION_RE`, `_INJECTION_SPLIT_RE`,
   `_INJECTION_FIXED_RE`, `_fixed_note_in_effect` (`ecopower.py:613-670`). Injection is nullable,
   so a miss shows as an unavailable injection sensor, not a hard error (watch for silent loss).
3. **DSO table column shuffle or new sub-area label** -> `_DSO_LABELS` (`ecopower.py:137-146`),
   the gbs row regex (`ecopower.py:457-462`), the dbs row regex + `_DBS_WRAPPED_LABEL_RE`
   (`ecopower.py:509`, `533-537`). Symptom: `Ecopower: no DSO rows parsed` or a missing sub-area.
4. **Tax row relabelled** -> `_extract_taxes` regexes (`ecopower.py:560-566`). Symptom: `could not
...
5. **Card filename family or price-page structure changed** -> `_CARD_RE`, `_DBS_CARD_RE`
   (`ecopower.py:132-164`), `_resolve_latest_pdf` / `_resolve_latest_dbs_pdf`
   (`ecopower.py:740-784`), and `discover` (`ecopower.py:259-295`). Symptom: `no Ecopower
   tariefkaart link found`.
6. **VAT rate change (no longer 6%)** -> the `0.06` in `_extract_taxes` (`ecopower.py:606`) and
   every `* 1.06` on flat fees (`ecopower.py:427`, `472-473`, `547-548`). These must move
   together.
