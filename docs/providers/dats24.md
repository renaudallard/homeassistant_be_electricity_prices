...
- **Negative-safe sign parsing.** The indicative regex captures an optional leading
  ...
  and applies `parse_sign` (`_pdf.py:810`). When `BE_spotSPP` is low the monthly
...
`parse_valid_until` (`_pdf.py:1097`), which catches the explicit `GELDIG VAN 1 APRIL
...
| Wrong publication label / `valid_until` | `_extract_publication` (`dats24.py:556-558`), `parse_valid_until` (`_pdf.py:1097`) | `TARIEFKAART <month> <year>` or the `GELDIG VAN` header changed |
