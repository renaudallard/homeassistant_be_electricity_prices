...
| --- | --- | --- |
| Extractor id | `luminus` | `luminus.py:705` |
| Label | `Luminus` | `luminus.py:706` |
| Regions served | Flanders, Wallonia | `luminus.py:702`, `_LUMINUS_REGIONS` |
| Publication | one fresh PDF per (product, region) via REST endpoint | `luminus.py:96`, `luminus.py:134` |
| Probe | none (`EXTRACTOR.probe` unset -> `None`) | `luminus.py:704-717` |
| Archive (`fetch_for_month`) | none (API-only, overwrite-in-place) | `luminus.py:704-717` |
...
(`_REGION_TO_TAB`, `luminus.py:98-101`). The response filename encodes the
...
Declared in `_CONTRACTS` (`luminus.py:112-129`); `_CONTRACTS_BY_ID` indexes them
(`luminus.py:131`); `EXTRACTOR.contracts` is built from them (`luminus.py:707-715`).

Retired / omitted product: **Luminus Sociaal/Social** (the regulated CREG
tariff) is intentionally not declared (`luminus.py:125-128`), same reasoning as
...
(`luminus.py:341-345`), which is what routes the coordinator to aggregate
ENTSO-E's 15-minute day-ahead curve to hourly. See the `DynamicRates` docstring
(`base.py:127-148`): Luminus is listed among the hourly-billing dynamic
suppliers (Frank default, Mega, TotalEnergies, Eneco).

## Fetch strategy

`fetch(session, contract_id, region)` (`luminus.py:175-190`):
...
4. `fetch_pdf_text(session, url)` downloads and pypdf-extracts the PDF text
   (`_pdf.py:158-166`); the payload is magic-byte-validated as a real PDF, so a
   CDN 404-disguised-as-HTML fails loud (`_pdf.py:147-155`).
...
(`base.py:375`). Per the `SnapshotProbe` contract (`base.py:347-351`), the
...
defaults to `None` (`base.py:381`). The endpoint is API-only and overwrite-in-
...
Luminus for (`base.py:359-361`). The coordinator therefore bills past months at
the current snapshot as a proxy.

### `discover()`

`discover(session)` (`luminus.py:155-169`) is a CI / live-check helper (not part
of the coordinator's runtime path). It GETs the sitemap
(`https://www.luminus.be/sitemap.xml`, `luminus.py:142`) and scrapes product
slugs from the `/fr|nl/particuliers/tarifs-energie|onze-tarieven/<slug>/`
structure (`_PRODUCT_PAGE_RE`, `luminus.py:146-148`), excluding the regulated
social-tariff index pages (`_EXCLUDED_SLUGS`, `luminus.py:152`). A failed sitemap
fetch returns an empty set rather than raising.

## Parsing

`parse_snapshot` (`luminus.py:193-234`) assembles the snapshot from a set of
...
branches hard on `region` (`luminus.py:207-214`) and never merges. That region-
...
| Field | Helper | Source |
| --- | --- | --- |
| Energy rates | `_extract_energy` | `luminus.py:300-375` |
| Injection | `_extract_injection` | `luminus.py:390-432` |
| Publication label | `_extract_publication_month` | `luminus.py:378-387` |
| Per-kWh taxes (excise, contribution, connection) | `_extract_per_kwh_taxes` | `luminus.py:474-504` |
| Energy fund (Flanders only) | `_extract_energy_fund` | `luminus.py:507-516` |
| Flanders renewables | `_extract_flanders_renewables` | `luminus.py:519-550` |
| Wallonia renewables | `_extract_wallonia_renewables` | `luminus.py:553-565` |
| Flanders DSO overlay | `_extract_flanders_dsos` | `luminus.py:583-634` |
| Wallonia DSO overlay | `_extract_wallonia_dsos` | `luminus.py:646-699` |
| Yearly fixed fee | `_extract_yearly_fee` | `luminus.py:265-277` |
| Exclusive-night fee | `_extract_excl_night_fee` | `luminus.py:280-297` |
| VAT multiplier | `_vat_multiplier` | `luminus.py:257-262` |
| `valid_until` | `parse_valid_until` (shared) | `_pdf.py:677-780` |

### Numeric token

`_NUM = r"\d+(?:[,.]\d+)?"` (`luminus.py:245`) is anchored on a starting and
...
(`luminus.py:241-245`). Values are parsed with the shared `to_float` (handles
Belgian comma decimals and every Unicode space variant, `_pdf.py:452-463`).

### Units

Printed energy rows are in `c€/kWh`; the extractor divides by 100 to store
EUR/kWh (`luminus.py:313-315`, `353-356`). Prices are 6% VAT inclusive as printed
(`luminus.py:42-44`), so the snapshot's `TaxOverlay.vat_rate` is set to `0.0`
(`luminus.py:229`) meaning "already VAT-incl" per the `TaxOverlay` convention
(`base.py:306-308`). The one exception is the Dynamic formula, printed `hors TVA`
(ex-VAT), handled below.

### Publication label

`_extract_publication_month` (`luminus.py:378-387`) reads the parenthesised
...
`_extract_energy(text, kind)` (`luminus.py:300-375`) always parses the yearly
...
creuses / exclusif-nuit), each divided by 100 (`luminus.py:347-356`). `fixed`
returns `FixedRates(single, peak, offpeak, exclusive_night, ...)`
(`luminus.py:359-367`); `variable` returns `VariableRates(current=mono, peak,
offpeak, exclusive_night, ...)` (`luminus.py:368-375`). Illustrative
...
occurrence and not the second (`luminus.py:307-315`). The second occurrence later
in the PDF is the bi-horaire fallback for non-SMR3 customers
(`luminus.py:303-306`). Returns `TimeOfUseRates(peak, transition, offpeak,
yearly_fixed_fee, weekend_rule="smartflex_seasonal")` (`luminus.py:322-328`).
...
first-year "free Sundays" promo is not modelled (`luminus.py:316-321`).
...
(`luminus.py:247-250`, matched at `luminus.py:331-335`). The sign character is
matched from the shared `SIGN_CHARS` class and resolved with `parse_sign`
(`_pdf.py:472-484`), so a card that flips to a Unicode minus does not silently
break polarity.
...
6% VAT multiplier (`luminus.py:337-345`):
...
The VAT rate is read by `_vat_multiplier` (`luminus.py:257-262`), which wraps the
...
convention as Engie), so `transport` is set to `0.0` there (`luminus.py:595-597`,
`luminus.py:627`).

### Flanders (`_extract_flanders_dsos`, `luminus.py:583-634`)

Eight Fluvius sub-areas mapped by printed label to canonical key
(`_FLANDERS_LABELS`, `luminus.py:571-580`). Watch the two label-to-key surprises:
...
Two column layouts are handled by the same row regex (`luminus.py:613-620`):
...
- **Static (fixed/variable/tou) cards print 8 numbers**: data_mgmt €/an,
  ...
  digital-meter columns (`nums[0..3]`) plus `nums[7]` prosumer (`luminus.py:623`).
- **Dynamic (SMR3) cards print 4 numbers**: data_mgmt, capacity_digital,
  ...
  `None` (`luminus.py:623`), because post-2024 SMR3 connections carry no
...
(`luminus.py:601-609`, applied at `luminus.py:628-630`).
...
### Wallonia (`_extract_wallonia_dsos`, `luminus.py:646-699`)
...
Two column layouts (`luminus.py:659-686`):
...
(`luminus.py:671-674`). They are mapped to `distribution_eco` / `_medium` /
`_pic` accordingly (`luminus.py:687-698`). Illustrative
(`test_comfy_wallonia_fixed_rates_and_dso`): AIEG mono `0.1087`, pleines
`0.1205`, creuses `0.0666`, transport `0.0274`, prosumer `81.03`.

## Tax overlay

`_extract_per_kwh_taxes` (`luminus.py:474-504`) reads the
`3 Taxes et redevances : WAL|FL|BRU` block via `_tax_block_values`
(`luminus.py:435-471`). That helper anchors on the colon after the label because
`Taxes et redevances` also appears in the `Composition du prix` legend without a
colon or region (`luminus.py:459-462`); the block runs until
...
`_NUM` tokens (`luminus.py:471`). The label order and matching value order are
documented in the `_tax_block_values` docstring (`luminus.py:435-458`): BTNR,
...
| `vat_rate` | `0.0` (prices already VAT-incl) | `luminus.py:229` |
...
underbilling (`luminus.py:491-502`). Illustrative
...
(`_extract_energy_fund`, `luminus.py:507-516`). In both fixture regions today BTR
...
(`_extract_flanders_renewables`, `luminus.py:519-550`): the primary regex sums
...
`_extract_injection(text, kind)` (`luminus.py:390-432`) covers two of the three
...
Two anchoring subtleties in the indicative regex (`luminus.py:404-409`):
...
1. **Applicable vs annual estimate.** The card prints both the applicable
   ...
   (`luminus.py:390-398`). `test_injection_uses_applicable_rate_not_annual_estimate`
...
silently crediting nothing (`luminus.py:425-431`). `test_missing_injection_row_fails_loud`
...
`_extract_yearly_fee` (`luminus.py:265-277`) captures the
...
`_extract_excl_night_fee` (`luminus.py:280-297`) reads the third column of the
...
- **SMR3 reduced data-management fee** from the `quart d'heure` footnote on
  dynamic Flanders cards, not the table's monthly column (`luminus.py:601-609`).
...
- **Fail-loud policy**: yearly fee, injection, per-kWh taxes, and both regional
  ...
  mispricing (`luminus.py:276`, `431`, `492-502`, `547-549`, `561-564`).
...
| --- | --- | --- |
| Every field misses / fetch fails | `fetch` + `fetch_pdf_text` (`luminus.py:175-190`, `_pdf.py:158-166`) | URL construction, slug/tabValue, PDF magic-byte validation |
| Energy rates wrong / missing | `_extract_energy` (`luminus.py:300-375`) | four-column vs three-column row, unit /100, TOU lookahead |
| Dynamic factor/base off by ~1.06 or ~10 | dynamic branch (`luminus.py:330-345`) | VAT multiplier + mWh->kWh + c->EUR conversion |
| Injection wrong or raising | `_extract_injection` (`luminus.py:390-432`) | applicable-vs-estimate `Tarif` capitalisation, VAT-exempt scaling |
| A DSO row missing | `_FLANDERS_LABELS` / `_WALLONIA_LABELS` + row regexes (`luminus.py:571-580`, `583-634`, `637-643`, `646-699`) | printed label renamed, or column count changed |
| Dynamic data-management fee wrong (Flanders) | footnote regex (`luminus.py:601-609`) | `quart d'heure ... gestion des données` phrasing drift |
| Tax value zeroed / block too short | `_tax_block_values` + `_extract_per_kwh_taxes` (`luminus.py:435-504`) | colon anchor, value-run boundary, BTNR/BTR ordering |
| Yearly / exclusive-night fee wrong | `_extract_yearly_fee` / `_extract_excl_night_fee` (`luminus.py:265-297`) | `Redevance fixe` line format, third-column `-` handling |
| Publication label empty | `_extract_publication_month` (`luminus.py:378-387`) | parens padding / month spelling |
| A new product appears / a slug 404s | `_CONTRACTS` + `discover` (`luminus.py:112-129`, `155-169`) | add a `_ContractDef`; sitemap slug directory |
