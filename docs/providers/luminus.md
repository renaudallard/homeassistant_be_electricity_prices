...
| Probe | none: the `SupplierExtractor` is built without a `probe` argument, so it defaults to `None` | `luminus.py:1014-1031` |
...
(`_REGION_TO_TAB`, `luminus.py:96-108`). The response filename encodes the
...
Declared in `_CONTRACTS` (`luminus.py:119-136`); `_CONTRACTS_BY_ID` indexes them
(`luminus.py:137`); `EXTRACTOR.contracts` is built from them (`luminus.py:1010-1019`).

Retired / omitted product: **Luminus Sociaal/Social** (the regulated CREG
tariff) is intentionally not declared (`luminus.py:132-135`), same reasoning as
...
(`luminus.py:447-451`), which is what routes the coordinator to aggregate
...
`fetch(session, contract_id, region)` (`luminus.py:182-197`):
...
`discover(session)` (`luminus.py:162-176`) is a CI / live-check helper (not part
...
structure (`_PRODUCT_PAGE_RE`, `luminus.py:153-155`), excluding the regulated
social-tariff index pages (`_EXCLUDED_SLUGS`, `luminus.py:160`). A failed sitemap
fetch returns an empty set rather than raising.

## Parsing

`parse_snapshot` (`luminus.py:298-337`) assembles the snapshot from a set of
...
branches hard on `region` (`luminus.py:312-317`) and never merges. That region-
...
| Field | Helper | Source |
| --- | --- | --- |
| Energy rates | `_extract_energy` | `luminus.py:403-478` |
| Injection | `_extract_injection` | `luminus.py:550-683` |
| Publication label | `_extract_publication_month` | `luminus.py:653-662` |
| Per-kWh taxes (excise, contribution, connection) | `_extract_per_kwh_taxes` | `luminus.py:774-804` |
| Energy fund (Flanders only) | `_extract_energy_fund` | `luminus.py:807-816` |
| Flanders renewables | `_extract_flanders_renewables` | `luminus.py:819-850` |
| Wallonia renewables | `_extract_wallonia_renewables` | `luminus.py:853-865` |
| Flanders DSO overlay | `_extract_flanders_dsos` | `luminus.py:825-876` |
| Wallonia DSO overlay | `_extract_wallonia_dsos` | `luminus.py:888-937` |
| Yearly fixed fee | `_extract_yearly_fee` | `luminus.py:438-450` |
| Exclusive-night fee | `_extract_excl_night_fee` | `luminus.py:453-470` |
| VAT multiplier | `_vat_multiplier` | `luminus.py:430-435` |
| `valid_until` | `parse_valid_until` (shared) | `_pdf.py:1178-1280` |
...
(`luminus.py:345-347`). Values are parsed with the shared `to_float` (handles
...
EUR/kWh (`luminus.py:419-421`, `346-349`). Prices are 6% VAT inclusive as printed
...
`_extract_publication_month` (`luminus.py:653-662`) reads the parenthesised
...
`_extract_energy(text, kind)` (`luminus.py:406-481`) always parses the yearly
...
creuses / exclusif-nuit), each divided by 100 (`luminus.py:534-543`). `fixed`
returns `FixedRates(single, peak, offpeak, exclusive_night, ...)`
(`luminus.py:543-551`); `variable` returns `VariableRates(current=mono, peak,
offpeak, exclusive_night, ...)` (`luminus.py:474-481`). Illustrative
...
first-year "free Sundays" promo is not modelled (`luminus.py:422-427`).
...
(`luminus.py:353-356`, matched at `luminus.py:437-441`). The sign character is
...
6% VAT multiplier (`luminus.py:443-451`):
...
The VAT rate is read by `_vat_multiplier` (`luminus.py:430-435`), which wraps the
...
convention as Engie), so `transport` is set to `0.0` there (`luminus.py:886-887`,
`luminus.py:723`).

### Flanders (`_extract_flanders_dsos`, `luminus.py:825-876`)

Eight Fluvius sub-areas mapped by printed label to canonical key
(`_FLANDERS_LABELS`, `luminus.py:825-888`). Watch the two label-to-key surprises:
...
(`luminus.py:692-700`, applied at `luminus.py:719-721`).
...
### Wallonia (`_extract_wallonia_dsos`, `luminus.py:888-937`)
...
Two column layouts (`luminus.py:750-777`):
...
(`luminus.py:933-984`). They are mapped to `distribution_eco` / `_medium` /
`_pic` accordingly (`luminus.py:975-986`). Illustrative
(`test_comfy_wallonia_fixed_rates_and_dso`): AIEG mono `0.1087`, pleines
`0.1205`, creuses `0.0666`, transport `0.0274`, prosumer `81.03`.

## Tax overlay

`_extract_per_kwh_taxes` (`luminus.py:774-804`) reads the
`3 Taxes et redevances : WAL|FL|BRU` block via `_tax_block_values`
(`luminus.py:534-571`). That helper anchors on the colon after the label because
`Taxes et redevances` also appears in the `Composition du prix` legend without a
colon or region (`luminus.py:559-562`); the block runs until
...
`_NUM` tokens (`luminus.py:348`). The label order and matching value order are
documented in the `_tax_block_values` docstring (`luminus.py:738-761`): BTNR,
...
underbilling (`luminus.py:591-602`). Illustrative
...
(`_extract_energy_fund`, `luminus.py:807-816`). In both fixture regions today BTR
...
(`_extract_flanders_renewables`, `luminus.py:819-850`): the primary regex sums
...
`_extract_injection(text, kind)` (`luminus.py:553-686`) covers two of the three
...
Two anchoring subtleties in the indicative regex (`luminus.py:507-512`):
...
1. **Applicable vs annual estimate.** The card prints both the applicable
   ...
   (`luminus.py:668-676`). `test_injection_uses_applicable_rate_not_annual_estimate`
...
silently crediting nothing (`luminus.py:719-725`). `test_missing_injection_row_fails_loud`
...
`_extract_yearly_fee` (`luminus.py:438-450`) captures the
...
`_extract_excl_night_fee` (`luminus.py:453-470`) reads the third column of the
...
| --- | --- | --- |
| Every field misses / fetch fails | `fetch` + `fetch_pdf_text` (`luminus.py:182-197`, `_pdf.py:179-186`) | URL construction, slug/tabValue, PDF magic-byte validation |
| Energy rates wrong / missing | `_extract_energy` (`luminus.py:403-478`) | four-column vs three-column row, unit /100, TOU lookahead |
| Dynamic factor/base off by ~1.06 or ~10 | dynamic branch (`luminus.py:436-451`) | VAT multiplier + mWh->kWh + c->EUR conversion |
| Injection wrong or raising | `_extract_injection` (`luminus.py:553-686`) | applicable-vs-estimate `Tarif` capitalisation, VAT-exempt scaling |
| A DSO row missing | `_FLANDERS_LABELS` / `_WALLONIA_LABELS` + row regexes (`luminus.py:825-888`, `567-618`, `621-627`, `630-683`) | printed label renamed, or column count changed |
| Dynamic data-management fee wrong (Flanders) | footnote regex (`luminus.py:692-700`) | `quart d'heure ... gestion des données` phrasing drift |
| Tax value zeroed / block too short | `_tax_block_values` + `_extract_per_kwh_taxes` (`luminus.py:689-758`) | colon anchor, value-run boundary, BTNR/BTR ordering |
| Yearly / exclusive-night fee wrong | `_extract_yearly_fee` / `_extract_excl_night_fee` (`luminus.py:371-449`) | `Redevance fixe` line format, third-column `-` handling |
| Publication label empty | `_extract_publication_month` (`luminus.py:653-662`) | parens padding / month spelling |
| A new product appears / a slug 404s | `_CONTRACTS` + `discover` (`luminus.py:119-136`, `148-162`) | add a `_ContractDef`; sitemap slug directory |
