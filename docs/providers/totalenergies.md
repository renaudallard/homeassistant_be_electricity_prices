...
(the union over every contract's `regions`, `providers/base.py:83`) therefore
...
| Validity date | `parse_valid_until` (shared) | `_pdf.py:1004` |
...
is left `None` (`SupplierSnapshot` default, `providers/base.py:825`). The only
...
8. **Publication label + validity**: `_extract_publication_month`
   (`totalenergies.py:470`) and the shared `parse_valid_until` (`_pdf.py:1004`) drive
