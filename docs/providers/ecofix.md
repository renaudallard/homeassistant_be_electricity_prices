...
where every fetch would fail (`ecofix.py`, asserted by
`test_ecofix_is_registered` at `tests/test_ecofix.py`). `EXTRACTOR.regions()`
...
cards. `discover()` reads it (`ecofix.py`), falling back to the old HEAD probe
when the page is unreachable.

## Contracts

Declared in `_CONTRACTS` (`ecofix.py`) and mapped into `Contract` objects in
the registry (`ecofix.py`). All three carry `regions = {flanders, wallonia}`
...
- No product is region-limited within Belgium's two selling regions and none is
  ...
  (`ecofix.py`), and so does a NEW product appear there: the old HEAD probe
...
`fetch(session, contract_id, region)` (`ecofix.py`):
...
2. Builds the URL with `_document_url` and downloads the PDF as
   **layout-preserving text** via `fetch_pdf_text_layout` (`_pdf.py`). Layout
   ...
   that the row-anchored regexes cannot match (`ecofix.py`). `fetch_pdf_text_layout`
...
`probe()` (`ecofix.py`) returns a freshness key via `head_freshness_key`
(`_pdf.py`), which HEADs the per-contract PDF and returns its `Last-Modified`
...
There is **no** `fetch_for_month`: the registry leaves it unset (`ecofix.py`
...
(`ecofix.py`), so the coordinator's proxy-forward fallback bills past
...
`discover(session)` (`ecofix.py`) reads `/tarieven/tariefkaarten` and returns
...
(`tests/test_discover.py`), and the fallback by
`test_head_probe_fallback_returns_every_contract_whose_url_200s`
(`tests/test_ecofix.py`). The registry check alone is vacuous, since the
...
`parse_snapshot` (`ecofix.py`) is the pure entry point; it fans out to a set
of narrowly-anchored helpers. The fields it pulls out:

| Field | Helper | Source |
| --- | --- | --- |
| Yearly fixed fee + Flanders renewables | `_extract_fee_and_flanders_renewables` | `ecofix.py` |
| Energy formula / rates | `_extract_energy` | `ecofix.py` |
| Injection | `_extract_injection` | `ecofix.py` |
| Publication label + `valid_until` | `_extract_publication` | `ecofix.py` |
| Federal excise + energy contribution | `_extract_federal_taxes` | `ecofix.py` |
| Wallonia connection fee | `_extract_wallonia_connection_fee` | `ecofix.py` |
| Wallonia renewables | `_extract_wallonia_renewables` | `ecofix.py` |
| Flanders DSO overlays | `_extract_flanders_dsos` | `ecofix.py` |
| Wallonia DSO overlays | `_extract_wallonia_dsos` | `ecofix.py` |
...
accordingly (`ecofix.py`). A `brussels` region yields an empty `dsos` dict and
zeroed regional levies, kept well-formed even though the registry filter should
never let a Brussels config reach here (`ecofix.py`).
...
- **Label order flips across cards.** The Vlaanderen block prints the yearly fee
  ...
  two tokens is always the renewable and the larger is the fee (`ecofix.py`,
  `ecofix.py`).
- **Vlaanderen block slice.** `_flanders_energy_block` (`ecofix.py`) carves the
...
- **Belpex 15M formula anchoring.** `_dynamic_formula_match` (`ecofix.py`)
  ...
  `test_afname_anchor_does_not_reach_injection_formula` (`tests/test_ecofix.py`).
- **Unit conversion.** PDF formulas are in c/kWh ex-VAT against Belpex in EUR/MWh.
  ...
  banner (`ecofix.py`). Belgian residential VAT is currently 6% and the card
...
- **DSO-name to canonical-key mapping.** Flanders labels map through
  ...
  (`ecofix.py`), where `WAVRE` maps to `rew` and `TECTEO - RESA` to `resa`;
...
`_extract_energy` with `kind == "dynamic"` (`ecofix.py`) reads the `Afname`
...
Illustrative (from `test_motion_online_energy_formula`, `tests/test_ecofix.py`):
...
`_extract_energy` with `kind == "variable"` (`ecofix.py`) reads the indicative
...
(`test_flexy_is_variable_with_indicative_monthly_rate`, `tests/test_ecofix.py`).
...
`_extract_flanders_dsos` (`ecofix.py`) parses the eight Fluvius sub-areas from
...
diverges the two regimes (`ecofix.py`,
...
`prosumer_eur_per_kva_year`, digital rows included (`ecofix.py`). Nothing
filters it by meter type, which this said for a long time: the only gate is
`_compensation_kva` (`fees.py`), which bills the prosumer fee on Walloon
compensation entries alone, so on a Flemish overlay the value is never read at
all (`tests/test_ecofix.py`).
...
`fluvius_zenne_dijle` (all eight, `tests/test_ecofix.py`).

### Wallonia

`_extract_wallonia_dsos` (`ecofix.py`) parses each Walloon DSO row, which carries
...
(c/kWh). `_build_wallonia_overlay` (`ecofix.py`) unpacks these into a
...
(`ecofix.py`): the card lists nine numerically-identical ORES sub-area rows,
...
(`test_ores_subarea_drift_is_rejected`, `tests/test_ecofix.py`).
...
(`providers/base.py`) it means the snapshot's prices are already handled for
...
`tests/test_ecofix.py`): federal_excise 0.0503288, energy_contribution
0.0020417, flanders_renewables 0.016.

`_extract_wallonia_connection_fee` (`ecofix.py`) and `_extract_wallonia_renewables`
(`ecofix.py`) are fatal on a miss because both are mandatory in Wallonia. The
...
- **Dynamic (Motion, Motion Online): hourly `factor*spot+base` (spot-indexed).**
  `_extract_injection` with `kind == "dynamic"` (`ecofix.py`) anchors on the
  ...
  residential injection is VAT-exempt, `ecofix.py`), and surfaces the printed
  ...
  (`test_dynamic_injection_missing_formula_is_fatal`, `tests/test_ecofix.py`).
  Illustrative (`test_motion_online_injection`, `tests/test_ecofix.py`):
...
- **July 2026 Flexy reflow (regression fix).** The July 2026 Flexy card re-rendered
  ...
  `Verbruik tussen 0 & 3.000 kWh` row cannot shadow it (`ecofix.py`,
...
- **Afname anchor must not reach the Injectie formula.** The tempered lookahead in
  ...
  formula would bind the injection formula to consumption (`ecofix.py`,
  `tests/test_ecofix.py`).
- **Injection role-swap protection.** Both dynamic formulas are anchored on their
  ...
  consumption and injection (`ecofix.py`).
...
- **Publication label scan skips a colliding version token.** The product name
  ...
  (`ecofix.py`, `test_publication_scan_skips_colliding_version_token`,
  `tests/test_ecofix.py`). The card has no `geldig`/`valable` keyword, so the
...
- **Two data-management columns per Fluvius row.** Bill the column matching the
  ...
  fifth); they are equal today (`ecofix.py`).
...
- **Mandatory Walloon fees are fatal on a miss.** The connection fee and green-energy
  contribution raise rather than zero out (`ecofix.py`,
  `test_missing_wallonia_connection_fee_is_fatal`, `tests/test_ecofix.py`).
...
All three are the May 2026 edition (`tests/test_ecofix.py`). Several tests
...
1. `_extract_fee_and_flanders_renewables` (`ecofix.py`) and `_flanders_energy_block`
   (`ecofix.py`): the Vlaanderen block is the most re-flowed part of the card
   (the July 2026 regression lived here). Watch the fee/renewable order and the
   `Verbruik` / `meter Piekuren` anchors.
2. `_dynamic_formula_match` (`ecofix.py`) and the `_extract_energy` /
   `_extract_injection` dynamic branches: any change to the `Afname` / `Injectie`
   labels, the `Belpex 15M` wording, or the sign glyph.
3. `_extract_flanders_dsos` (`ecofix.py`) and `_FLANDERS_LABELS` (`ecofix.py`):
...
4. `_extract_wallonia_dsos` / `_extract_ores` / `_ORES_PATTERN` (`ecofix.py`):
   a Walloon column reorder or an ORES sub-area split (the latter raises by design).
5. `_extract_publication` (`ecofix.py`): a new header token near the month, or a
   language switch away from Dutch month names in `_DUTCH_MONTHS` (`ecofix.py`).
6. `discover()` / `_document_url` (`ecofix.py`): a change to the
