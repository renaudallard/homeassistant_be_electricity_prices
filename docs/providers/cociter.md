...
Each row produces a `DsoOverlay` (`base.py:371-415`) with the four distribution
...
re-apply VAT (see `TaxOverlay` comment, `base.py:531-578`). The tax test
...
(`SupplierSnapshot` comment, `base.py:599-629`). A miss on the variable card is
...
`DsoOverlay` and `ImpactRates`, `base.py:371-415`, `base.py:255-277`).
