...
(see `DynamicRates`, `base.py:180-220`).
...
re-apply VAT (see `TaxOverlay` comment, `base.py:682-729`). The tax test
...
(`SupplierSnapshot` comment, `base.py:750-780`). A miss on the variable card is
...
`DsoOverlay` and `ImpactRates`, `base.py:536-586`, `base.py:357-389`).
