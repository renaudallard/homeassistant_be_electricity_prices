...
re-apply VAT (see `TaxOverlay` comment, `base.py:726-773`). The tax test
...
(`SupplierSnapshot` comment, `base.py:794-824`). A miss on the variable card is
...
(`InjectionRates` comment, `base.py:469-496`). Unit handling mirrors the dynamic
