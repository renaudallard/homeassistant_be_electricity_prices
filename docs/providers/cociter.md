...
`frozenset({REGION_WALLONIA})` (`cociter.py:527`, `cociter.py:537`,
`cociter.py:546`) and
`EXTRACTOR.regions()` (the union over contracts, see
`base.py:383`) resolves to Wallonia alone.

Cociter publishes one PDF card per (product, month) under predictable
filenames on a single listing page (`cociter.py:28-38`, `cociter.py:79`):
...
|----|-------|------------|---------|------------------------|----------------|-------|
| `cociter_variable` | Cociter Tarif Variable | `variable` | Wallonia | `True` | n/a | BELIX-indexed monthly variable energy. Publishes per-meter indicative rates (mono, bi-hourly, exclusive-night). Injection is an hourly `factor*spot+base` BELPEX formula with no printed monthly indicative, so it needs an ENTSO-E spot: hence `spot_indexed_injection=True` (`cociter.py:538-540`). |
| `cociter_dynamic` | Cociter Tarif Dynamique | `dynamic` | Wallonia | `False` (default) | `True` | SMR3 quarter-hourly BELPEX dynamic contract. Bills on the native 15-minute Belpex grid, so `DynamicRates.quarter_hourly=True` (`cociter.py:392`). Dynamic contracts already collect the ENTSO-E key via the energy formula, so `spot_indexed_injection` stays `False`. |
...
`_CONTRACT_PATTERNS` map (`cociter.py:126-129`) binds each id to the filename
...
(`base.py:69-75`). Do not drop it; without a spot curve the variable card's
...
`fetch` (`cociter.py:132-144`) looks up the filename pattern for the requested
...
unknown `contract_id` raises `ExtractorError` (`cociter.py:139-140`), covered
by `test_unknown_contract_raises`.

`_find_latest` (`cociter.py:507-517`) fetches the listing HTML once, runs the
contract's regex (`_VAR_RE` / `_DYN_RE`, `cociter.py:101-106`) to collect every
...
listing links no matching card (`cociter.py:512-513`).

### Freshness probe: `probe`

`probe` (`cociter.py:184-202`) is the cheap freshness key the coordinator
...
any fetch error it returns `None` (`cociter.py:200-201`), letting the
...
(`cociter.py:147-181`) builds the target `YYMM` from the requested
...
- unknown contract id (`cociter.py:161-163`),
- listing fetch fails (`cociter.py:166-168`),
- the listing does not link the requested month (`cociter.py:174-175`),
- the PDF 404s or does not parse (`cociter.py:176-180`),
- `archive_validity_check` rejects the card as not covering the month
  (`cociter.py:181`).

`archive_validity_check` (`_pdf.py:638-674`) is two-tier: if the parsed
...
the French month names `_FR_MONTHS` (`cociter.py:84-97`). This guards against a
CDN serving a substituted current card under an archived URL, which would
otherwise mis-bill past consumption.

The three `fetch_for_month` tests (`test_cociter.py:274-320`) exercise: a
...
returning `None`. The listing fixture `_LISTING_HTML` (`test_cociter.py:243-247`)
is inline HTML with three `RCVar_YMR_Coop-YYMM-fr.pdf` links.

### Product discovery: `discover`

`discover` (`cociter.py:212-228`) is the new-product tripwire. It fetches the
...
via `_DISCOVER_FAMILIES` (`cociter.py:206-209`) and surfacing any unknown
family verbatim. An unrecognised family in the output means Cociter has added a
product the integration does not yet model.

## Parsing

`parse_snapshot` (`cociter.py:231-249`) is the pure, test-exposed parser. It
assembles a `SupplierSnapshot` from six sub-parsers. The mapping of snapshot
field to helper:

| Snapshot field | Helper | Source |
|----------------|--------|--------|
| `energy` | `_extract_energy` | `cociter.py:328-393` |
| `dsos` | `_extract_dsos` (+ `_extract_transport`) | `cociter.py:396-455` |
| `taxes` | `_extract_taxes` | `cociter.py:458-504` |
| `injection` | `_extract_injection` | `cociter.py:275-325` |
| `supplier_prosumer_eur_per_kva_year` | `_extract_supplier_prosumer` | `cociter.py:252-272` |
| `valid_until` | `parse_valid_until` (shared) | `_pdf.py:677` |

Shared numeric helpers: `to_float` (`_pdf.py:452-463`) parses Belgian decimals
(`15,93`) and strips every Unicode space variant used as a thousands separator;
`parse_sign` (`_pdf.py:477-484`) turns any hyphen/dash/Unicode-minus into
`-1.0`; `SIGN_CHARS` (`_pdf.py:473`) is the character class of accepted sign
glyphs. `fetch_pdf_text` (`_pdf.py:158-165`) downloads the PDF and extracts
text with pypdf off the event loop.

### Energy: `_extract_energy`

The yearly abonnement is common to both products, matched by
`(\d+,\d+) €/an ... TVAC` (`cociter.py:329`). A miss is fatal
(`ExtractorError`, `cociter.py:334`) rather than a silent zero standing charge.
...
For `cociter_variable` (`cociter.py:337-366`), four indicative per-meter rates
...
Only the mono rate is mandatory (`cociter.py:342-345`); the others are `None`
when absent so pricing falls back to `current`. The BELIX indexation formula is
parsed for the diagnostic `formula` string only (`cociter.py:349-353`); it
...
For `cociter_dynamic` (`cociter.py:368-393`) the SMR3 formula
...
"HOURLY") via `QUARTER\s*HOURL\s*Y`. The unit conversion (`cociter.py:380-390`)
is the subtle part:
...
`test_dynamic_extracts_factor_and_base` (`test_cociter.py:172-187`) pins the
...
`quarter_hourly=True` flag (`cociter.py:392`) keeps the native 15-minute slots
(see `DynamicRates`, `base.py:127-148`).

### DSO overlay: `_extract_dsos`

`_extract_dsos` (`cociter.py:396-444`) parses one row per Wallonian DSO. The
DSO label to canonical registry key map is `_DSO_KEY` (`cociter.py:114-120`):
...
An assertion (`cociter.py:122-124`) enforces that this key set equals
...
The two card layouts differ by column 6 onward (`cociter.py:398-411`):
...
the document (`cociter.py:413`), not on column count. This is deliberate: an
end-of-line anchor would silently lose the prosumer value if a 7th column were
ever added to the variable card. The row regex (`cociter.py:418-423`) captures
...
`test_dso_extraction_keys_off_header_not_column_count` (`test_cociter.py:118-133`)
...
(`test_cociter.py:74-89`) pins illustrative AIEG values: `distribution_single
...
Each row produces a `DsoOverlay` (`base.py:248-285`) with the four distribution
...
`_extract_transport` (`cociter.py:447-455`) parses the single ELIA transport
...
20% of the all-in (illustrative). A miss is fatal (`cociter.py:454`);
under-billing every kWh silently is worse than a loud failure. The
`test_missing_transport_or_abonnement_is_fatal` test (`test_cociter.py:136-153`)
confirms both this and the abonnement raise.

### Taxes: `_extract_taxes`

`_extract_taxes` (`cociter.py:458-504`) pulls three things:
...
1. The Walloon renewables contribution, anchored on the quoted heading
   ...
   the number within ~200 chars (`cociter.py:464-469`). A miss is fatal
   (`cociter.py:485-491`); the ~3 c€/kWh contribution is mandatory.
2. The `Taxes et redevances` block, a single line of three values anchored on
   ...
   Redevance de raccordement` (`cociter.py:475-482`). A miss is fatal
   (`cociter.py:483-484`).

Mapping into `TaxOverlay` (`cociter.py:498-504`): `energy_contribution`
...
Critically `vat_rate=0.0` (`cociter.py:503`): the whole card is TVAC, so the
snapshot's prices are already VAT-inclusive and the pricing engine must not
re-apply VAT (see `TaxOverlay` comment, `base.py:306-308`). The tax test
(`test_cociter.py:156-169`) pins illustrative Wallonian values and asserts
...
`_extract_supplier_prosumer` (`cociter.py:252-272`) parses the variable card's
supplier-side PV forfait, billed on top of the DSO prosumer column. It is
returned only for `cociter_variable` (`cociter.py:267-268`); the dynamic SMR3
...
`([\d,]+) €/kVA/an TVAC` (`cociter.py:269`). The card prints 37,10 EUR/kVA/an
TVAC (illustrative, pinned by `test_variable_extracts_supplier_prosumer_forfait`,
`test_cociter.py:92-101`). The anchor is deliberately the "EUR/kVA/an TVAC"
...
(`SupplierSnapshot` comment, `base.py:327-332`). A miss on the variable card is
fatal (`cociter.py:270-271`): every variable card prints it, so absence is a
...
(`cociter.py:114-120`). Every card carries all five rows. The overlay per DSO
...
`DsoOverlay` and `ImpactRates`, `base.py:185-208`, `base.py:274-284`).
...
`_extract_injection` (`cociter.py:275-325`) sets `current=None` and populates
`factor` and `base` (`cociter.py:320-325`). Because there is no indicative
...
(`cociter.py:538-540`); dropping the gate or the spot silently drifts the solar
credit.

Injection is VAT-exempt for residential, so `factor`/`base` are never VAT-scaled
(`InjectionRates` comment, `base.py:216-229`). Unit handling mirrors the dynamic
...
(`cociter.py:318-323`). From the printed `(0,097 x BELPEX - 2,1)` the tests pin
`factor == 0.97` and `base == -0.021` (illustrative,
`test_cociter.py:190-217`). The variable and dynamic cards carry the same
...
- The dynamic SMR3 card carries two formulas (consumption first, injection
  ...
  glyph (`cociter.py:299-306`). The apostrophe class `['‘’ʼ]` tolerates the
...
(`cociter.py:316-317`): both products always publish an injection formula, so
...
`test_injection_missing_formula_raises` (`test_cociter.py:220-229`) confirms the
raise.
...
- **Everything is TVAC.** Prices are VAT-inclusive, so `vat_rate=0.0`
  (`cociter.py:503`, `base.py:306-308`). The supplier PV forfait is likewise
  already TVAC and must never be VAT-scaled (`cociter.py:261-262`).
- **Fail-loud parsers.** The abonnement (`cociter.py:334`), ELIA transport
  (`cociter.py:454`), taxes block (`cociter.py:484`), Walloon renewables
  (`cociter.py:489-491`), injection formula (`cociter.py:317`), and the
  variable PV forfait (`cociter.py:271`) all raise on a miss rather than
...
- **Header-based DSO discrimination.** Column 6 means "prosumer" on the
  ...
  to survive future column additions (`cociter.py:408-413`).
- **BELPEX unit + VAT conversion.** Dynamic factor is `factor_pdf * vat_mult *
  10` and base is `base_c * vat_mult / 100` (`cociter.py:380-390`). The VAT
...
- **quarter_hourly=True.** Cociter Dynamique bills on the 15-minute Belpex grid;
  keep native quarter-hour slots, not the hourly mean (`cociter.py:386-392`,
  `base.py:127-148`). YTD statistics still aggregate to hourly.
- **Split-glyph spellings.** pypdf can split "HOURLY" into `HOURL Y` and emit
  ...
  (`cociter.py:300-306`, `cociter.py:374`, `SIGN_CHARS`, `_pdf.py:473`).
- **Injection has no indicative fallback.** `current=None` always; the credit
  is spot-only, gated on `spot_indexed_injection` (`cociter.py:538-540`).
...
- **Archive validity cross-check.** `fetch_for_month` runs
  ...
  (`cociter.py:181`, `_pdf.py:638-674`).
- **DSO map / const lockstep assertion.** `_DSO_KEY` must equal
  `WALLONIA_DSO_KEYS` or import fails (`cociter.py:122-124`).
...
(`test_cociter.py:243-247`), not a file, and models the listing page's
...
1. **Filename or listing URL change** -> `_VAR_RE` / `_DYN_RE`
   (`cociter.py:101-106`), `_INDEX_URL` (`cociter.py:79`), and the `discover`
   family regex (`cociter.py:224-226`). Everything (fetch, probe,
   fetch_for_month, discover) keys off these.
2. **Energy row labels or formula wording** -> `_extract_energy`
   (`cociter.py:328-393`): the `Compteur monohoraire` / `Heures pleines` /
...
3. **DSO table layout / column order / new column** -> `_extract_dsos`
   (`cociter.py:396-444`) and the `"Tarif prosumer"` header discriminator
   (`cociter.py:413`). A new DSO also needs `_DSO_KEY` and
   `const.WALLONIA_DSO_KEYS` updated together (`cociter.py:122-124`).
4. **Injection formula relocation or wording** -> `_extract_injection`
   (`cociter.py:275-325`): the `Le prix de l'injection` anchor and the
   `Tout compteur` fallback.
5. **Tax block relabeling** -> `_extract_taxes` (`cociter.py:458-504`): the
   `énergies renouvelables` and `Cotisation énergie / Droit d'accises spécial /
   Redevance de raccordement` anchors.
6. **Transport row rename** -> `_extract_transport` (`cociter.py:447-455`).
7. **PV forfait footnote rewording** -> `_extract_supplier_prosumer`
   (`cociter.py:252-272`): the `EUR/kVA/an TVAC` anchor.
8. **Validity-header format change** -> shared `parse_valid_until`
   (`_pdf.py:677`) and the `_FR_MONTHS` fallback used by
