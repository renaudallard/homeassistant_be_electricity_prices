...
Each row produces a `DsoOverlay` (`base.py:370-407`) with the four distribution
...
re-apply VAT (see `TaxOverlay` comment, `base.py:523-570`). The tax test
...
(`SupplierSnapshot` comment, `base.py:591-621`). A miss on the variable card is
...
`DsoOverlay` and `ImpactRates`, `base.py:370-407`, `base.py:255-277`).
