...
(see `DynamicRates`, `base.py:166-185`).
...
Each row produces a `DsoOverlay` (`base.py:380-424`) with the four distribution
...
re-apply VAT (see `TaxOverlay` comment, `base.py:540-587`). The tax test
...
(`SupplierSnapshot` comment, `base.py:608-638`). A miss on the variable card is
...
`DsoOverlay` and `ImpactRates`, `base.py:380-424`, `base.py:264-286`).
...
- **quarter_hourly=True.** Cociter Dynamique bills on the 15-minute Belpex grid;
  ...
  `base.py:166-185`). YTD statistics still aggregate to hourly.
