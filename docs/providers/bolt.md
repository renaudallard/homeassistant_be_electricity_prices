...
into `_fetch_pdf_text` (`bolt.py:406`) precisely so `live_check.py` can fetch a card once and parse
...
`probe` (`bolt.py:365`) HEADs the listing page and returns the first present header, preferring
...
`discover` (`bolt.py:384`) GETs the listing HTML and returns the set of `<folder>/<slug>` prefixes
...
>
> `_extract_legacy_energy` (`bolt.py:669`) reads the older shape, keyed on which
...
| `taxes.{flanders,wallonia,brussels}_renewables` | `_extract_renewables` (`bolt.py:1173`) | certificats verts + Flanders WKK; zeroed outside the active region |
...
1. **Monthly platform fee, billed annually.** `_extract_yearly_fee` (`bolt.py:604`) matches
   ...
   parses. `test_fix_yearly_fee_is_monthly_x_12` (`tests/test_bolt.py:65`) asserts `10.99 * 12`
   (illustrative).
...
would break. `test_variable_uses_current_monthly_not_annual_estimate` (`tests/test_bolt.py:157`)
...
- `variable`: a miss is a layout drift, not a mono contract; variable cards always publish distinct
  ...
  rate. `test_variable_missing_bihourly_rates_fails_loud` (`tests/test_bolt.py:149`) enforces this.
...
- The July 2026 fix cards print a NEGATIVE second column (`Prix mensuel 3,40 -0,43`, the "Exclusif
  ...
  `test_injection_accepts_negative_second_column` (`tests/test_bolt.py:133`) locks this in.
...
absorbed it. `_row_is_explicit_zero` (`bolt.py:1031`) recognises that shape and prices it as 0. A dash
...
`test_taxes_split_correctly_per_region` (`tests/test_bolt.py:171`) checks nationwide excise
...
`test_wallonia_dso_handles_vertical_layout` (`tests/test_bolt.py:197`) exercises this.

**Flanders (`_extract_flanders_dsos`, `bolt.py:1220`).** Eight Fluvius sub-areas via `_FLANDERS_LABELS`
...
**Wallonia (`_extract_wallonia_dsos`, `bolt.py:1286`).** Five DSOs via `_WALLONIA_LABELS`
...
`bolt.py:706`). `test_resa_is_cheaper_than_rew_after_label_swap` (`tests/test_bolt.py:213`) guards
the invariant in CI.

**Brussels (`_extract_brussels_dsos`, `bolt.py:1372`).** One row, `Sibelga`, with six captured
...
`test_brussels_extracts_sibelga` (`tests/test_bolt.py:242`) checks distribution 0.0996, off-peak
...
- **U+2028 line separators.** Normalized to `\n` at the top of `parse_snapshot` (`bolt.py:534`);
...
1. **DSO row regexes** (`_extract_flanders_dsos` `bolt.py:1220`, `_extract_wallonia_dsos`
   `bolt.py:719`, `_extract_brussels_dsos` `bolt.py:1372`). Column-count changes, a renamed sub-area
...
2. **RESA/REW swap** (`_WALLONIA_LABELS` `bolt.py:1277`). If the ERROR invariant fires, Bolt probably
   fixed the upstream layout; remove the swap and re-point the labels straight.
3. **`_extract_energy` bi-horaire span** (`bolt.py:735`). The two-`Jour Nuit`-subhead anchor is
   fragile; if Bolt reorders the injection/consumption blocks or drops a subhead, the variable path
   raises loud.
4. **`_extract_yearly_fee`** (`bolt.py:604`). A phrasing change away from `€ N / mois` raises.
5. **`_extract_injection`** (`bolt.py:927`). A relabeled `Injection` header or a third
   consumption-side `Prix mensuel` row shifts the anchor; a new second-column sign convention needs
   the `-?` tolerance revisited.
6. **`_extract_taxes` / `_extract_renewables`** (`bolt.py:1173`, `bolt.py:1173`). Federal levy and
   certificats-verts misses raise; the connection-fee footnote `{0,3}` cap may need widening if Bolt
   adds markers.
7. **URL construction** (`_document_url` `bolt.py:294`, `_resolve_variable_suffix` `bolt.py:309`).
   ...
   `_CARD_URL_RE` (`bolt.py:140`) would stop matching; `discover` (`bolt.py:384`) plus the
