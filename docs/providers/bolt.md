...
`test_bolt_is_registered` (`tests/test_bolt.py:50`) pins the count at exactly seven and asserts
...
1. **Monthly platform fee, billed annually.** `_extract_yearly_fee` (`bolt.py:370`) matches
   ...
   parses. `test_fix_yearly_fee_is_monthly_x_12` (`tests/test_bolt.py:61`) asserts `10.99 * 12`
   (illustrative).
...
would break. `test_variable_uses_current_monthly_not_annual_estimate` (`tests/test_bolt.py:121`)
...
- `variable`: a miss is a layout drift, not a mono contract; variable cards always publish distinct
  ...
  rate. `test_variable_missing_bihourly_rates_fails_loud` (`tests/test_bolt.py:113`) enforces this.
...
- The July 2026 fix cards print a NEGATIVE second column (`Prix mensuel 3,40 -0,43`, the "Exclusif
  ...
  `test_injection_accepts_negative_second_column` (`tests/test_bolt.py:100`) locks this in.

`test_injection_is_flat_monthly_indicative` (`tests/test_bolt.py:84`) checks both fix and variable
...
`test_taxes_split_correctly_per_region` (`tests/test_bolt.py:135`) checks nationwide excise
...
`test_wallonia_dso_handles_vertical_layout` (`tests/test_bolt.py:161`) exercises this.
...
**Wallonia (`_extract_wallonia_dsos`, `bolt.py:719`).** Five DSOs via `_WALLONIA_LABELS`
...
`bolt.py:706`). `test_resa_is_cheaper_than_rew_after_label_swap` (`tests/test_bolt.py:177`) guards
the invariant in CI.
...
`test_brussels_extracts_sibelga` (`tests/test_bolt.py:206`) checks distribution 0.0996, off-peak
...
1. **DSO row regexes** (`_extract_flanders_dsos` `bolt.py:653`, `_extract_wallonia_dsos`
   `bolt.py:719`, `_extract_brussels_dsos` `bolt.py:805`). Column-count changes, a renamed sub-area
...
2. **RESA/REW swap** (`_WALLONIA_LABELS` `bolt.py:710`). If the ERROR invariant fires, Bolt probably
...
5. **`_extract_injection`** (`bolt.py:505`). A relabeled `Injection` header or a third
   consumption-side `Prix mensuel` row shifts the anchor; a new second-column sign convention needs
   the `-?` tolerance revisited.
6. **`_extract_taxes` / `_extract_renewables`** (`bolt.py:547`, `bolt.py:613`). Federal levy and
