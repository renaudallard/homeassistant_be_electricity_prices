...
| Publication shape | Monthly PDF card per contract, at a predictable CDN URL; a public HTML listing page links every current PDF | `bolt.py:28`, `bolt.py:112` |
| Fetch transport | `fetch_pdf_text_layout` (pdfplumber, layout-aware) | `bolt.py:241` |
| Probe | HEAD the listing page, prefer `ETag` then `Last-Modified` | `bolt.py:180` |
| Archive | Only `bolt_fix` (slug `fix`) is monthly-archived back to 2024-01; everything else falls back to the current snapshot | `bolt.py:281` |
| VAT convention | Prices are VAT-incl; `vat_rate=0.0` | `bolt.py:369`, `base.py:307` |
...
Two folder families, one filename convention (`bolt.py:168`):
...
(`_VARIABLE_SUFFIX = "11"` today, `bolt.py:114`). The `_fr_` token means the French-language card;
all three regions still live inside that one French document. The listing page
`https://www.boltenergie.be/fr/listes-des-prix` (`bolt.py:113`) links every current PDF directly.

## Contracts

Bolt declares seven residential-electricity contracts (`bolt.py:144`). All are region-unrestricted
...
`test_bolt_is_registered` (`tests/test_bolt.py:49`) pins the count at exactly seven and asserts
...
`fetch(session, contract_id, region)` (`bolt.py:268`) validates the contract id, calls the shared
`_fetch_pdf_text` to get `(url, text)`, then hands off to `parse_snapshot`. The download is factored
into `_fetch_pdf_text` (`bolt.py:222`) precisely so `live_check.py` can fetch a card once and parse
...
- **60 s PDF timeout** (`bolt.py:238`). The shared default is 30 s, but Bolt's CDN occasionally
...
- **Fixed-card previous-month fallback** (`bolt.py:242`). Fixed cards may not be published yet on
  ...
  cannot signal staleness through it; the `_LOGGER.warning` at `bolt.py:256` is the only trace that
  last month's card is being served. Variable-folder contracts do not get this fallback (they
  re-raise, `bolt.py:250`).

The month suffix in `_document_url` is deliberately `dt_util.now()` (Brussels local) and not UTC
(`bolt.py:172`): UTC would mis-key by last month for the first 1-2 Brussels hours of every month.

### `probe` (freshness)

`probe` (`bolt.py:180`) HEADs the listing page and returns the first present header, preferring
...
`discover` (`bolt.py:199`) GETs the listing HTML and returns the set of `<folder>/<slug>` prefixes
...
`fetch_for_month(session, contract_id, region, year_month)` (`bolt.py:281`) supports the
time-correct yearly-cost flow. It gates on folder `fix` AND slug `fix` (`bolt.py:299`) and returns
...
`None`, and the YTD path falls back to the current snapshot as a proxy (`bolt.py:291`).
...
today's rates. So after parsing it runs `archive_validity_check` (`bolt.py:317`,
...
(`tests/test_bolt.py:230`) pins this: the April fixture is accepted for April 2026 and rejected for
January 2026.

## Parsing

`parse_snapshot(contract_id, text, region, source_url)` (`bolt.py:320`) is the pure parser exposed
...
(`bolt.py:327`). Then it fans out to the field extractors and assembles a `SupplierSnapshot`.
...
| `injection` | `_extract_injection` (`bolt.py:506`) | flat monthly indicative (`factor`/`base` = `None`); spot-indexed `factor`/`base` for `bolt_dynamic` |
...
2. **`Prix mensuel` is the current month's price for every kind.** The line prints two adjacent
   ...
   NOT a day/peak rate) (`bolt.py:440`). Values are in c/kWh, so the parser divides by 100.
...
(`bolt.py:453`). The bi-horaire consumption row is the LAST same-line adjacent-number pair between
...
The fallback logic when no bi-horaire pair is found is kind-dependent (`bolt.py:470`):
...
(`bolt.py:537`); there is no spot dependency. For `bolt_dynamic` the same card's injection formula
...
`test_injection_is_flat_monthly_indicative` (`tests/test_bolt.py:80`) checks both fix and variable
...
regex miss raises (`bolt.py:565`); the apostrophe class `['’]` handles both straight and curly
...
values (`bolt.py:577`); the `{0,3}` cap deliberately stops a future integer-only Flanders value from
...
leading digits of a multi-digit value (`bolt.py:621`). Certificats verts is charged in every region,
...
(`tests/test_bolt.py:194`) checks Antwerpen: transport 0.0, distribution 0.0535, exclusive-night
...
(`bolt.py:706`). Ten numbers per row: mono, jour, nuit, excl_nuit, PIC, MEDIUM, ECO, transport,
...
every card parsed). The check uses a process-wide `_RESA_REW_LOGGED` latch (`bolt.py:110`) so it
rings HA's notification bell at most once per boot. Three outcomes (`bolt.py:761`):
...
`bolt.py:642`). `test_resa_is_cheaper_than_rew_after_label_swap` (`tests/test_bolt.py:180`) guards
the invariant in CI.
...
- **VAT-incl.** Prices are already VAT-incl, so `vat_rate=0.0` (`bolt.py:369`). An extractor that
...
- **No parseable `valid_until`.** Bolt cards print `Carte Tarifaire Bolt Fixe <Month> <Year>` but no
  ...
  falls back to a textual month match on `_FR_MONTH_NAMES` (`bolt.py:116`, `bolt.py:317`).
- **U+2028 line separators.** Normalized to `\n` at the top of `parse_snapshot` (`bolt.py:327`);
  every downstream regex depends on that.
- **5 MB PDFs, slow CDN.** 60 s timeout to survive a 2-3x slowdown (issue #13, `bolt.py:236`).
...
- **First-of-month fixed fallback.** Missing current-month fix card falls back to the previous month
  with a warning; Brussels-local month math avoids the UTC seam (`bolt.py:242`).
...
- **Exclusive-night everywhere.** `Prix mensuel` group 2 and Fluvius group 4 and the Sibelga column
  are all dedicated night-circuit rates, not day/peak rates (`bolt.py:439`, `bolt.py:681`,
  `bolt.py:764`).
...
2. **RESA/REW swap** (`_WALLONIA_LABELS` `bolt.py:706`). If the ERROR invariant fires, Bolt probably
   fixed the upstream layout; remove the swap and re-point the labels straight.
3. **`_extract_energy` bi-horaire span** (`bolt.py:453`). The two-`Jour Nuit`-subhead anchor is
...
6. **`_extract_taxes` / `_extract_renewables`** (`bolt.py:483`, `bolt.py:549`). Federal levy and
   ...
   adds markers.
7. **URL construction** (`_document_url` `bolt.py:168`, `_VARIABLE_SUFFIX` `bolt.py:114`). If Bolt
   bumps the variable version past `11`, update the constant; `discover` (`bolt.py:199`) plus the
