...
into `_fetch_pdf_text` (`bolt.py:359`) precisely so `live_check.py` can fetch a card once and parse
...
`probe` (`bolt.py:318`) HEADs the listing page and returns the first present header, preferring
...
`discover` (`bolt.py:337`) GETs the listing HTML and returns the set of `<folder>/<slug>` prefixes
...
`YYYYMM` suffix going back to 2024-01. Variable cards are keyed by version rather than by month, so
past months cannot be addressed at all; `plenty_fix` sits in the `fix` folder and so its URL rolls
...
>
> `_extract_legacy_energy` (`bolt.py:604`) reads the older shape, keyed on which
...
| --- | --- | --- |
| `energy` | `_extract_energy` (`bolt.py:670`) | `FixedRates` or `VariableRates` |
| `injection` | `_extract_injection` (`bolt.py:775`) | flat monthly indicative (`factor`/`base` = `None`); spot-indexed `factor`/`base` for `bolt_dynamic` |
| `publication_label` | `_extract_publication_month` (`bolt.py:760`) | `<Month> <Year>` header. The accent classes span the whole Latin-1 range rather than the accents French month names actually use: Bolt's August 2026 fixed card prints "Aôut 2026" (circumflex on the wrong vowel) and an exact class blanked the label on that typo. The value is display-only and never feeds pricing, so a misspelling is tolerated verbatim rather than corrected or dropped. |
| `taxes.federal_excise`, `energy_contribution`, `region_connection_fee` | `_extract_taxes` (`bolt.py:884`) | 3-column FL/WAL/BX rows, sliced by region |
| `taxes.energy_fund_eur_per_month` | `_extract_energy_fund` (`bolt.py:947`) | Flanders only. The card prints both categories: a domiciled residential connection pays the `résidentiel` row, which is `-` (0); a **professional** contract pays the `non-résidentiel` row (10,07 EUR/month on the August 2026 card). The two rows need separate patterns, since the residential value sits after a U+2028 and the non-residential values are inline on the label line |
| `taxes.{flanders,wallonia,brussels}_renewables` | `_extract_renewables` (`bolt.py:988`) | certificats verts + Flanders WKK; zeroed outside the active region |
...
1. **Monthly platform fee, billed annually.** `_extract_yearly_fee` (`bolt.py:539`) matches
...
would break. `test_variable_uses_current_monthly_not_annual_estimate` (`tests/test_bolt.py:123`)
...
- `variable`: a miss is a layout drift, not a mono contract; variable cards always publish distinct
  ...
  rate. `test_variable_missing_bihourly_rates_fails_loud` (`tests/test_bolt.py:115`) enforces this.
...
- The July 2026 fix cards print a NEGATIVE second column (`Prix mensuel 3,40 -0,43`, the "Exclusif
  ...
  `test_injection_accepts_negative_second_column` (`tests/test_bolt.py:102`) locks this in.

`test_injection_is_flat_monthly_indicative` (`tests/test_bolt.py:86`) checks both fix and variable
...
`test_taxes_split_correctly_per_region` (`tests/test_bolt.py:137`) checks nationwide excise
...
**Flanders (`_extract_flanders_dsos`, `bolt.py:1035`).** Eight Fluvius sub-areas via `_FLANDERS_LABELS`
...
**Wallonia (`_extract_wallonia_dsos`, `bolt.py:1101`).** Five DSOs via `_WALLONIA_LABELS`
...
**Brussels (`_extract_brussels_dsos`, `bolt.py:1187`).** One row, `Sibelga`, with six captured
...
`test_brussels_extracts_sibelga` (`tests/test_bolt.py:208`) checks distribution 0.0996, off-peak
...
- **U+2028 line separators.** Normalized to `\n` at the top of `parse_snapshot` (`bolt.py:469`);
...
1. **DSO row regexes** (`_extract_flanders_dsos` `bolt.py:1035`, `_extract_wallonia_dsos`
   `bolt.py:719`, `_extract_brussels_dsos` `bolt.py:1187`). Column-count changes, a renamed sub-area
...
2. **RESA/REW swap** (`_WALLONIA_LABELS` `bolt.py:1092`). If the ERROR invariant fires, Bolt probably
   fixed the upstream layout; remove the swap and re-point the labels straight.
3. **`_extract_energy` bi-horaire span** (`bolt.py:670`). The two-`Jour Nuit`-subhead anchor is
   fragile; if Bolt reorders the injection/consumption blocks or drops a subhead, the variable path
   raises loud.
4. **`_extract_yearly_fee`** (`bolt.py:539`). A phrasing change away from `€ N / mois` raises.
5. **`_extract_injection`** (`bolt.py:775`). A relabeled `Injection` header or a third
   consumption-side `Prix mensuel` row shifts the anchor; a new second-column sign convention needs
   the `-?` tolerance revisited.
6. **`_extract_taxes` / `_extract_renewables`** (`bolt.py:988`, `bolt.py:988`). Federal levy and
   certificats-verts misses raise; the connection-fee footnote `{0,3}` cap may need widening if Bolt
   adds markers.
7. **URL construction** (`_document_url` `bolt.py:247`, `_resolve_variable_suffix` `bolt.py:262`).
   ...
   `_CARD_URL_RE` (`bolt.py:139`) would stop matching; `discover` (`bolt.py:337`) plus the
