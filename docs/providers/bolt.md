...
`ETag` then `Last-Modified` via `head_freshness_key` (`_pdf.py:416`). Bolt is the reason
...
| `valid_until` | `parse_valid_until` (`_pdf.py:996`) | always `None` in practice; Bolt prints no parseable validity date |
