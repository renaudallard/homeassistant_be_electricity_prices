...
| `version` | `0.11.44` | Manifest version. CI auto-tags and publishes a release when this bumps on `main`. |
...
each have different regional levies and a different set of DSOs. `DSO_CHOICES` (`const.py:106`)
...
A supplier is one registry entry, a `SupplierExtractor` (`providers/base.py:733`). It declares
...
The DSO tariff mode (`CONF_DSO_TARIFF_MODE`, `const.py:203`) is a grid-side billing choice
...
The solar regime (`CONF_SOLAR_REGIME`, `const.py:253`) is independent again: `none` (no panels),
...
3. `async_load_persistent` (`coordinator.py:1066`) loads the last snapshot from `.storage` so an
   offline boot can still serve last-known prices.
4. `async_config_entry_first_refresh` runs `_async_update_data` (`coordinator.py:1167`). It runs
...
5. `EXTRACTOR.fetch(session, contract, region)` returns a `SupplierSnapshot` (`providers/base.py:529`):
...
The coordinator ticks hourly (`UPDATE_INTERVAL_MINUTES` = 60, `const.py:291`). Freshness has
...
- TTL fallback: suppliers with no usable probe (Engie, Luminus, DATS 24, where the only cheap
  ...
  `coordinator.py:224`).
- On-disk cache: the latest snapshot is persisted to `.storage` (`STORAGE_VERSION`, `const.py:293`)
...
1. Add `providers/<supplier>.py` exposing a top-level `EXTRACTOR: SupplierExtractor`
   (`providers/base.py:531`, `SupplierProtocol` at `providers/base.py:770`). It declares the
