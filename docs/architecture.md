...
| `version` | `0.27.0` | Manifest version. CI auto-tags and publishes a release when this bumps on `main`. |
...
| `brugel.py` | Brugel's published Brussels distribution tariffs. One small PDF a year, stating "prix hors TVA", read for a single figure: Sibelga's `Puissance mise a disposition` annual term, which is half its fixed charge and which Bolt's card is alone in not printing. Cached per year for the life of the process, with a short backoff on failure. Returns `None` on any failure, and `resolve_brussels_power_term` then leaves the card billing exactly what it billed before. Sibelga's own site answers 403, so the regulator is the source rather than the operator. Fetched in `_update_body` BEFORE the snapshot is resolved, beside the annual volume and for the same reason: `_resolve_snapshot` reads the cache synchronously and cannot await, so a fetch after it left the tick that retrieved the card billing without the term. |
...
1. Add `providers/<supplier>.py` exposing a top-level `EXTRACTOR: SupplierExtractor`
   (`providers/base.py`, `SupplierProtocol` at `providers/base.py`). It declares the
