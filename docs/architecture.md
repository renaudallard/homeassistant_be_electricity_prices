...
A supplier is one registry entry, a `SupplierExtractor` (`providers/base.py:948`). It declares
...
| `tou` | 3 hour-of-day bands (peak / transition / offpeak) | `TimeOfUseRates` (`providers/base.py:282`) | Weekday schedule shared; `weekend_rule` varies per product. Needs a smart meter. |
| `tou_impact` | Wallonia CWaPE 3-band (pic / medium / eco) | `ImpactRates` (`providers/base.py:330`) | CWaPE hour-of-day bands, every day; needs SMR3 and DSO Impact opt-in. |
...
5. `EXTRACTOR.fetch(session, contract, region)` returns a `SupplierSnapshot` (`providers/base.py:702`):
...
1. Add `providers/<supplier>.py` exposing a top-level `EXTRACTOR: SupplierExtractor`
   (`providers/base.py:531`, `SupplierProtocol` at `providers/base.py:985`). It declares the
