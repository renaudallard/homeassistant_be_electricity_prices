...
| `version` | `0.28.3` | Manifest version. CI auto-tags and publishes a release when this bumps on `main`. |
...
| `contract_periods.py` | The contracts a household held earlier in the year, recorded from the options menu when it changes supplier: the days each covers (`previous_periods`), the card it stands on (`period_card`) and what it cost, priced through `_compute_current_year_cost` closed on the day before the switch. The coordinator prices them once a day in the background and adds them to `current_year_cost`; the compare pages and the backfill split the year the same way. |
| `creg_ev.py` | The flat-rate ceiling for reimbursing a company car charged at home, computed from the CREG's monthly prices the way the SPF Finances does: one CSV, fetched once a quarter and kept in a store for it, never raises. |
