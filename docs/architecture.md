...
| `version` | `0.15.0` | Manifest version. CI auto-tags and publishes a release when this bumps on `main`. |
...
A supplier is one registry entry, a `SupplierExtractor` (`providers/base.py:912`). It declares
...
| `dynamic` | `factor x spot + base` per slot | `DynamicRates` (`providers/base.py:190`) | `quarter_hourly` picks the 15-minute vs hourly billing grid. |
| `tou` | 3 hour-of-day bands (peak / transition / offpeak) | `TimeOfUseRates` (`providers/base.py:256`) | Weekday schedule shared; `weekend_rule` varies per product. Needs a smart meter. |
| `tou_impact` | Wallonia CWaPE 3-band (pic / medium / eco) | `ImpactRates` (`providers/base.py:294`) | CWaPE hour-of-day bands, every day; needs SMR3 and DSO Impact opt-in. |
| `spot_monthly` | Flat monthly rate `factor x monthly_mean(spot) + base` | `SpotMonthlyRates` (`providers/base.py:213`) | energie.be Variabel (Belpex_RLP) and the expert custom monthly-average mode; the coordinator averages the ENTSO-E spot cache per delivery month. Needs an ENTSO-E key. Distinct from `variable`, which reads a rate the card already resolved: this kind is for cards that name the index but publish only a forecast of it. |
...
5. `EXTRACTOR.fetch(session, contract, region)` returns a `SupplierSnapshot` (`providers/base.py:666`):
...
1. Add `providers/<supplier>.py` exposing a top-level `EXTRACTOR: SupplierExtractor`
   (`providers/base.py:531`, `SupplierProtocol` at `providers/base.py:949`). It declares the
