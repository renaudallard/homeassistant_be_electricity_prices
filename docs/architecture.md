...
| `version` | `0.20.14` | Manifest version. CI auto-tags and publishes a release when this bumps on `main`. |
...
5. `EXTRACTOR.fetch(session, contract, region)` returns a `SupplierSnapshot` (`providers/base.py:852`):
