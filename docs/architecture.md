...
| `version` | `0.18.1` | Manifest version. CI auto-tags and publishes a release when this bumps on `main`. |
...
The DSO tariff mode (`CONF_DSO_TARIFF_MODE`, `const.py:302`) is a grid-side billing choice
...
The solar regime (`CONF_SOLAR_REGIME`, `const.py:369`) is independent again: `none` (no panels),
...
3. `async_load_persistent` (`coordinator.py:413`) loads the last snapshot from `.storage` so an
   offline boot can still serve last-known prices.
4. `async_config_entry_first_refresh` runs `_async_update_data` (`coordinator.py:599`). It runs
...
8. The result is packed into `CoordinatorData` (`coordinator.py:174`): the `hourly` table keyed by
...
The coordinator ticks hourly (`UPDATE_INTERVAL_MINUTES` = 60, `const.py:429`). Freshness has
...
- TTL fallback: suppliers with no usable probe (DATS 24, energie.be, Engie, Luminus, where the
  ...
  `coordinator.py:225`).
- On-disk cache: the latest snapshot is persisted to `.storage` (`STORAGE_VERSION`, `const.py:431`)
...
1. Add `providers/<supplier>.py` exposing a top-level `EXTRACTOR: SupplierExtractor`
   (`providers/base.py:531`, `SupplierProtocol` at `providers/base.py:1017`). It declares the
