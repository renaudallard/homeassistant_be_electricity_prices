...
| `_historical_spots` | UTC-hour -> EUR/kWh for past hours, replayed for YTD; kept for a trailing year and persisted |
...
never revisited, and nothing clears it until the prune drops it a year later. That
...
| `projected_year_consumption_kwh` / `projected_year_injection_kwh` | `float \| None` | what the calendar year will have metered by 31 December: 1 January to yesterday as measured, plus last year's same remaining days, else this year's extrapolated on a Synergrid profile the entry already holds (`projected_volume.py`). `None` when neither is available, and injection is `None` without a solar regime | `projected_year_consumption` / `projected_year_injection` sensors |
| `volume_projection_diagnostics` | `dict[str, dict[str, Any]] \| None` | the basis behind each, keyed by side: `volume_basis`, `ytd_kwh`, `remaining_kwh` | attributes of the same sensors, and the diagnostics dump |
...
- **Day + night register pair** (`CONF_DAY_*_KWH` + `CONF_NIGHT_*_KWH`): one recorder delta per day per register, fanned into band slots, on the days BOTH registers report (`_paired_keys`, `energy_meters.py`). A live register writes a row for every day whether or not it moved, so a day only one half holds is missing data rather than a zero: a register that stopped mid-year is billed on the days both cover and `days_seen` says so, and such a day leaves the other side too (the hourly walks and the backfill drop the hours in `MeteredHours.unknown` from both maps, and the per-hour feed-in credit added to the per-day walk only credits the days that walk billed), since crediting the feed-in of a day whose consumption is not billed drives the year down on days nothing was charged, and one that records nothing at all is refused like a partial pair below. Both are judged on the days before today (`_split_today`): today's value is a live reading off the state history, which a register compiling no statistics still has, so with today in the pair such a register looked alive, the year to date was billed on today alone and no register was named for the Repairs card. Today is billed on top when both halves read it and neither has stopped (`_stopped`, relative to the pair, so a statistics stall that holds both back names neither); otherwise today leaves both sides like any day one half did not report, and the hourly walks skip the live top-up (`MeteredHours.today_ok`), which is all or nothing across a side's sensors, since a stopped pair's day drops out at midnight and billing it live meant taking it back the next morning. A pair that does not report the same days is billed off the side's totals sensor instead when one is wired and has recorded at least the days (or hours) the pair can still bill (`_total_stands_in`), on the per-day walk, the hourly walks and the yearly volume alike; a totals sensor recording less, one with no statistics at all included, leaves the pair billed on the days both report (discussion #66), and the register is still named for the Repairs card; the hourly walks top today up off that same sensor (`MeteredHours.sensors`). The hourly walk and the backfill read the pair through `_metered_hourly_kwh` on the same rule, hour by hour. The two SIDES are compared the same way (`_silent_periods`): when consumption's last reported day (or hour) trails injection's by more than `_REGISTER_STOPPED_AFTER_DAYS`, or the other way round, every period after it leaves both sides, today included, so a consumption meter renamed away mid-year no longer bills its later days at zero consumption against the full feed-in credit. A wired consumption side that reported nothing at all while feed-in did is silent for the whole window; a feed-in side that reported nothing is not (panels come later), and nothing is cut before a side's first period. The hourly walk, the backfill and the diagnostics read both sides through `_metered_sides`, and the daily volume read names the silent side's sensors on the `register_pair_incomplete` Repairs card. An entry with no solar regime does not read the injection side at all (`_bills_injection`): nothing is credited or netted there, so a broken or stopped injection meter can neither refuse the year nor cut its days, and raises no card (discussion #66, where one injection register compiling no statistics took a household with no panels to the fees floor). The per-day memo keys on it, since a compare what-if quoting a regime shares the household's meter keys.
...
A `total_increasing` meter is walked through the day's states the way HA's own statistics walk them (`_cycled_total`, from `get_significant_states` over the whole day with a minimal response): a reading below 0.9 x the one before it (`_RESET_BELOW`, the line `reset_detected` draws) starts a new cycle counted from zero, the cycle it ends keeps what it counted, and a smaller fall is a dip that nets against its cycle. Comparing the current reading with midnight's alone read a reset that climbed back to within 10% of the midnight reading as a dip (0 instead of 19) and one that climbed past it as the difference (5 instead of 25); the walk reads each the way the statistics do after midnight (`tests/recorder/test_meter_reset_walk.py`). Taking every fall as a reset billed a 0,01 kWh dip of a 12345,60 register as 12345,59 kWh for the day (`tests/recorder/test_meter_dip.py`). Any other meter reads the current reading minus midnight's, and a fall below it is a reset only when the meter published a `last_reset` later than local midnight. `last_reset` is the signal that generalises: HA's `utility_meter` reports `TOTAL` when `net_consumption` is set and `TOTAL_INCREASING` otherwise, and it cycles either way, so a `net_consumption` helper on a monthly cycle is a falling `TOTAL` meter that genuinely does reset. Gating on the class alone read its rollover as an ordinary fall and returned minus the whole previous cycle as today's kWh (312,4 kWh at midnight, 4,2 after the reset, reported as -308,2). A cycle that already stands below zero after a day of export reads zero, not its signed value. Any other fall reads as **zero**. The picker accepts any `device_class=energy` sensor, so a `utility_meter` with `net_consumption` or a bidirectional meter can be wired, and it goes backwards whenever the site exports more than it draws. Past days drop that fall: `_recorder_deltas` ignores every negative `change`, since a sum-chain restart looks exactly like one (discussion #66). Today drops it too, so the day reads the same before and after midnight; it used to bill the signed delta, and `current_year_cost` stepped up overnight by the day's export. A net register is therefore **not supported**: wire consumption and injection as separate climbing sensors. Reading a fall as a reset instead billed the meter's **whole lifetime total** as a single day (a 12350 kWh register that had exported 4.5 kWh reported 12345.6 kWh, roughly 4300 EUR onto `current_year_cost`), so a sensor publishing no state class reads a fall as zero too.
...
Serialization is `_snapshot_to_dict` (`snapshot_codec.py`) and `_snapshot_from_dict` (`snapshot_codec.py`), which stamp and check `_SNAPSHOT_SCHEMA_VERSION` as described in section 2.3. What is persisted is the card **as parsed**, not as priced: `_set_snapshot` keeps both, and the per-entry VAT and excise-band resolution is re-applied on load. Historical spots (hours and quarters alike, with the complete-day and quarter-grid day sets) are kept for a trailing year: 365 closed days plus today and tomorrow, anchored on local midnight (`_prune_historical_spots`, `coordinator_spots.py`). Fetching still starts at the year-to-date window (`_ensure_historical_spots`); the prune only decides what is kept, so the persisted blob holds every day what it used to reach on 31 December, about 0,37 MB of hours and 1,06 MB with the quarters a floored feed-in keeps. It keeps a trailing year rather than cutting at 1 January because a static card's per-slot feed-in is credited on the past year's closed days in the projection and on the compare page (`_credit_year`, `compare_inputs.py`), and a January cut emptied that window every new year. Keeping December beside January bills nothing extra: every walk reads an hour by its own key or a month by its local (year, month) (`test_no_december_hour_reaches_a_january_bill`, `test_a_january_backfill_prices_no_december_hour`). The prune waits while a backfill runs (`_spot_prune_holds`, set by `backfill_range`): the backfill reads the same dict between turns of the loop, and a window older than the trailing year needs exactly the hours the prune drops. On entry removal, `async_remove_entry` (`__init__.py`) deletes every Repairs issue id and removes the Store file so nothing outlives the entry; `test_repair_issue_kinds_match_the_declared_strings` pins that list against `strings.json` so a newly added issue cannot skip it.
