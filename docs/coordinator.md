...
1. **Dynamic** (`coordinator.py:3971`): replay `_historical_spots` through `_ytd_hourly_energy` (`coordinator.py:3980`), billing each recorded hour at its actual `factor*spot+base`. Empty spot cache -> fees only.
...
3. **Static per-day** (`coordinator.py:4033`): `_resolve_daily_kwh` (`coordinator.py:3552`) gives per-day `(day_cons, night_cons, day_inj, night_inj)`, each day billed against `static_breakdown` for its month.
...
- **Single totals sensor** (`CONF_CONSUMPTION_KWH` / `CONF_INJECTION_KWH`): for mono meters the total goes to the day slot and the math sums it; for bi/dynamic meters `_recorder_daily_band_ratio` (`coordinator.py:3515`) recovers the day/night split from hourly recorder statistics binned on `is_offpeak`, defaulting to a time-weighted `_default_band_ratio_for` (`coordinator.py:3665`) for days with no accumulation so a flat Sunday isn't billed all-peak.
...
- YTD credit (`_ytd_spot_injection_credit`, `coordinator.py:4150`): an isolated term that replays hourly spots for the injection side only, subtracted from the bill in both the hourly path (`coordinator.py:4150`) and the static per-day path (`coordinator.py:4150`). Its own guard (`coordinator.py:4150`) fires only for the exact shape (`factor`/`base` set, `current is None`, spots cached, an injection sensor wired).
