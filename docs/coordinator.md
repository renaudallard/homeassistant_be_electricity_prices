...
There is a **second**, finer version inside the serialized snapshot: `_SNAPSHOT_SCHEMA_VERSION`, currently `22` (`snapshot_store.py:668`). `_snapshot_to_dict` stamps it (`snapshot_store.py:671`); `_snapshot_from_dict` raises `ValueError` when a loaded blob's `_schema_version` is below it (`snapshot_store.py:706`), which `async_load_persistent` catches and treats as "discard and re-fetch".
