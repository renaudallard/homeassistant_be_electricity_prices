...
There is a **second**, finer version inside the serialized snapshot: `_SNAPSHOT_SCHEMA_VERSION`, currently `15` (`coordinator.py:4143`). `_snapshot_to_dict` stamps it (`coordinator.py:4152`); `_snapshot_from_dict` raises `ValueError` when a loaded blob's `_schema_version` is below it (`coordinator.py:4168`), which `async_load_persistent` catches and treats as "discard and re-fetch".
...
- YTD credit (`_ytd_spot_injection_credit`, `coordinator.py:3718`): an isolated term that replays hourly spots for the injection side only, subtracted from the bill in both the hourly path (`coordinator.py:3986`) and the static per-day path (`coordinator.py:4088`). Its own guard (`coordinator.py:3741`) fires only for the exact shape (`factor`/`base` set, `current is None`, spots cached, an injection sensor wired).
