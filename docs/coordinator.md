...
`_compute_current_year_cost` (`ytd_cost.py:586`) computes the running bill from Jan 1 of the local year to today. It bills each past day at the tariff of the month that day belongs to, using an archived snapshot when the supplier exposes `fetch_for_month` (`providers/base.py:979`) and the current snapshot as a proxy otherwise (`_snapshot_for_month`, `snapshot_store.py:330`). When a contract start date is set it routes every past month through `_effective_snapshot_for_month` (`cohort.py:436`) instead, which splices the signing cohort's energy leg onto each delivery month's overlays, and dispatches on that cohort's effective energy kind so a re-priced variable contract takes the monthly-mean path. The whole year is recomputed from scratch each tick by design (`ytd_cost.py:586`): prior days are not immutable (a late ENTSO-E fill or a backfill correction changes a past rate), and the full replay is cheap pure arithmetic.
