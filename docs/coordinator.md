...
- YTD credit (`_ytd_spot_injection_credit`, `ytd_cost.py:484`): an isolated term that replays hourly spots for the injection side only, subtracted from the bill in both the hourly path (`ytd_cost.py:484`) and the static per-day path (`ytd_cost.py:484`). Its own guard (`ytd_cost.py:484`) fires only for the exact shape (`factor`/`base` set, `current is None`, spots cached, an injection sensor wired).
