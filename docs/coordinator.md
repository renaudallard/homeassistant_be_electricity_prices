...
There is a **second**, finer version inside the serialized snapshot: `_SNAPSHOT_SCHEMA_VERSION`, currently `18` (`coordinator.py:4573`). `_snapshot_to_dict` stamps it (`coordinator.py:4576`); `_snapshot_from_dict` raises `ValueError` when a loaded blob's `_schema_version` is below it (`coordinator.py:4611`), which `async_load_persistent` catches and treats as "discard and re-fetch".
...
- YTD credit (`_ytd_spot_injection_credit`, `coordinator.py:4127`): an isolated term that replays hourly spots for the injection side only, subtracted from the bill in both the hourly path (`coordinator.py:4127`) and the static per-day path (`coordinator.py:4127`). Its own guard (`coordinator.py:4127`) fires only for the exact shape (`factor`/`base` set, `current is None`, spots cached, an injection sensor wired).
