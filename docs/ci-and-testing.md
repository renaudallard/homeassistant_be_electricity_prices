...
- `_validate_energy` (`scripts/live_check.py:2884`) dispatches on the energy dataclass type and
...
`_drift_warnings` (`scripts/live_check.py:3449`) compares each supplier's summed fetch time and
...
`_BYTES_BUDGET_OVERRIDES` (`scripts/live_check.py:3326`) for the known-large catalogues (Bolt,
...
against the set `_failed_suppliers` reads off the check labels, `scripts/live_check.py:3436`). The
