...
`_check_card_freshness` (`scripts/live_check.py:1616`) asks a question no other check here asks:
...
The mechanism is a deliberate asymmetry: `_expect_newest_card` (`scripts/live_check.py:1267`) scans
...
`_expect_energy_contribution` (`scripts/live_check.py:592`) instead, which accepts
...
`_validate_snapshot` (`scripts/live_check.py:2316`) runs two gates:

- `_validate_energy` (`scripts/live_check.py:2377`) dispatches on the energy dataclass type and
...
- `_validate_injection` (`scripts/live_check.py:1879`) gates that the feed-in credit parsed and
...
`_drift_warnings` (`scripts/live_check.py:2809`) compares each supplier's summed fetch time and
...
`_BYTES_BUDGET_OVERRIDES` (`scripts/live_check.py:2696`) for the known-large catalogues (Bolt,
...
against the set `_failed_suppliers` reads off the check labels, `scripts/live_check.py:2796`). The
...
repo root (`scripts/live_check.py:2377`), each a side-channel the workflow reads to file a separate
