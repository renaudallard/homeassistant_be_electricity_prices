...
`_validate_snapshot` (`scripts/live_check.py:2561`) runs two gates:

- `_validate_energy` (`scripts/live_check.py:2622`) dispatches on the energy dataclass type and
...
`_drift_warnings` (`scripts/live_check.py:3149`) compares each supplier's summed fetch time and
...
`_BYTES_BUDGET_OVERRIDES` (`scripts/live_check.py:3027`) for the known-large catalogues (Bolt,
...
against the set `_failed_suppliers` reads off the check labels, `scripts/live_check.py:3136`). The
