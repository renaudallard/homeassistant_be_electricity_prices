...
`_validate_snapshot` (`scripts/live_check.py:2303`) runs two gates:

- `_validate_energy` (`scripts/live_check.py:2364`) dispatches on the energy dataclass type and
...
`_drift_warnings` (`scripts/live_check.py:2796`) compares each supplier's summed fetch time and
...
`_BYTES_BUDGET_OVERRIDES` (`scripts/live_check.py:2683`) for the known-large catalogues (Bolt,
...
against the set `_failed_suppliers` reads off the check labels, `scripts/live_check.py:2783`). The
...
repo root (`scripts/live_check.py:2364`), each a side-channel the workflow reads to file a separate
