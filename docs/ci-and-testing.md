...
`_validate_snapshot` (`scripts/live_check.py:2342`) runs two gates:

- `_validate_energy` (`scripts/live_check.py:2403`) dispatches on the energy dataclass type and
...
`_drift_warnings` (`scripts/live_check.py:2835`) compares each supplier's summed fetch time and
...
`_BYTES_BUDGET_OVERRIDES` (`scripts/live_check.py:2722`) for the known-large catalogues (Bolt,
...
against the set `_failed_suppliers` reads off the check labels, `scripts/live_check.py:2822`). The
...
repo root (`scripts/live_check.py:2403`), each a side-channel the workflow reads to file a separate
...
(`scripts/live_check.py:2342`) is the single definition of what gates CI. The classification reads
