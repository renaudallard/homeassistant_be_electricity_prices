...
`_validate_snapshot` (`scripts/live_check.py:2180`) runs two gates:

- `_validate_energy` (`scripts/live_check.py:2241`) dispatches on the energy dataclass type and
...
- `_validate_injection` (`scripts/live_check.py:1853`) gates that the feed-in credit parsed and
  ...
  expectations live in `_INJECTION_SHAPE` (`scripts/live_check.py:1937`); the DATS 24 check passes
...
`_drift_warnings` (`scripts/live_check.py:2673`) compares each supplier's summed fetch time and
...
`_BYTES_BUDGET_OVERRIDES` (`scripts/live_check.py:2560`) for the known-large catalogues (Bolt,
...
against the set `_failed_suppliers` reads off the check labels, `scripts/live_check.py:2660`). The
