...
`_expect_energy_contribution` (`scripts/live_check.py:568`) instead, which accepts
...
`_validate_snapshot` (`scripts/live_check.py:2164`) runs two gates:

- `_validate_energy` (`scripts/live_check.py:2225`) dispatches on the energy dataclass type and
...
- `_validate_injection` (`scripts/live_check.py:1853`) gates that the feed-in credit parsed and
  ...
  expectations live in `_INJECTION_SHAPE` (`scripts/live_check.py:1921`); the DATS 24 check passes
...
`_drift_warnings` (`scripts/live_check.py:2657`) compares each supplier's summed fetch time and
...
`_BYTES_BUDGET_OVERRIDES` (`scripts/live_check.py:2544`) for the known-large catalogues (Bolt,
...
against the set `_failed_suppliers` reads off the check labels, `scripts/live_check.py:2644`). The
