...
`_validate_snapshot` (`scripts/live_check.py:2275`) runs two gates:

- `_validate_energy` (`scripts/live_check.py:2336`) dispatches on the energy dataclass type and
...
`_drift_warnings` (`scripts/live_check.py:2768`) compares each supplier's summed fetch time and
...
`_BYTES_BUDGET_OVERRIDES` (`scripts/live_check.py:2655`) for the known-large catalogues (Bolt,
...
against the set `_failed_suppliers` reads off the check labels, `scripts/live_check.py:2755`). The
