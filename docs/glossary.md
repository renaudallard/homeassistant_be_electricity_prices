...
| Belpex / eSpot_15 / EPEX / EPEX DA | Names Belgian suppliers print for the wholesale day-ahead spot their dynamic formula multiplies. The integration always sources this curve from ENTSO-E, not the branded feed; the labels are only documentation of what a card references. | `providers/base.py:148`, README.md:124 |
...
| energy fund | Flemish Energiefonds, billed as EUR/month (not per kWh); 0 outside Flanders and 0 for domiciled Flemish customers. | `providers/base.py:470`, README.md:147 |
...
| probe (`SnapshotProbe`) | A cheap freshness check (HEAD, ETag, Last-Modified, or resolved PDF URL) run hourly; an unchanged return means the snapshot is still valid, so the full fetch is skipped. `None` means no probe and the time-based TTL takes over. | `providers/base.py:517`, README.md:102 |
