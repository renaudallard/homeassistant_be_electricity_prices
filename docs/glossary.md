...
| apply_vat / CONF_INCLUDE_VAT | Resolves an ex-VAT snapshot for one config entry: bakes the fixed and annual fees by the chosen factor and sets the per-kWh `vat_rate` with it. The Flemish energy fund is excluded: it is levied VAT-free and is billed exactly as the card prints it. Identity on a VAT-inclusive card. Called per entry (never before the cross-entry shared cache) so a VAT-deductible business and a private customer can share a tuple. | `providers/base.py:556`, `coordinator.py:550` |
