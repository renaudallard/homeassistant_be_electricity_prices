...
| TaxOverlay | Federal + regional levy dataclass: excise, energy contribution, three regional renewables, connection fee, energy fund (per month), and `vat_rate`. | `providers/base.py:454` |
| apply_vat / CONF_INCLUDE_VAT | Resolves an ex-VAT snapshot for one config entry: bakes the fixed and annual fees by the chosen factor and sets the per-kWh `vat_rate` with it. Identity on a VAT-inclusive card. Called per entry (never before the cross-entry shared cache) so a VAT-deductible business and a private customer can share a tuple. | `providers/base.py:548`, `coordinator.py:1682` |
