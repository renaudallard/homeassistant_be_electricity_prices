...
| Brussels (`REGION_BRUSSELS`) | One of the three Belgian regions. Single DSO (Sibelga), Brugel regulator, `brussels_renewables` levy. | `const.py:41`, `providers/base.py:713` |
...
| DSO overlay | The `DsoOverlay` for the one sub-area a snapshot's `dsos` dict is keyed by the user's `CONF_DSO`; the coordinator picks it and feeds `compute_breakdown`. | `providers/base.py:752`, `providers/base.py:752` |
...
| fetch_for_month | The `ArchivedSnapshotFetcher` slot on a `SupplierExtractor`; enables time-correct historical billing. `None` means the coordinator uses the current snapshot as a proxy. | `providers/base.py:988` |
...
| Flanders (`REGION_FLANDERS`) | One of the three regions. Eight Fluvius sub-areas, VREG regulator, the capacity tariff, `flanders_renewables` levy, Energiefonds. | `const.py:39`, `providers/base.py:711` |
...
| probe (`SnapshotProbe`) | A cheap freshness check (HEAD, ETag, Last-Modified, or resolved PDF URL) run hourly; an unchanged return means the snapshot is still valid, so the full fetch is skipped. `None` means no probe and the time-based TTL takes over. | `providers/base.py:974`, README.md:234 |
| prosumer / prosumer fee | Compensation-regime solar cost in EUR per kVA of inverter capacity per year. The DSO publishes `prosumer_eur_per_kva_year` (Wallonia only, until 2030); some suppliers add `supplier_prosumer_eur_per_kva_year` on top (already TVAC). | `providers/base.py:764`, `providers/base.py:764` |
...
| region connection fee | Regional per-kWh connection levy in the tax overlay (`region_connection_fee`). | `providers/base.py:714` |
| regional renewables | Region-specific green-energy / cogen surcharges: `flanders_renewables`, `wallonia_renewables`, `brussels_renewables` (EUR/kWh). The pricing engine picks the one matching the entry's region; extractors leave the others at 0. | `providers/base.py:713`, `providers/base.py:713` |
...
| snapshot (`SupplierSnapshot`) | Everything extracted from one supplier tariff card for one `(supplier, contract)`: energy formula, per-DSO overlays, tax overlay, optional injection, source URL, and `valid_until`. | `providers/base.py:772` |
| SnapshotFetcher | The required per-supplier callable that fetches and returns a `SupplierSnapshot` from a live source. | `providers/base.py:742` |
...
| SupplierExtractor | The registry entry a provider module exposes as top-level `EXTRACTOR`: id, label, contracts, `fetch`, optional `probe` and `fetch_for_month`. | `providers/base.py:1004`, `providers/base.py:1004` |
...
| TSO (transmission system operator) | Elia, the operator of the Belgian high-voltage grid whose transmission charge is the `transport` term. Distinct from the DSO (local distribution). | `providers/base.py:535`, README.md:268 |
...
- `vat_rate = 0.0` does not mean "no VAT": it means the prices are already
  ...
  `vat_rate` (`providers/base.py:727`).
