...
| capacity tariff | Flanders-only network charge billed on the household's monthly peak power: `peak_kw * capacity_eur_per_kw_year / 12`. The billed peak is `max(measured_peak, VREG_CAPACITY_FLOOR_KW)`. | `const.py:238`, `providers/base.py:278`, README.md:155 |
...
| VREG_CAPACITY_FLOOR_KW | Regulated minimum monthly peak (2.5 kW, illustrative, from the source constant) that Flanders bills against: billed peak is `max(measured, 2.5)`. Set by VREG at the January 2023 capacity-tariff introduction. | `const.py:242` |
