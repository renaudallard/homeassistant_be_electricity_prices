...
(`providers/_rates.py`) chosen by the contract's `kind`. All rate dataclasses
...
| `kind` | `TariffKind` | required | One of `"fixed"`, `"variable"`, `"dynamic"`, `"tou"`, `"tou_impact"`, `"spot_monthly"` (`providers/_rates.py`). Selects which `EnergyRates` variant the snapshot carries. |
...
| `quarter_hourly_option` | `bool` | `False` | `True` when the supplier sells this product on both settlements, so the flow shows a **settlement** step and `resolve_settlement_grid` applies the answer. Two cases, and they differ in how far the answer reaches. Frank Energie's five Dynamisch tiers are `dynamic` either way and only `DynamicRates.quarter_hourly` moves. Bolt's four variable cards, in both segments, settle the same printed formula against the RLP-weighted month or per quarter-hour, which are different `EnergyRates` kinds, so `effective_kind` moves the CONTRACT KIND with the answer and everything the flow decides before it has fetched a card (meter list, mandatory ENTSO-E key, signing-rate boxes, ranking cell) reads it through that. Not for a supplier that sells the two grids as two products (Energy Knights Agilior against Agilis), nor for one whose card fixes the grid and whose parser sets `quarter_hourly` itself. |
| `direct_debit_discount` | `bool` | `False` | `True` when this product's card prices a direct-debit payer differently, so the flow shows a **direct_debit** step and `resolve_direct_debit` takes the card's reduction off the standing charge. Brusol's Groene stroom is the only one today: 250 EUR/yr, 230 on domiciliëring. The euros are read off the card like every other figure, never written here; this flag only decides whether the household is asked. Same registry-versus-parser agreement as the two flags above, and the same failure if they disagree: unset, no step asks, nothing is stored and a parsed reduction is billed to nobody. |
...
`weekend_rule` (`WeekendRule`, `providers/_rates.py`) selects the weekend
schedule:
...
| Field | Type | Default | Meaning |
| --- | --- | --- | --- |
| `current` | `float \| None` | `None` | Supplier's monthly indicative price, used when no live spot is available. An illustrative value that appears in the source comment is Eneco Power Fix's "Maandprijs" of 4.76 c/kWh (`providers/_rates.py`; illustrative only). |
...
| `index_realised` | `float \| None` | `None` | The value the credit's index settled at for the card's OWN month, in EUR/kWh, once the supplier published it. EBEM names the month just closed on the next card ("de SPP0 vorige maand bedroeg 79,11") and Trevion names the month outright; `fetch_for_month` settles the archived month on it, `current` is rebuilt from the card's own coefficients, and the engine bills that figure instead of computing a weighted mean. The `VariableRates` field of the same name does this for the consumption leg; the two indices differ and settle independently. |
...
| `direct_debit_discount_eur` | `float \| None` | `None` | What the card takes off the YEARLY STANDING CHARGE for a household paying by direct debit, or `None` where it offers none, which is every card but Brusol's Groene stroom. Held as the REDUCTION rather than the reduced fee, so it cannot silently disagree with `yearly_fixed_fee` beside it and the extractor can check its reading against the total the same sentence prints. Whether this household pays that way is not on the card: `resolve_direct_debit` applies the config flow's answer and clears this field as it goes, so no later reader can apply it twice. |
...
| `regional_tax_overlay` | `regional_tax_overlay(text, *, supplier, region, excise, renewables, contribution=None, fund=None) -> TaxOverlay` | The tax block of a single-region, VAT-inclusive card. `region` picks which of the three regional renewables fields the levy lands in and the other two stay zero, which is what such a card actually says; an unknown region raises rather than zeroing all three. Callers pass their own compiled anchors; this holds the POLICY, which is what drifted: excise mandatory (patterns tried in order, so a flat row wins over the tiered one being phased out), renewables mandatory and all summed, contribution optional (absent = the levy abolished on 2026-08-01, not a layout drift), fund optional and in EUR/month so unscaled. Used by energie.be, Energy Knights, EnergyVision (for Flanders and for Brussels) and Frank. |
...
- `_storage_error_code` (`_pdf.py`) splits one case back out of that
  ...
  Azure Blob and S3 both shape it as a root `<Error>` carrying a `<Code>`; the
  match is anchored at the start of the payload so an HTML page that merely
  mentions such a block is not caught. A hit is wrapped as
  `storage error fetching <url>: <Code>` and classifies as transient, because no
  code change can fix it. Luminus switched anonymous access off on the storage
  account behind `api-next/get-pricelist` on 2026-08-10 and every card came back
