...
| `quarter_hourly_option` | `bool` | `False` | `True` when the supplier lets the customer settle this product on the 15-minute grid instead of the hourly one its card prints, so the meter step shows a **Bill per quarter-hour** box and `resolve_settlement_grid` applies the answer. Frank Energie's five Dynamisch tiers are the only case: the app switches the account monthly and the coefficients do not change, so nothing in the PDF says which grid a household is on. Not for a supplier that sells the two grids as two products (Energy Knights Agilior against Agilis), nor for one whose card fixes the grid and whose parser sets `quarter_hourly` itself. |
