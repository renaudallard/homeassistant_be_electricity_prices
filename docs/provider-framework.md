...
(`providers/base.py:609`, `SupplierProtocol`). The module's job is to turn the
...
Defined at `providers/base.py:558`. A cheap freshness key. The coordinator calls
...
`weekend_rule` (`WeekendRule`, `providers/base.py:225`) selects the weekend
schedule:
