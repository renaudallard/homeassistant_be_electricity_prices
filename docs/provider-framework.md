...
`weekend_rule` (`WeekendRule`, `providers/base.py:196`) selects the weekend
schedule:
...
| Field | Type | Default | Meaning |
| --- | --- | --- | --- |
| `current` | `float \| None` | `None` | Supplier's monthly indicative price, used when no live spot is available. An illustrative value that appears in the source comment is Eneco Power Fix's "Maandprijs" of 4.76 c/kWh (`providers/base.py:123`; illustrative only). |
...
| `fetch_text` | `fetch_text(session, url, *, timeout=20) -> str` | GET an HTML listing / index / plain-text source. Raises `ExtractorError` on non-2xx or network error (`_pdf.py:441`). |
...
- `_fetch_validated_pdf_bytes` (`_pdf.py:140`) is shared by the three
...
- `error_text` (`_pdf.py:86`) renders the wrapped exception as `str(err)` or, when
...
- `_read_pdf_bytes` (`_pdf.py:109`) refuses a body whose declared Content-Length
