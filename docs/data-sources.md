...
| --- | --- | --- |
| `backfill_range` (`backfill.py:806`) | `backfill_statistics` service | Always runs over the requested range; `clear=True` deletes the series first. |
| `backfill_if_missing` (`backfill.py:969`) | fire-and-forget task from `async_setup_entry` | Probes the recorder at the Jan 1 anchor and runs only when nothing exists. |
...
unique id `f"{entry_id}_{key}"` via `_stat_id` (`backfill.py:136`). When the
...
injected each past hour. `_backfill_cost_sensor` (`backfill.py:508`) recovers
...
`_hour_iter` grid (`backfill.py:149`).
...
`_ensure_dynamic_spots` (`backfill.py:278`) reuses the coordinator's
...
`_seed_short_term_sum` (`backfill.py:755`) writes one short-term row at the last
...
`_backfill_cost_sensor` runs one running total per hour (`backfill.py:508`)
...
is series-scoped, not range-scoped: `_clear_all` (`backfill.py:256`) deletes the
