...
constructs a fresh `EntsoeClient` per call (`coordinator.py:1812`,
`coordinator.py:1872`). Two paths use it:

- Live curve, `_fetch_spot_prices` (`coordinator.py:1854`). Windows the request
  ...
  (`coordinator.py:1890`, `coordinator.py:1900`). `quarter_hourly` is derived from
  the loaded snapshot's energy kind (`coordinator.py:1888`).
- Historical backfill, `_ensure_historical_spots` (`coordinator.py:1737`).
  ...
  of its 24 hours are cached (`coordinator.py:1801`), tolerating both the
  ...
  (`coordinator.py:1818`). A negative cache, `_short_spot_days` with a TTL, marks
  stable past days that stay short after a fetch so subsequent ticks skip them
  (`coordinator.py:1847`); today and yesterday are always re-fetched.
