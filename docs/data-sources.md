...
| --- | --- | --- |
| `backfill_range` (`backfill.py:1037`) | `backfill_statistics` service | Always runs over the requested range; `clear=True` deletes the series first. |
| `backfill_if_missing` (`backfill.py:1206`) | fire-and-forget task from `async_setup_entry` | Probes the recorder at the Jan 1 anchor and runs only when nothing exists. |
...
injected each past hour. `_backfill_cost_sensor` (`backfill.py:681`) recovers
...
`_seed_short_term_sum` (`backfill.py:980`) writes one short-term row at the last
...
`_backfill_cost_sensor` runs one running total per hour (`backfill.py:681`)
