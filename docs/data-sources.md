...
| --- | --- | --- |
| `backfill_range` (`backfill.py:805`) | `backfill_statistics` service | Always runs over the requested range; `clear=True` deletes the series first. |
| `backfill_if_missing` (`backfill.py:964`) | fire-and-forget task from `async_setup_entry` | Probes the recorder at the Jan 1 anchor and runs only when nothing exists. |
...
unique id `f"{entry_id}_{key}"` via `_stat_id` (`backfill.py:137`). When the
...
(`_PRICE_SENSOR_KEYS`, `backfill.py:119`) is maintained by hand in lockstep with
...
injected each past hour. `_backfill_cost_sensor` (`backfill.py:507`) recovers
...
`_hour_iter` grid (`backfill.py:150`).
...
`_ensure_dynamic_spots` (`backfill.py:277`) reuses the coordinator's
...
`_seed_short_term_sum` (`backfill.py:754`) writes one short-term row at the last
...
`_backfill_cost_sensor` runs one running total per hour (`backfill.py:507`)
...
is series-scoped, not range-scoped: `_clear_all` (`backfill.py:255`) deletes the
