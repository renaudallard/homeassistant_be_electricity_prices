...
| --- | --- | --- |
| `backfill_range` (`backfill.py:797`) | `backfill_statistics` service | Always runs over the requested range; `clear=True` deletes the series first. |
| `backfill_if_missing` (`backfill.py:956`) | fire-and-forget task from `async_setup_entry` | Probes the recorder at the Jan 1 anchor and runs only when nothing exists. |
...
unique id `f"{entry_id}_{key}"` via `_stat_id` (`backfill.py:128`). When the
...
(`_PRICE_SENSOR_KEYS`, `backfill.py:118`) is maintained by hand in lockstep with
...
injected each past hour. `_backfill_cost_sensor` (`backfill.py:498`) recovers
...
`_hour_iter` grid (`backfill.py:141`).
...
`_ensure_dynamic_spots` (`backfill.py:268`) reuses the coordinator's
...
`_seed_short_term_sum` (`backfill.py:746`) writes one short-term row at the last
...
`_backfill_cost_sensor` runs one running total per hour (`backfill.py:498`)
...
is series-scoped, not range-scoped: `_clear_all` (`backfill.py:246`) deletes the
