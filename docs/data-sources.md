...
unique id `f"{entry_id}_{key}"` via `_stat_id` (`backfill.py:151`). When the
...
(`_PRICE_SENSOR_KEYS`, `backfill.py:141`) is maintained by hand in lockstep with
...
`_hour_iter` grid (`backfill.py:164`).

### Billing each past hour at its historical rate

Both backfill passes cache one `SupplierSnapshot` per month via
`_month_snapshot_cache` (`cohort.py:430`, called at `backfill.py:431`), so a 365-day window
...
The injection credit reuses `_historical_injection_rate` (`injection.py:337`,
...
`_ensure_dynamic_spots` (`backfill.py:291`) reuses the coordinator's
...
is series-scoped, not range-scoped: `_clear_all` (`backfill.py:269`) deletes the
...
- `recorder` provides every statistics primitive backfill uses:
  ...
  `clear_statistics` for the destructive path (`backfill.py:269`), and the
