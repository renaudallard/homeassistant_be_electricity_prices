...
| --- | --- | --- |
| `ENTSOE_BASE_URL` | `https://web-api.tp.entsoe.eu/api` | `const.py:205` |
| `ENTSOE_BE_DOMAIN` | `10YBE----------2` (BE bidding zone EIC) | `const.py:206` |

The client is constructed with the user's ENTSO-E API key and Home Assistant's
shared `aiohttp` session (`api.py:68`). The key is passed on every request as
the `securityToken` query parameter (`api.py:106`). There is no separate login
step; ENTSO-E authenticates per request.

### The query it builds

`fetch_day_ahead(period_start, period_end, *, quarter_hourly=False)` builds a
GET against `ENTSOE_BASE_URL` with these parameters (`api.py:100`):
...
`_fmt` (`api.py:299`) converts the caller's datetimes to UTC and formats them as
`%Y%m%d%H%M`, the compact stamp ENTSO-E expects. The request carries a 30-second
total timeout (`api.py:110`).
...
Two exception types are defined (`api.py:57`, `api.py:61`):
...
- `TimeoutError` is caught explicitly alongside `aiohttp.ClientError`
  (`api.py:120`). On Python 3.11+ the `aiohttp.ClientTimeout` fires
...
- Credential redaction. `aiohttp` client errors stringify with the full request
  ...
  `snapshot_stale` Repairs card, the HA log). `_redact` (`api.py:72`) replaces
  the literal key and applies `_TOKEN_RE` (`api.py:54`) to scrub the token from
...
it as `EntsoeError` (`api.py:154`) so a hostile payload surfaces as a categorised
...
(`api.py:132`). The A44 document is small today (roughly 100 KB hourly, larger
...
(`api.py:169`) and raises `EntsoeAuthError` with a best-effort reason extracted
from the document's `Reason` block by `_ack_reason` (`api.py:282`).
...
15-minute MTU go-live (2025-10-01; see the note at `const.py:208`). The parser
handles three resolutions via `_resolution_to_timedelta` (`api.py:321`):
...
publication may still be usable (`api.py:202`).

Points are accumulated into `(sum, count)` buckets keyed first by the resolution
step in seconds, then by slot key (`api.py:185`). Bucketing per resolution is
...
points" into one unweighted mean would mis-price every hour (`api.py:178`).
Within a single resolution, duplicate points still average, which is the correct
handling of a corrected re-publication.

The slot key depends on the caller's `quarter_hourly` flag (`api.py:257`):
...
(`api.py:269`): in hourly mode it iterates resolutions largest-step-first and
...
(`api.py:210`, `api.py:250`) so the caller never sees a gap it would interpolate
...
sub-hour slot, with the explicit positions used as a floor (`api.py:230`).

### Timezone handling

`_parse_iso_utc` (`api.py:303`) parses each `timeInterval` boundary with
...
UTC rather than the HA host's local time (`api.py:316`). Everything in this
...
timestamp is wrapped as `EntsoeError` (`api.py:310`) so the coordinator keeps
...
1000 to get EUR/kWh at the point where it is read (`api.py:227`). The return
shape is:
...
("malformed point in document", `api.py:225`).
...
constructs a fresh `EntsoeClient` per call (`coordinator.py:1296`,
`coordinator.py:1356`). Two paths use it:

- Live curve, `_fetch_spot_prices` (`coordinator.py:1338`). Windows the request
  ...
  (`coordinator.py:1349`, `coordinator.py:1383`). `quarter_hourly` is derived from
  the loaded snapshot's energy kind (`coordinator.py:1372`).
- Historical backfill, `_ensure_historical_spots` (`coordinator.py:1232`).
  ...
  of its 24 hours are cached (`coordinator.py:1274`), tolerating both the
  ...
  (`coordinator.py:1302`). A negative cache, `_short_spot_days` with a TTL, marks
  stable past days that stay short after a fetch so subsequent ticks skip them
  (`coordinator.py:1331`); today and yesterday are always re-fetched.

`_historical_spots` is persisted to HA storage (`STORAGE_VERSION = 2`,
`const.py:221`) and reloaded on restart (`coordinator.py:661`). The reload is
...
rather than being re-saved indefinitely (`coordinator.py:655`). Persisted keys
are ISO strings; a naive one is treated as UTC on load (`coordinator.py:670`).
...
| --- | --- | --- |
| `backfill_range` (`backfill.py:646`) | `backfill_statistics` service | Always runs over the requested range; `clear=True` deletes the series first. |
| `backfill_if_missing` (`backfill.py:746`) | fire-and-forget task from `async_setup_entry` | Probes the recorder at the Jan 1 anchor and runs only when nothing exists. |
...
(service handler `_async_backfill_service` at `__init__.py:509`, one-shot
scheduling at `__init__.py:166`). The service handler validates that a snapshot
is loaded and raises a localized `ServiceValidationError` otherwise, matching the
window services (`__init__.py:522`).
...
unique id `f"{entry_id}_{key}"` via `_stat_id` (`backfill.py:119`). When the
...
(`_PRICE_SENSOR_KEYS`, `backfill.py:101`) is maintained by hand in lockstep with
...
injected each past hour. `_backfill_cost_sensor` (`backfill.py:424`) recovers
...
UTC-hour totals (`backfill.py:500`). The recorder helpers treat their date
...
`_hour_iter` grid (`backfill.py:492`).
...
(`backfill.py:376`, `backfill.py:544`). A dynamic supplier with no spot for the
hour is skipped, because `factor * spot + base` needs both terms
(`backfill.py:373`). `KeyError` / `ValueError` (a missing DSO row for an archived
month, or a non-static rate kind reaching the static path) skips just that hour
rather than tearing the whole backfill down (`backfill.py:377`).

The injection credit reuses `_historical_injection_rate` (`backfill.py:393`,
`backfill.py:556`), the same coordinator helper the live YTD path uses, so a
monthly-indexed, spot-indexed, or fixed injection rate is resolved identically in
both places.

### Spot provisioning for backfill

`_ensure_dynamic_spots` (`backfill.py:254`) reuses the coordinator's
...
`backfill.py:286`) to match the live coordinator's local-day anchoring.
...
- The cost series must stay within a single calendar year. `backfill_range`
  ...
  `backfill.py:683`) and never crosses a year boundary; a multi-year request only
  backfills the end year's cost (`backfill.py:672`). The price `mean` sensors are
  unaffected and keep the full requested window.
- The accumulator starts from Jan 1 but only emits rows on or after
  `emit_from` (`backfill.py:627`), so a mid-year `start` still carries the correct
  year-to-date sum instead of restarting from zero and clashing with the existing
  head of the series.

`_backfill_cost_sensor` runs one running total per hour (`backfill.py:531`)
...
(`backfill.py:573`, `backfill.py:581`). Dividing by that day's actual UTC-hour
...
`backfill.py:588`) is prorated the same way against `days_in_full_month`. The
compensation regime clamps the displayed energy term at zero (`backfill.py:619`),
...
safe (`backfill.py:654`). `backfill_if_missing` avoids redundant work by probing
...
(`backfill.py:189`) queries `statistics_during_period` over a 2-day window from
...
is series-scoped, not range-scoped: `_clear_all` (`backfill.py:232`) deletes the
...
Jan 1 of the end year, it raises `ServiceValidationError` (`backfill.py:688`),
...
- `recorder` provides every statistics primitive backfill uses:
  ...
  `StatisticMeanType` (`backfill.py:307`, `backfill.py:460`),
  `statistics_during_period` for the missing-probe (`backfill.py:211`),
  `clear_statistics` for the destructive path (`backfill.py:247`), and the
  ...
  `try/except ImportError` (`backfill.py:207`, `backfill.py:244`) so a bare HA
