...
| --- | --- | --- |
| `ENTSOE_BASE_URL` | `https://web-api.tp.entsoe.eu/api` | `const.py:401` |
| `ENTSOE_BE_DOMAIN` | `10YBE----------2` (BE bidding zone EIC) | `const.py:402` |
...
| --- | --- | --- |
| `backfill_range` (`backfill.py:963`) | `backfill_statistics` service | Always runs over the requested range; `clear=True` deletes the series first. |
| `backfill_if_missing` (`backfill.py:1130`) | fire-and-forget task from `async_setup_entry` | Probes the recorder at the Jan 1 anchor and runs only when nothing exists. |
...
(service handler `_async_backfill_service` at `__init__.py:684`, one-shot
...
injected each past hour. `_backfill_cost_sensor` (`backfill.py:668`) recovers
...
`_month_snapshot_cache` (`cohort.py:507`, called at `backfill.py:431`), so a 365-day window
...
`_ensure_dynamic_spots` (`backfill.py:290`) reuses the coordinator's
...
`_seed_short_term_sum` (`backfill.py:912`) writes one short-term row at the last
...
`_backfill_cost_sensor` runs one running total per hour (`backfill.py:668`)
...
is series-scoped, not range-scoped: `_clear_all` (`backfill.py:268`) deletes the
