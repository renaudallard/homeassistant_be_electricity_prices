...
| --- | --- | --- |
| `backfill_range` (`backfill.py:826`) | `backfill_statistics` service | Always runs over the requested range; `clear=True` deletes the series first. |
| `backfill_if_missing` (`backfill.py:985`) | fire-and-forget task from `async_setup_entry` | Probes the recorder at the Jan 1 anchor and runs only when nothing exists. |
...
unique id `f"{entry_id}_{key}"` via `_stat_id` (`backfill.py:129`). When the
...
injected each past hour. `_backfill_cost_sensor` (`backfill.py:531`) recovers
...
`_hour_iter` grid (`backfill.py:142`).
...
`_seed_short_term_sum` (`backfill.py:775`) writes one short-term row at the last
