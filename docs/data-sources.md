...
| --- | --- | --- |
| `backfill_range` (`backfill.py:724`) | `backfill_statistics` service | Always runs over the requested range; `clear=True` deletes the series first. |
| `backfill_if_missing` (`backfill.py:824`) | fire-and-forget task from `async_setup_entry` | Probes the recorder at the Jan 1 anchor and runs only when nothing exists. |
...
unique id `f"{entry_id}_{key}"` via `_stat_id` (`backfill.py:125`). When the
...
(`_PRICE_SENSOR_KEYS`, `backfill.py:107`) is maintained by hand in lockstep with
...
injected each past hour. `_backfill_cost_sensor` (`backfill.py:483`) recovers
...
UTC-hour totals (`backfill.py:564`). The recorder helpers treat their date
...
(`backfill.py:426`, `backfill.py:608`). A dynamic supplier with no spot for the
hour is skipped, because `factor * spot + base` needs both terms
(`backfill.py:423`). `KeyError` / `ValueError` (a missing DSO row for an archived
month, or a non-static rate kind reaching the static path) skips just that hour
rather than tearing the whole backfill down (`backfill.py:427`).

The injection credit reuses `_historical_injection_rate` (`backfill.py:452`,
`backfill.py:629`), the same coordinator helper the live YTD path uses, so a
monthly-indexed, spot-indexed, or fixed injection rate is resolved identically in
both places.

### Spot provisioning for backfill

`_ensure_dynamic_spots` (`backfill.py:260`) reuses the coordinator's
...
(`backfill.py:303`): a static-energy contract whose injection is itself
...
`backfill.py:314`) to match the live coordinator's local-day anchoring.
...
- The cost series must stay within a single calendar year. `backfill_range`
  ...
  `backfill.py:761`) and never crosses a year boundary; a multi-year request only
  backfills the end year's cost (`backfill.py:788`). The price `mean` sensors are
  unaffected and keep the full requested window.
- The accumulator starts from Jan 1 but only emits rows on or after
  `emit_from` (`backfill.py:705`), so a mid-year `start` still carries the correct
...
(`backfill.py:645`, `backfill.py:647`). Dividing by that day's actual UTC-hour
...
`backfill.py:672`) is prorated the same way against `days_in_full_month`. The
compensation regime clamps the displayed energy term at zero (`backfill.py:698`),
...
safe (`backfill.py:735`). `backfill_if_missing` avoids redundant work by probing
...
(`backfill.py:195`) queries `statistics_during_period` over a 2-day window from
...
is series-scoped, not range-scoped: `_clear_all` (`backfill.py:238`) deletes the
...
Jan 1 of the end year, it raises `ServiceValidationError` (`backfill.py:774`),
...
- `recorder` provides every statistics primitive backfill uses:
  ...
  `StatisticMeanType` (`backfill.py:358`, `backfill.py:519`),
  `statistics_during_period` for the missing-probe (`backfill.py:218`),
  `clear_statistics` for the destructive path (`backfill.py:253`), and the
  ...
  `try/except ImportError` (`backfill.py:213`, `backfill.py:249`) so a bare HA
