...
| Class | Base | Role |
| --- | --- | --- |
| `_WizardStepsMixin` | - | The shared step chain (`async_step_contract` through `async_step_meters`) plus the branch helpers (`config_flow.py:1156`) |
| `BePricesConfigFlow` | `_WizardStepsMixin, ConfigFlow` | Install-time flow; entry step `async_step_user`, finalizes with `async_create_entry` (`config_flow.py:1500`) |
| `BePricesOptionsFlow` | `_WizardStepsMixin, OptionsFlow` | Post-install; menu -> `edit` (re-runs the chain pre-filled) or `compare` (throwaway quote) (`config_flow.py:1548`) |
...
mixin's docstring at `config_flow.py:1157` states the invariant: `_after_meter` is
overridden in `BePricesConfigFlow` to add the install-time unique-id reject, and
`_finalize` is abstract (`config_flow.py:1501` raises `NotImplementedError`).
...
(`config_flow.py:26` module docstring, `config_flow.py:1536`).
...
| --- | --- | --- | --- | --- |
| `user` | `async_step_user` (`config_flow.py:1512`) | Supplier, region | `CONF_SUPPLIER`, `CONF_REGION` | Always (install entry step) |
| `contract` | `async_step_contract` (`config_flow.py:1172`) | Contract (region-filtered) | `CONF_CONTRACT` | Always; aborts `supplier_region_unavailable` if none |
| `dso` | `async_step_dso` (`config_flow.py:1236`) | Distribution operator | `CONF_DSO` | Always |
| `meter` | `async_step_meter` (`config_flow.py:1247`) | Meter type | `CONF_METER` | Always; option list narrows by contract kind |
| `dso_tariff_mode` | `async_step_dso_tariff_mode` (`config_flow.py:1381`) | DSO billing mode (simple/bi/impact) | `CONF_DSO_TARIFF_MODE` | Region == Wallonia (`config_flow.py:1396`) |
| `api_key` | `async_step_api_key` (`config_flow.py:1260`) | ENTSO-E token (required) | `CONF_API_KEY` | Contract kind == `dynamic` or `spot_monthly` (both are spot-indexed) |
...
| `capacity` | `async_step_capacity` (`config_flow.py:1278`) | Peak source (sensor/fixed) + value | `CONF_CAPACITY_MODE`, `CONF_CAPACITY_PEAK_SENSOR`, `CONF_CAPACITY_FIXED_KW` | Region == Flanders (`config_flow.py:1451`, `:1418`) |
| `connection_power` | `async_step_connection_power` (`config_flow.py:1409`) | Brussels connection-power tier | `CONF_CONNECTION_KVA_TIER` | Region == Brussels (`config_flow.py:1424`) |
| `solar` | `async_step_solar` (`config_flow.py:1294`) | Inverter kVA + regime | `CONF_SOLAR_KVA`, `CONF_SOLAR_REGIME` | Always |
| `injection_api_key` | `async_step_injection_api_key` (`config_flow.py:1331`) | ENTSO-E token (optional) | `CONF_API_KEY` | `_needs_injection_api_key` true (`config_flow.py:1304`) |
...
| `meters` | `async_step_meters` (`config_flow.py:1364`) | kWh sensors (registers or totals) | 6 `CONF_*_KWH` keys | Always (final step, then `_finalize`) |
...
in the mixin: `_after_meter` (`config_flow.py:1392`), `_after_dso_tariff_mode`
(`config_flow.py:1428`), `_after_api_key` (`config_flow.py:1438`), `_before_solar`
(`config_flow.py:1420`), and `_after_solar` (`config_flow.py:1317`).
...
`async_step_user` seeds `self._data = {}` on first entry (`config_flow.py:1516`).
The OptionsFlow's `edit` step seeds instead from `{**entry.data, **entry.options}`
(`config_flow.py:1570`), which is why every later step can pre-fill.

### `contract`: region-filtered product list

Schema `_contract_schema` (`config_flow.py:317`). Contracts come from
`_contracts_for(supplier_id, region)` (`config_flow.py:198`), which reads
...
filtered list is empty (`config_flow.py:1178`), for example a Flanders-only supplier
selected with region Wallonia. The default is pre-selected only when the stored
`CONF_CONTRACT` still exists in the filtered set (`config_flow.py:329`); a stale id
leaves the field unset so the user must repick.

### `dso`: distribution operator

Schema `_dso_schema` (`config_flow.py:431`). Options come from `DSO_CHOICES[region]`
(`const.py:101`) via `_region_dso_options` (`config_flow.py:205`): 8 Fluvius
...
Schema `_meter_schema` (`config_flow.py:642`). The key rule (`config_flow.py:654`):
...
billed energy by TOU slot, two billing modes that do not mix (`config_flow.py:645`
comment). `_contract_kind` (`config_flow.py:216`) resolves the kind from the
...
Schema `_dso_tariff_mode_schema` (`config_flow.py:443`), default `DSO_MODE_BI_HORAIRE`.
Options are `DSO_TARIFF_MODES` = `simple | bi_horaire | impact` (`const.py:174`),
`translation_key="dso_tariff_mode"`.

Reached only when region is Wallonia (`_after_meter`, `config_flow.py:1396`). Tarif
...
capacity tariff; `config_flow.py:1393` comment).

### `api_key`: ENTSO-E token for spot-indexed energy (required)

Schema `_api_key_schema` (`config_flow.py:675`), a `PASSWORD` text field. Reached
...
ENTSO-E day-ahead endpoint by `_validate_entsoe_key` (`config_flow.py:686`) before
the flow proceeds:
...
The validator queries a 24h window anchored on yesterday (`config_flow.py:703`): a
...
Schema `_capacity_schema` (`config_flow.py:717`). Reached from `_after_api_key` or
`_after_dso_tariff_mode` when region is Flanders (`config_flow.py:1451`, `:1418`).
Fields:
...
- `CONF_CAPACITY_PEAK_SENSOR`: an `EntitySelector` restricted to
  `device_class=["power","apparent_power"]` (`config_flow.py:737`). The restriction
...
`_apply_energy_manager_capacity_default` (`config_flow.py:1018`), which tries two
sources in order.
...
source, or the consumption sensor is not a Riemann child (`config_flow.py:1035`
comment). A non-power source is left blank so the device_class-filtered picker
forces a deliberate choice (issue #19 again, `config_flow.py:1086`).

### `connection_power`: Brussels connection-power tier

Schema `_connection_power_schema` (`config_flow.py:459`), default
...
`_before_solar` when region is Brussels (`config_flow.py:1424`). Brussels bills a
...
straight to solar (`config_flow.py:1421` comment).

### `solar`: inverter kVA + regime

Schema `_solar_schema` (`config_flow.py:1107`). Fields:
...
The region filter (`config_flow.py:1112`): `SOLAR_REGIME_COMPENSATION` is offered
...
falls back to `SOLAR_REGIME_NONE` (`config_flow.py:1119`).

### `injection_api_key`: optional ENTSO-E token for spot-indexed injection

Schema is inline (`config_flow.py:1354`), an *optional* `PASSWORD` field. The gate is
`_needs_injection_api_key` (`config_flow.py:1304`), which is true when all of:
...
`_contract_has_spot_injection` (`config_flow.py:230`) reads the registry's
...
step, this one is skippable (`config_flow.py:1334` docstring): submitting blank pops
...
`_validate_entsoe_key` the same way as the dynamic step (`config_flow.py:1347`).

### `meters`: cumulative kWh sensors (current-year cost)

Schema `_meters_schema` (`config_flow.py:772`). All six fields are optional
`EntitySelector`s restricted to `device_class="energy"` (`config_flow.py:791`) so a
...
| Single cumulative totals | `CONF_CONSUMPTION_KWH`, `CONF_INJECTION_KWH` | Coordinator splits deltas into day/night via `is_offpeak(now)` and persists them (`config_flow.py:773` docstring; `const.py:197`) |

When both are filled for the same side, the day/night registers win (more accurate;
`config_flow.py:783`). Each side (consumption, injection) is resolved independently,
...
`_apply_energy_manager_defaults` (`config_flow.py:919`) before rendering, but only
when *none* of the six keys is already set (`config_flow.py:933`). It reads the
...
`EntitySelector` default; `config_flow.py:969`). For each side it then tries
`_utility_meter_day_night_children` (`config_flow.py:840`) to also pre-fill the
...
- classifies each tariff name via `_classify_tariff` (`config_flow.py:815`), which
...
- bails to `{}` on any ambiguity (a name carrying both a day and a night token, or
  ...
  the year cost (`config_flow.py:858` comment).
...
| Rule | Where | Reason |
| --- | --- | --- |
| Supplier has no contract in region -> abort `supplier_region_unavailable` | `config_flow.py:1178` | Region filtering deferred from the supplier step to here |
| Dynamic/TOU/Impact contract forces `METER_DYNAMIC` | `config_flow.py:654` | Smart meter required; mixing bi-horaire network with TOU energy mis-bills |
| `dso_tariff_mode` (incl. Impact) only in Wallonia | `config_flow.py:1396` | Impact is CWaPE-only; other regions bill differently |
| `capacity` step only in Flanders | `config_flow.py:1451`, `:1418` | Only Flanders has the capaciteitstarief |
| `connection_power` step only in Brussels | `config_flow.py:1424` | Only Brussels charges the Brugel OSP fee |
| Compensation regime only in Wallonia | `config_flow.py:1112` | Avoids double-counting the Flemish capacity tariff |
| Peak sensor restricted to power/apparent_power | `config_flow.py:737` | Issue #19: a kWh sensor would inflate the capacity bill |
| kWh sensors restricted to device_class energy | `config_flow.py:791` | A non-energy sensor would be read as raw kWh |
| ENTSO-E key validated live before finalize | `config_flow.py:686` | Prevents finalizing an entry that fails on first refresh |
| Duplicate (supplier, contract, region, dso) tuple rejected | `config_flow.py:1524`, `:1544` | Two coordinators on the same tuple double-poll the supplier |
...
`BePricesConfigFlow._after_meter` (`config_flow.py:1524`) sets it after the meter
...
OptionsFlow enforces the same at finalize (`config_flow.py:1536`): if the edited
...
- `_meter_schema` clears the default when the stored meter is not in the
  kind-narrowed option list (`config_flow.py:660`).
- `_solar_schema` falls back to `none` when the stored regime is filtered out
  (`config_flow.py:1119`).

## Options flow

`BePricesOptionsFlow` (`config_flow.py:1548`) opens on `async_step_init`
(`config_flow.py:1558`) with a two-item menu (`async_show_menu`):

| Menu option | Step | Effect |
| --- | --- | --- |
| `edit` | `async_step_edit` (`config_flow.py:1566`) | Re-run the whole step chain pre-filled, save back to `entry.data` |
| `compare` | `async_step_compare` (`config_flow.py:1628`) | One-off quote against another supplier; nothing saved |
...
(`config_flow.py:1570`) so every downstream schema pre-fills from the live entry,
...
`_finalize` (`config_flow.py:1536`):

1. Recomputes the unique id from the edited tuple and aborts `already_configured`
   on collision with another entry (`config_flow.py:1592`).
2. Computes the new title via `_entry_title` (`config_flow.py:1144`),
   `"<supplier label> - <contract label> (<Region>)"`.
3. Skips the write entirely when nothing changed. The no-op check compares against
   the *merged* `{**data, **options}` (`config_flow.py:1605`), not `entry.data`
   ...
   entities and the warmed snapshot for no benefit (`config_flow.py:1593` comment).
...
The compare branch (`config_flow.py:1621` onward) walks `compare -> compare_contract
...
| Step | Method | Notes |
| --- | --- | --- |
| `compare` | `config_flow.py:1628` | Supplier picker via `_compare_supplier_options` (`config_flow.py:247`): suppliers with at least one contract in the user's region, excluding the expert `custom` supplier and any withdrawn one. Aborts `compare_no_alternative` if none |
| `compare_contract` | `config_flow.py:1656` | Contract picker via `_compare_contract_schema` (`config_flow.py:272`), spans static and dynamic kinds; excludes the user's current contract only when the same supplier is picked. Aborts `compare_no_alternative` when nothing remains |
| `compare_meter` | `config_flow.py:1694` | Only for static targets; dynamic/TOU/TOU-Impact targets are forced to `METER_DYNAMIC` and skip the step (`config_flow.py:1717`) |
| `compare_api_key` | `config_flow.py:1764` | Shown when `_after_compare_meter` (`config_flow.py:1736`) finds the quote needs spot data the entry lacks: a dynamic target, or (injection regime) a spot-indexed-injection contract on *either* side. Key used only for the quote, not saved (`strings.json:227`) |
| `compare_result` | `config_flow.py:1792` | Renders a side-by-side annual + YTD estimate via `_build_compare_placeholders` (`config_flow.py:1805`); submit aborts `compare_done` |

The compare-meter narrowing mirrors the install `_meter_schema` exactly (dynamic/
tou/tou_impact all require a smart meter; `config_flow.py:1712` comment). The
...
`coord._historical_spots` around the fetch (`config_flow.py:2140`). Placeholder
...
populates every token (even the reloading-entry fallback at `config_flow.py:1827`)
