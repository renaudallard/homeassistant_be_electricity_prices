...
Schema `_dso_schema` (`flow_schemas.py:491`). Options come from `DSO_CHOICES[region]`
...
Schema `_meter_schema` (`flow_schemas.py:782`). The key rule (`flow_schemas.py:782`):
...
Schema `_dso_tariff_mode_schema` (`flow_schemas.py:521`), default `DSO_MODE_BI_HORAIRE`.
...
Schema `_api_key_schema` (`flow_schemas.py:815`), a `PASSWORD` text field. Reached
...
ENTSO-E day-ahead endpoint by `_validate_entsoe_key` (`flow_schemas.py:826`) before
the flow proceeds:
...
Schema `_capacity_schema` (`flow_schemas.py:857`). Reached from `_after_api_key` or
...
Schema `_connection_power_schema` (`flow_schemas.py:540`), default
...
Schema `_solar_schema` (`flow_schemas.py:1000`). Fields:
...
`_validate_entsoe_key` the same way as the dynamic step (`flow_schemas.py:826`).

### `meters`: cumulative kWh sensors (current-year cost)

Schema `_meters_schema` (`flow_schemas.py:942`). All six fields are optional
...
| `compare` | `async_step_compare` (`compare_flow.py:335`) | One-off quote against another supplier; nothing saved |
...
| Step | Method | Notes |
| --- | --- | --- |
| `compare` | `compare_flow.py:335` | Supplier picker via `_compare_supplier_options` (`compare_flow.py:160`): suppliers with at least one contract in the user's region, excluding the expert `custom` supplier and any withdrawn one. Aborts `compare_no_alternative` if none |
| `compare_contract` | `compare_flow.py:319` | Contract picker via `_compare_contract_schema` (`compare_flow.py:185`), spans static and dynamic kinds; excludes the user's current contract only when the same supplier is picked. Aborts `compare_no_alternative` when nothing remains |
...
| `compare_api_key` | `compare_flow.py:486` | Shown when `_after_compare_meter` (`compare_flow.py:501`) finds the quote needs spot data the entry lacks: a spot-priced target (`SPOT_PRICED_CONTRACT_KINDS` - dynamic per slot, spot-monthly on the delivery month's mean), or (injection regime) a spot-indexed-injection contract on *either* side. Key used only for the quote, not saved |
| `compare_result` | `compare_flow.py:514` | Renders a side-by-side annual + YTD estimate via `_build_compare_placeholders` (`compare_flow.py:571`); submit aborts `compare_done`. Each side is priced on the spot its own energy shape bills: a dynamic leg on the mean of the fetched day-ahead window (linear in spot, so the yearly average is that mean), a spot-monthly leg on the DELIVERY MONTH's mean, which is the flat rate it actually bills and does not move with the day the dialog opened |
...
historical spot cache go through `_borrowed_spot_cache` (`compare_flow.py:118`),
