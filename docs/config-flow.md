...
| Class | Base | Role |
| --- | --- | --- |
| `_WizardStepsMixin` | - | The shared step chain (`async_step_contract` through `async_step_meters`) plus the branch helpers (`config_flow.py:860`) |
| `BePricesConfigFlow` | `_WizardStepsMixin, ConfigFlow` | Install-time flow; entry step `async_step_user`, finalizes with `async_create_entry` (`config_flow.py:1078`) |
| `BePricesOptionsFlow` | `_WizardStepsMixin, OptionsFlow` | Post-install; menu -> `edit` (re-runs the chain pre-filled) or `compare` (throwaway quote) (`config_flow.py:1156`) |
...
mixin's docstring at `config_flow.py:860` states the invariant: `_after_meter` is
overridden in `BePricesConfigFlow` to add the install-time unique-id reject, and
`_finalize` is abstract (`config_flow.py:1071` raises `NotImplementedError`).
...
(`config_flow.py:26` module docstring, `config_flow.py:1050`).
...
| --- | --- | --- | --- | --- |
| `user` | `async_step_user` (`config_flow.py:984`) | Supplier, region | `CONF_SUPPLIER`, `CONF_REGION` | Always (install entry step) |
| `contract` | `async_step_contract` (`config_flow.py:876`) | Contract (region-filtered) | `CONF_CONTRACT` | Always; aborts `supplier_region_unavailable` if none |
...
| `api_key` | `async_step_api_key` (`config_flow.py:915`) | ENTSO-E token (required) | `CONF_API_KEY` | Contract kind == `dynamic` or `spot_monthly` (both are spot-indexed) |
...
| `capacity` | `async_step_capacity` (`config_flow.py:933`) | Peak source (sensor/fixed) + value | `CONF_CAPACITY_MODE`, `CONF_CAPACITY_PEAK_SENSOR`, `CONF_CAPACITY_FIXED_KW` | Region == Flanders (`config_flow.py:1062`, `:968`) |
| `connection_power` | `async_step_connection_power` (`config_flow.py:1037`) | Brussels connection-power tier | `CONF_CONNECTION_KVA_TIER` | Region == Brussels (`config_flow.py:949`) |
| `solar` | `async_step_solar` (`config_flow.py:945`) | Inverter kVA + regime | `CONF_SOLAR_KVA`, `CONF_SOLAR_REGIME` | Always |
| `injection_api_key` | `async_step_injection_api_key` (`config_flow.py:874`) | ENTSO-E token (optional) | `CONF_API_KEY` | `_needs_injection_api_key` true (`config_flow.py:955`) |
...
in the mixin: `_after_meter` (`config_flow.py:930`), `_after_dso_tariff_mode`
(`config_flow.py:1056`), `_after_api_key` (`config_flow.py:967`), `_before_solar`
(`config_flow.py:949`), and `_after_solar` (`config_flow.py:869`).
...
`async_step_user` seeds `self._data = {}` on first entry (`config_flow.py:987`).
The OptionsFlow's `edit` step seeds instead from `{**entry.data, **entry.options}`
(`config_flow.py:1041`), which is why every later step can pre-fill.

### `contract`: region-filtered product list

Schema `_contract_schema` (`config_flow.py:249`). Contracts come from
`_contracts_for(supplier_id, region)` (`config_flow.py:142`), which reads
...
filtered list is empty (`config_flow.py:881`), for example a Flanders-only supplier
selected with region Wallonia. The default is pre-selected only when the stored
`CONF_CONTRACT` still exists in the filtered set (`config_flow.py:259`); a stale id
leaves the field unset so the user must repick.

### `dso`: distribution operator

Schema `_dso_schema` (`config_flow.py:255`). Options come from `DSO_CHOICES[region]`
(`const.py:85`) via `_region_dso_options` (`config_flow.py:149`): 8 Fluvius
...
Schema `_meter_schema` (`config_flow.py:299`). The key rule (`config_flow.py:409`):
...
- Otherwise the full `METER_TYPES` list applies (`mono`, `bi`, `dynamic`,
  `exclusive_night`; `const.py:147`) with `METER_MONO` as the fallback.
...
billed energy by TOU slot, two billing modes that do not mix (`config_flow.py:403`
comment). `_contract_kind` (`config_flow.py:160`) resolves the kind from the
...
`const.py:139`, a dedicated night circuit (electric water heater, night-storage
...
Schema `_dso_tariff_mode_schema` (`config_flow.py:276`), default `DSO_MODE_BI_HORAIRE`.
Options are `DSO_TARIFF_MODES` = `simple | bi_horaire | impact` (`const.py:158`),
`translation_key="dso_tariff_mode"`.

Reached only when region is Wallonia (`_after_meter`, `config_flow.py:930`). Tarif
...
automatically when the DSO does not publish Impact rates (`const.py:149`), so the
...
Schema `_api_key_schema` (`config_flow.py:431`), a `PASSWORD` text field. Reached
...
The validator queries a 24h window anchored on yesterday (`config_flow.py:360`): a
...
Schema `_capacity_schema` (`config_flow.py:374`). Reached from `_after_api_key` or
`_after_dso_tariff_mode` when region is Flanders (`config_flow.py:1062`, `:968`).
Fields:
...
- `CONF_CAPACITY_PEAK_SENSOR`: an `EntitySelector` restricted to
  `device_class=["power","apparent_power"]` (`config_flow.py:491`). The restriction
...
(`config_flow.py:631`). That helper walks dashboard kWh grid source -> Riemann
...
source, or the consumption sensor is not a Riemann child (`config_flow.py:742`
comment). A non-power source is left blank so the device_class-filtered picker
forces a deliberate choice (issue #19 again, `config_flow.py:790`).

### `connection_power`: Brussels connection-power tier

Schema `_connection_power_schema` (`config_flow.py:283`), default
`DEFAULT_CONNECTION_KVA_TIER` = `le6` (`const.py:179`). Options are the four
residential tiers `CONNECTION_KVA_TIERS` (`const.py:173`): `le1_44`, `le6`,
...
against the parsed OSP table (`const.py:164`). Other regions have no such fee and go
straight to solar (`config_flow.py:949` comment).

### `solar`: inverter kVA + regime

Schema `_solar_schema` (`config_flow.py:811`). Fields:

- `CONF_SOLAR_KVA`: `NumberSelector` box 0-50 step 0.1, default 0.0 (0 means no
  panels, no prosumer cost; `const.py:202`).
- `CONF_SOLAR_REGIME`: `translation_key="solar_regime"`, options built from
  `SOLAR_REGIMES` (`const.py:213`) with a region filter.

The region filter (`config_flow.py:816`): `SOLAR_REGIME_COMPENSATION` is offered
...
falls back to `SOLAR_REGIME_NONE` (`config_flow.py:822`).

### `injection_api_key`: optional ENTSO-E token for spot-indexed injection

Schema is inline (`config_flow.py:995`), an *optional* `PASSWORD` field. The gate is
`_needs_injection_api_key` (`config_flow.py:955`), which is true when all of:
...
`_contract_has_spot_injection` (`config_flow.py:165`) reads the registry's
`Contract.spot_indexed_injection` flag (`providers/base.py:75`). That flag marks a
...
step, this one is skippable (`config_flow.py:874` docstring): submitting blank pops
...
`_validate_entsoe_key` the same way as the dynamic step (`config_flow.py:989`).

### `meters`: cumulative kWh sensors (current-year cost)

Schema `_meters_schema` (`config_flow.py:413`). All six fields are optional
`EntitySelector`s restricted to `device_class="energy"` (`config_flow.py:430`) so a
power/temperature/unitless sensor cannot be read as raw kWh. There are two wirings
per side, both feeding the `current_year_cost` computation:
...
| Single cumulative totals | `CONF_CONSUMPTION_KWH`, `CONF_INJECTION_KWH` | Coordinator splits deltas into day/night via `is_offpeak(now)` and persists them (`config_flow.py:413` docstring; `const.py:181`) |

When both are filled for the same side, the day/night registers win (more accurate;
`config_flow.py:521`). Each side (consumption, injection) is resolved independently,
...
`_apply_energy_manager_defaults` (`config_flow.py:659`) before rendering, but only
when *none* of the six keys is already set (`config_flow.py:673`). It reads the
...
`EntitySelector` default; `config_flow.py:607`). For each side it then tries
`_utility_meter_day_night_children` (`config_flow.py:580`) to also pre-fill the
...
- classifies each tariff name via `_classify_tariff` (`config_flow.py:456`), which
...
- bails to `{}` on any ambiguity (a name carrying both a day and a night token, or
  ...
  the year cost (`config_flow.py:598` comment).
...
| Rule | Where | Reason |
| --- | --- | --- |
| Supplier has no contract in region -> abort `supplier_region_unavailable` | `config_flow.py:881` | Region filtering deferred from the supplier step to here |
| Dynamic/TOU/Impact contract forces `METER_DYNAMIC` | `config_flow.py:409` | Smart meter required; mixing bi-horaire network with TOU energy mis-bills |
| `dso_tariff_mode` (incl. Impact) only in Wallonia | `config_flow.py:930` | Impact is CWaPE-only; other regions bill differently |
| `capacity` step only in Flanders | `config_flow.py:1062`, `:968` | Only Flanders has the capaciteitstarief |
| `connection_power` step only in Brussels | `config_flow.py:949` | Only Brussels charges the Brugel OSP fee |
| Compensation regime only in Wallonia | `config_flow.py:816` | Avoids double-counting the Flemish capacity tariff |
| Peak sensor restricted to power/apparent_power | `config_flow.py:491` | Issue #19: a kWh sensor would inflate the capacity bill |
| kWh sensors restricted to device_class energy | `config_flow.py:430` | A non-energy sensor would be read as raw kWh |
| ENTSO-E key validated live before finalize | `config_flow.py:343` | Prevents finalizing an entry that fails on first refresh |
| Duplicate (supplier, contract, region, dso) tuple rejected | `config_flow.py:1098`, `:1050` | Two coordinators on the same tuple double-poll the supplier |
...
side is rejected" rule described in `const.py:181` is enforced downstream in the
...
`BePricesConfigFlow._after_meter` (`config_flow.py:1098`) sets it after the meter
...
OptionsFlow enforces the same at finalize (`config_flow.py:1050`): if the edited
...
- `_meter_schema` clears the default when the stored meter is not in the
  kind-narrowed option list (`config_flow.py:317`).
- `_solar_schema` falls back to `none` when the stored regime is filtered out
  (`config_flow.py:822`).

## Options flow

`BePricesOptionsFlow` (`config_flow.py:1156`) opens on `async_step_init`
(`config_flow.py:1030`) with a two-item menu (`async_show_menu`):

| Menu option | Step | Effect |
| --- | --- | --- |
| `edit` | `async_step_edit` (`config_flow.py:1174`) | Re-run the whole step chain pre-filled, save back to `entry.data` |
| `compare` | `async_step_compare` (`config_flow.py:1100`) | One-off quote against another supplier; nothing saved |
...
(`config_flow.py:1041`) so every downstream schema pre-fills from the live entry,
...
`_finalize` (`config_flow.py:1050`):

1. Recomputes the unique id from the edited tuple and aborts `already_configured`
   on collision with another entry (`config_flow.py:1058`).
2. Computes the new title via `_entry_title` (`config_flow.py:848`),
   `"<supplier label> - <contract label> (<Region>)"`.
3. Skips the write entirely when nothing changed. The no-op check compares against
   the *merged* `{**data, **options}` (`config_flow.py:1213`), not `entry.data`
   ...
   entities and the warmed snapshot for no benefit (`config_flow.py:1201` comment).
...
The compare branch (`config_flow.py:1093` onward) walks `compare -> compare_contract
...
| Step | Method | Notes |
| --- | --- | --- |
| `compare` | `config_flow.py:1100` | Supplier picker via `_compare_supplier_options` (`config_flow.py:182`): suppliers with at least one contract in the user's region. Aborts `compare_no_alternative` if none |
| `compare_contract` | `config_flow.py:1128` | Contract picker via `_compare_contract_schema` (`config_flow.py:208`), spans static and dynamic kinds; excludes the user's current contract only when the same supplier is picked. Aborts `compare_no_alternative` when nothing remains |
| `compare_meter` | `config_flow.py:1302` | Only for static targets; dynamic/TOU/TOU-Impact targets are forced to `METER_DYNAMIC` and skip the step (`config_flow.py:1189`) |
| `compare_api_key` | `config_flow.py:1236` | Shown when `_after_compare_meter` (`config_flow.py:1208`) finds the quote needs spot data the entry lacks: a dynamic target, or (injection regime) a spot-indexed-injection contract on *either* side. Key used only for the quote, not saved (`strings.json:141`) |
| `compare_result` | `config_flow.py:1264` | Renders a side-by-side annual + YTD estimate via `_build_compare_placeholders` (`config_flow.py:1277`); submit aborts `compare_done` |

The compare-meter narrowing mirrors the install `_meter_schema` exactly (dynamic/
tou/tou_impact all require a smart meter; `config_flow.py:1320` comment). The
...
`coord._historical_spots` around the fetch (`config_flow.py:1730`). Placeholder
...
populates every token (even the reloading-entry fallback at `config_flow.py:1300`)
