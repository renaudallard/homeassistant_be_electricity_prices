...
| Class | Base | Role |
| --- | --- | --- |
| `_WizardStepsMixin` | - | The shared step chain (`async_step_contract` through `async_step_meters`) plus the branch helpers (`config_flow.py:1546`) |
| `BePricesConfigFlow` | `_WizardStepsMixin, ConfigFlow` | Install-time flow; entry step `async_step_user`, finalizes with `async_create_entry` (`config_flow.py:1740`) |
| `BePricesOptionsFlow` | `_WizardStepsMixin, OptionsFlow` | Post-install; menu -> `edit` (re-runs the chain pre-filled) or `compare` (throwaway quote) (`config_flow.py:1779`) |
...
`_finalize` is abstract (`config_flow.py:1695` raises `NotImplementedError`).
...
| --- | --- | --- | --- | --- |
| `user` | `async_step_user` (`config_flow.py:1740`) | Supplier, region | `CONF_SUPPLIER`, `CONF_REGION` | Always (install entry step) |
| `contract` | `async_step_contract` (`config_flow.py:1328`) | Contract (region-filtered), optional start / end date | `CONF_CONTRACT`, `CONF_CONTRACT_START_DATE`, `CONF_CONTRACT_END_DATE` | Always. A supplier/region mismatch is now caught on the step where BOTH are chosen (`_region_mismatch_error`) and re-shows that form with `supplier_region_unavailable` on the supplier field, instead of aborting a step later and discarding every other edit made in the same options run; rejects a future start or an end not after the start |
| `signed_rate` | `async_step_signed_rate` (`config_flow.py:1377`) | The rate actually signed at: single / peak / offpeak / exclusive night, or spot factor / base, plus the yearly fee | The 6 `CONF_MANUAL_*` keys (`_MANUAL_RATE_KEYS`) | `_needs_manual_rate` true (`config_flow.py:1353`): a start date is set on a fixed / dynamic contract of a non-custom supplier. Offered whether or not the supplier archives past cards, because what is typed wins over the archived card |
| `dso` | `async_step_dso` (`config_flow.py:1394`) | Distribution operator | `CONF_DSO` | Always |
| `meter` | `async_step_meter` (`config_flow.py:1405`) | Meter type | `CONF_METER` | Always; option list narrows by contract kind |
| `dso_tariff_mode` | `async_step_dso_tariff_mode` (`config_flow.py:1576`) | DSO billing mode (simple/bi/impact) | `CONF_DSO_TARIFF_MODE` | Region == Wallonia (`config_flow.py:1576`) |
| `api_key` | `async_step_api_key` (`config_flow.py:1442`) | ENTSO-E token (required) | `CONF_API_KEY` | Contract kind == `dynamic` or `spot_monthly` (both are spot-indexed) |
...
| `capacity` | `async_step_capacity` (`config_flow.py:1460`) | Peak source (sensor/fixed) + value | `CONF_CAPACITY_MODE`, `CONF_CAPACITY_PEAK_SENSOR`, `CONF_CAPACITY_FIXED_KW` | Region == Flanders (`config_flow.py:1460`, `:1418`) |
| `connection_power` | `async_step_connection_power` (`config_flow.py:1604`) | Brussels connection-power tier | `CONF_CONNECTION_KVA_TIER` | Region == Brussels (`config_flow.py:1604`) |
| `solar` | `async_step_solar` (`config_flow.py:1476`) | Inverter kVA + regime | `CONF_SOLAR_KVA`, `CONF_SOLAR_REGIME` | Always |
| `injection_api_key` | `async_step_injection_api_key` (`config_flow.py:1513`) | ENTSO-E token (optional) | `CONF_API_KEY` | `_needs_injection_api_key` true (`config_flow.py:1486`) |
...
in the mixin: `_after_meter` (`config_flow.py:1587`), `_after_dso_tariff_mode`
(`config_flow.py:1430`), `_after_api_key` (`config_flow.py:1633`), `_before_solar`
(`config_flow.py:1422`), and `_after_solar` (`config_flow.py:1499`).
...
`async_step_user` seeds `self._data = {}` on first entry (`config_flow.py:1740`).
...
Reached only when region is Wallonia (`_after_meter`, `config_flow.py:1587`). Tarif
...
`_after_dso_tariff_mode` when region is Flanders (`config_flow.py:1623`, `:1418`).
Fields:
...
`_apply_energy_manager_capacity_default` (`config_flow.py:1174`), which tries two
sources in order.
...
`_before_solar` when region is Brussels (`config_flow.py:1615`). Brussels bills a
...
Schema `_solar_schema` (`config_flow.py:1263`). Fields:
...
`_needs_injection_api_key` (`config_flow.py:1486`), which is true when all of:
...
Schema `_meters_schema` (`config_flow.py:917`). All six fields are optional
...
`_apply_energy_manager_defaults` (`config_flow.py:1075`) before rendering, but only
...
`_utility_meter_day_night_children` (`config_flow.py:985`) to also pre-fill the
...
- classifies each tariff name via `_classify_tariff` (`config_flow.py:960`), which
...
| `capacity` step only in Flanders | `config_flow.py:2962`, `:1418` | Only Flanders has the capaciteitstarief |
...
The unique id is built by `_unique_id_for` (`config_flow.py:1699`): the string
...
`BePricesOptionsFlow` (`config_flow.py:1779`) opens on `async_step_init`
(`config_flow.py:1560`) with a two-item menu (`async_show_menu`):

| Menu option | Step | Effect |
| --- | --- | --- |
| `edit` | `async_step_edit` (`config_flow.py:1797`) | Re-run the whole step chain pre-filled, save back to `entry.data` |
| `compare` | `async_step_compare` (`config_flow.py:1866`) | One-off quote against another supplier; nothing saved |
...
2. Computes the new title via `_entry_title` (`config_flow.py:1300`),
...
| `compare_api_key` | `config_flow.py:1769` | Shown when `_after_compare_meter` (`config_flow.py:1977`) finds the quote needs spot data the entry lacks: a dynamic target, or (injection regime) a spot-indexed-injection contract on *either* side. Key used only for the quote, not saved (`strings.json:227`) |
| `compare_result` | `config_flow.py:1797` | Renders a side-by-side annual + YTD estimate via `_build_compare_placeholders` (`config_flow.py:2046`); submit aborts `compare_done` |
