...
Schema `_user_schema` (`flow_schemas.py:450`). Two dropdowns:
...
Schema `_contract_schema` (`flow_schemas.py:475`). Contracts come from
...
Schema `_dso_schema` (`flow_schemas.py:647`). Options come from `DSO_CHOICES[region]`
...
Schema `_meter_schema` (`flow_schemas.py:938`). The key rule (`flow_schemas.py:938`):
...
comment). `_contract_kind` (`flow_schemas.py:241`) resolves the kind from the
...
Schema `_dso_tariff_mode_schema` (`flow_schemas.py:677`), default `DSO_MODE_BI_HORAIRE`.
Options are `DSO_TARIFF_MODES` = `simple | bi_horaire | impact` (`const.py:335`),
...
Schema `_api_key_schema` (`flow_schemas.py:1003`), a `PASSWORD` text field. Reached
...
by `_validate_entsoe_key` (`flow_schemas.py:1014`) before the flow proceeds:
...
Schema `_capacity_schema` (`flow_schemas.py:1055`). Reached from `_after_api_key` or
...
Schema `_connection_power_schema` (`flow_schemas.py:696`), default
`DEFAULT_CONNECTION_KVA_TIER` = `le6` (`const.py:373`). Options are the four
residential tiers `CONNECTION_KVA_TIERS` (`const.py:354`): `le1_44`, `le6`,
...
Schema `_solar_schema` (`flow_schemas.py:1207`). Fields:
...
- `CONF_SOLAR_REGIME`: `translation_key="solar_regime"`, options built from
  `SOLAR_REGIMES` (`const.py:407`) with a region filter.
...
falls back to `SOLAR_REGIME_NONE` (`const.py:404`).
...
`_validate_entsoe_key` the same way as the dynamic step (`flow_schemas.py:1014`).

### `meters`: cumulative kWh sensors (current-year cost)

Schema `_meters_schema` (`flow_schemas.py:1140`). All six fields are optional
...
| Single cumulative totals | `CONF_CONSUMPTION_KWH`, `CONF_INJECTION_KWH` | Coordinator splits deltas into day/night via `is_offpeak(now)` and persists them (`const.py:394` docstring; `const.py:394`) |
...
`_IMPACT_DEFAULT_CONTRACTS` gives (`flow_schemas.py:674`): that card states only
...
step. `async_run_daily_compare` (`compare_flow.py:646`) drives
...
| `compare_solar` | `compare_flow.py:397` | What-if solar regime via `_compare_solar_schema` (`flow_schemas.py:1235`), narrowed to the region by the shared `_regime_options` (`flow_schemas.py:1187`). Skipped for an entry with no solar. Reached from both exits of `compare_meter`, so a dynamic target gets it too |
