...
- Region: the `REGIONS` tuple (`const.py`), rendered with `translation_key="region"`
  so `selector.region.options` in `strings.json` supplies the localized labels.

`async_step_user` seeds `self._data = {}` on first entry (`config_flow.py`).
The OptionsFlow's `edit` step seeds instead from `{**entry.data, **entry.options}`
(`config_flow.py`), which is why every later step can pre-fill.
...
Schema `_meter_schema` (`flow_schemas.py`). The key rule (`flow_schemas.py`):
...
- Otherwise the full `METER_TYPES` list applies (`mono`, `bi`, `dynamic`,
  `exclusive_night`; `const.py`) with `METER_MONO` as the fallback.
...
`const.py`, a dedicated night circuit (electric water heater, night-storage
...
Schema `_dso_tariff_mode_schema` (`flow_schemas.py`), default `DSO_MODE_BI_HORAIRE`.
...
Schema `_api_key_schema` (`flow_schemas.py`), a `PASSWORD` text field. Reached
...
`_fetch_spot_prices` (`coordinator_spots.py`) raises `missing ENTSO-E API key`
...
- `CONF_CAPACITY_PEAK_SENSOR`: an `EntitySelector` restricted to
  `device_class=["power","apparent_power"]` (`flow_schemas.py`). The restriction
...
`_apply_energy_manager_capacity_default` (`flow_prefill.py`), which tries two
sources in order.
...
falls back to `SOLAR_REGIME_NONE` (`const.py`).
...
1. `_contract_is_month_indexed(supplier, contract)` is true: the contract's ENERGY
   ...
   Eco Flux, Eneco Flex and Flex One, EBEM Groen Variabel and B@sic+, and every
...
- classifies each tariff name via `_classify_tariff` (`flow_prefill.py`), which
...
- bails to `{}` on any ambiguity (a name carrying both a day and a night token, or
  ...
  the year cost (`flow_schemas.py` comment),
...
Anything pre-filled stays editable (`strings.json`).
...
The unique id is built by `_unique_id_for` (`config_flow.py`): the string
`supplier:contract:region:dso`, **plus the meter for an exclusive-night circuit**.
On install, `BePricesConfigFlow._after_meter` (`config_flow.py`) sets it after
...
dedup. The OptionsFlow enforces the same at finalize (`config_flow.py`): if the
...
would leave the earlier contract out of the year. The copy kept for the
contract left still holds that box and its start date, and `previous_periods`
(`contract_periods.py`) starts its days there, so a household whose first
contract began mid-year still bills nothing before it. The step then hands off to
...
(`config_flow.py`) so every downstream schema pre-fills from the live entry,
...
user to edit the existing entry instead (`strings.json`).
...
`_IMPACT_DEFAULT_CONTRACTS` gives (`flow_schemas.py`): that card states only
...
| `compare_result` | `compare_flow.py` | Renders a side-by-side annual + YTD estimate via `_build_compare_placeholders` (`compare_placeholders.py`); submit aborts `compare_done`. Each side is priced on the spot its own energy shape bills: a dynamic leg on the mean of the fetched day-ahead window (linear in spot, so the yearly average is that mean), a spot-monthly leg on the DELIVERY MONTH's mean, which is the flat rate it actually bills and does not move with the day the dialog opened. A static card's per-slot feed-in credit is quoted on the closed days of day-ahead the entry holds for the past year (`_credit_spots`, `compare_inputs.py`), the window the projection credits it on, once a month of them is held. The year-to-date rows replay each month on its archived card through `_compute_current_year_cost` whenever both suppliers keep an archive, a spot-priced side included once the entry's own day-ahead cache covers every day of the window (`_spots_cover`, `compare_inputs.py`), which it does on an entry that is spot-priced itself. The page never fetches a year of day-ahead for a spot-priced side, so one without an archive or without those spots falls back to today's rate times the window's kWh |
...
| --- | --- |
| `step_id="user"` + fields | `config.step.user` (`strings.json`) |
| Each `async_step_<x>` | `config.step.<x>` (title, description, `data.<CONF>`) |
| `errors[CONF_API_KEY]="invalid_api_key"` | `config.error.invalid_api_key` (`strings.json`) |
| `errors[CONF_API_KEY]="empty_api_key"` | `config.error.empty_api_key` (`strings.json`) |
| `async_abort(reason="...")` | `config.abort.<reason>` (`strings.json`) |
| `translation_key=` on a selector | `selector.<key>.options.*` (`strings.json`) |
