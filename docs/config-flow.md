...
| Class | Base | Role |
| --- | --- | --- |
| `_WizardStepsMixin` | - | The shared step chain (`async_step_contract` through `async_step_meters`) plus the branch helpers (`config_flow.py:412`) |
| `BePricesConfigFlow` | `_WizardStepsMixin, ConfigFlow` | Install-time flow; entry step `async_step_user`, finalizes with `async_create_entry` (`config_flow.py:620`) |
| `BePricesOptionsFlow` | `_WizardStepsMixin, OptionsFlow` | Post-install; menu -> `edit` (re-runs the chain pre-filled) or `compare` (throwaway quote) (`config_flow.py:645`) |
...
`_finalize` is abstract (`config_flow.py:575` raises `NotImplementedError`).
...
| --- | --- | --- | --- | --- |
| `user` | `async_step_user` (`config_flow.py:620`) | Supplier, region | `CONF_SUPPLIER`, `CONF_REGION` | Always (install entry step) |
| `contract` | `async_step_contract` (`config_flow.py:190`) | Contract (region-filtered), optional start / end date | `CONF_CONTRACT`, `CONF_CONTRACT_START_DATE`, `CONF_CONTRACT_END_DATE` | Always. A supplier/region mismatch is now caught on the step where BOTH are chosen (`_region_mismatch_error`) and re-shows that form with `supplier_region_unavailable` on the supplier field, instead of aborting a step later and discarding every other edit made in the same options run; rejects a future start or an end not after the start |
| `signed_rate` | `async_step_signed_rate` (`config_flow.py:243`) | The rate actually signed at: single / peak / offpeak / exclusive night, or spot factor / base, plus the yearly fee | The 6 `CONF_MANUAL_*` keys (`_MANUAL_RATE_KEYS`) | `_needs_manual_rate` true (`config_flow.py:215`): a start date is set on a fixed, dynamic or spot-monthly contract of a non-custom supplier (the two spot-priced kinds sign a coefficient pair, so they get the factor / base boxes; fixed gets the rate boxes). Offered whether or not the supplier archives past cards, because what is typed wins over the archived card |
| `dso` | `async_step_dso` (`config_flow.py:260`) | Distribution operator | `CONF_DSO` | Always |
| `meter` | `async_step_meter` (`config_flow.py:271`) | Meter type | `CONF_METER` | Always; option list narrows by contract kind |
...
in the mixin: `_after_meter` (`config_flow.py:453`), `_after_dso_tariff_mode`
(`config_flow.py:501`), `_after_api_key` (`config_flow.py:511`), `_before_solar`
(`config_flow.py:493`), and `_after_solar` (`config_flow.py:365`).

## Step details and the billing constraint behind each branch

### `user` / `edit`: supplier + region

Schema `_user_schema` (`flow_schemas.py:379`). Two dropdowns:
...
`async_step_user` seeds `self._data = {}` on first entry (`config_flow.py:620`).
...
Schema `_contract_schema` (`flow_schemas.py:404`). Contracts come from
...
Schema `_dso_schema` (`flow_schemas.py:564`). Options come from `DSO_CHOICES[region]`
(`const.py:101`) via `_region_dso_options` (`flow_schemas.py:225`): 8 Fluvius
...
Schema `_meter_schema` (`flow_schemas.py:855`). The key rule (`flow_schemas.py:855`):
...
comment). `_contract_kind` (`flow_schemas.py:236`) resolves the kind from the
...
Schema `_dso_tariff_mode_schema` (`flow_schemas.py:594`), default `DSO_MODE_BI_HORAIRE`.
Options are `DSO_TARIFF_MODES` = `simple | bi_horaire | impact` (`const.py:294`),
`translation_key="dso_tariff_mode"`.

Reached only when region is Wallonia (`_after_meter`, `config_flow.py:453`). Tarif
...
Schema `_api_key_schema` (`flow_schemas.py:888`), a `PASSWORD` text field. Reached
...
ENTSO-E day-ahead endpoint by `_validate_entsoe_key` (`flow_schemas.py:899`) before
the flow proceeds:
...
Schema `_capacity_schema` (`flow_schemas.py:930`). Reached from `_after_api_key` or
`_after_dso_tariff_mode` when region is Flanders (`config_flow.py:501`, `:507`).
Fields:
...
Schema `_connection_power_schema` (`flow_schemas.py:613`), default
`DEFAULT_CONNECTION_KVA_TIER` = `le6` (`const.py:332`). Options are the four
residential tiers `CONNECTION_KVA_TIERS` (`const.py:313`): `le1_44`, `le6`,
...
Schema `_solar_schema` (`flow_schemas.py:1073`). Fields:
...
- `CONF_SOLAR_REGIME`: `translation_key="solar_regime"`, options built from
  `SOLAR_REGIMES` (`const.py:366`) with a region filter.
...
falls back to `SOLAR_REGIME_NONE` (`const.py:363`).
...
`_needs_injection_api_key` (`config_flow.py:352`), which is true when all of:
...
`_contract_has_spot_injection` (`flow_schemas.py:291`) reads the registry's
...
`_validate_entsoe_key` the same way as the dynamic step (`flow_schemas.py:899`).

### `meters`: cumulative kWh sensors (current-year cost)

Schema `_meters_schema` (`flow_schemas.py:1015`). All six fields are optional
...
| Single cumulative totals | `CONF_CONSUMPTION_KWH`, `CONF_INJECTION_KWH` | Coordinator splits deltas into day/night via `is_offpeak(now)` and persists them (`const.py:353` docstring; `const.py:353`) |
...
The unique id is built by `_unique_id_for` (`config_flow.py:579`): the string
...
`BePricesOptionsFlow` (`config_flow.py:645`) opens on `async_step_init`
...
| `compare` | `async_step_compare` (`compare_flow.py:420`) | One-off quote against another supplier; nothing saved |
...
`_IMPACT_DEFAULT_CONTRACTS` gives (`flow_schemas.py:591`): that card states only
...
`_SweepStepsMixin` (`compare_flow.py:1778`) is a separate branch reached from a
...
| `compare_solar` | `compare_flow.py:397` | What-if solar regime via `_compare_solar_schema` (`flow_schemas.py:1101`), narrowed to the region by the shared `_regime_options` (`flow_schemas.py:1053`). Skipped for an entry with no solar. Reached from both exits of `compare_meter`, so a dynamic target gets it too |
| `compare_api_key` | `compare_flow.py:486` | Shown when `_after_compare_meter` (`compare_flow.py:599`) finds the quote needs spot data the entry lacks: a spot-priced target (`SPOT_PRICED_CONTRACT_KINDS` - dynamic per slot, spot-monthly on the delivery month's mean), or (injection regime) a spot-indexed-injection contract on *either* side. Key used only for the quote, not saved |
| `compare_result` | `compare_flow.py:514` | Renders a side-by-side annual + YTD estimate via `_build_compare_placeholders` (`compare_flow.py:1207`); submit aborts `compare_done`. Each side is priced on the spot its own energy shape bills: a dynamic leg on the mean of the fetched day-ahead window (linear in spot, so the yearly average is that mean), a spot-monthly leg on the DELIVERY MONTH's mean, which is the flat rate it actually bills and does not move with the day the dialog opened |
...
historical spot cache go through `_borrowed_spot_cache` (`compare_flow.py:132`),
...
The builder is in two halves: `_resolve_household` (`compare_flow.py:678`) resolves everything that does not depend on which contract is being quoted -- the meter reads, the recorder walk, the measured hour shapes, the day-ahead window -- and returns a `_HouseholdQuote` (`compare_flow.py:368`); the rest of `_build_compare_placeholders` is the target side, recomputed per contract. The household half is O(1) in the number of contracts compared, which is what makes quoting more than one affordable. Placeholder
