...
| Class | Base | Role |
| --- | --- | --- |
| `_WizardStepsMixin` | - | The shared step chain (`async_step_contract` through `async_step_meters`) plus the branch helpers (`config_flow.py:511`) |
| `BePricesConfigFlow` | `_WizardStepsMixin, ConfigFlow` | Install-time flow; entry step `async_step_user`, finalizes with `async_create_entry` (`config_flow.py:719`) |
| `BePricesOptionsFlow` | `_WizardStepsMixin, OptionsFlow` | Post-install; menu -> `edit` (re-runs the chain pre-filled) or `compare` (throwaway quote) (`config_flow.py:744`) |
...
`_finalize` is abstract (`config_flow.py:674` raises `NotImplementedError`).
...
| --- | --- | --- | --- | --- |
| `user` | `async_step_user` (`config_flow.py:719`) | Supplier, region | `CONF_SUPPLIER`, `CONF_REGION` | Always (install entry step) |
...
| `capacity` | `async_step_capacity` (`config_flow.py:416`) | Peak source (sensor/fixed) + value | `CONF_CAPACITY_MODE`, `CONF_CAPACITY_PEAK_SENSOR`, `CONF_CAPACITY_FIXED_KW` | Region == Flanders (`config_flow.py:416`) |
| `connection_power` | `async_step_connection_power` (`config_flow.py:581`) | Brussels connection-power tier | `CONF_CONNECTION_KVA_TIER` | Region == Brussels (`config_flow.py:581`) |
| `solar` | `async_step_solar` (`config_flow.py:432`) | Inverter kVA + regime | `CONF_SOLAR_KVA`, `CONF_SOLAR_REGIME` | Always |
| `injection_api_key` | `async_step_injection_api_key` (`config_flow.py:475`) | ENTSO-E token (optional) | `CONF_API_KEY` | `_needs_optional_api_key` true (`config_flow.py:442`) |
...
in the mixin: `_after_meter` (`config_flow.py:552`), `_after_dso_tariff_mode`
(`config_flow.py:501`), `_after_api_key` (`config_flow.py:610`), `_before_solar`
(`config_flow.py:493`), and `_after_solar` (`config_flow.py:461`).

## Step details and the billing constraint behind each branch

### `user` / `edit`: supplier + region

Schema `_user_schema` (`flow_schemas.py:408`). Two dropdowns:
...
`async_step_user` seeds `self._data = {}` on first entry (`config_flow.py:719`).
...
Schema `_contract_schema` (`flow_schemas.py:433`). Contracts come from
...
Schema `_dso_schema` (`flow_schemas.py:603`). Options come from `DSO_CHOICES[region]`
...
Schema `_meter_schema` (`flow_schemas.py:894`). The key rule (`flow_schemas.py:894`):
...
Schema `_dso_tariff_mode_schema` (`flow_schemas.py:633`), default `DSO_MODE_BI_HORAIRE`.
Options are `DSO_TARIFF_MODES` = `simple | bi_horaire | impact` (`const.py:320`),
`translation_key="dso_tariff_mode"`.

Reached only when region is Wallonia (`_after_meter`, `config_flow.py:552`). Tarif
...
Schema `_api_key_schema` (`flow_schemas.py:927`), a `PASSWORD` text field. Reached
...
by `_validate_entsoe_key` (`flow_schemas.py:938`) before the flow proceeds:
...
`_fetch_spot_prices` (`coordinator_spots.py:552`) raises `missing ENTSO-E API key`
...
Schema `_capacity_schema` (`flow_schemas.py:979`). Reached from `_after_api_key` or
`_after_dso_tariff_mode` when region is Flanders (`config_flow.py:600`, `:507`).
Fields:
...
Schema `_connection_power_schema` (`flow_schemas.py:652`), default
...
`_before_solar` when region is Brussels (`config_flow.py:592`). Brussels bills a
...
Schema `_solar_schema` (`flow_schemas.py:1131`). Fields:
...
`_needs_optional_api_key` (`config_flow.py:442`), which is true when no
...
`_validate_entsoe_key` the same way as the dynamic step (`flow_schemas.py:938`).

### `meters`: cumulative kWh sensors (current-year cost)

Schema `_meters_schema` (`flow_schemas.py:1064`). All six fields are optional
...
The unique id is built by `_unique_id_for` (`config_flow.py:678`): the string
...
`BePricesOptionsFlow` (`config_flow.py:744`) opens on `async_step_init`
(`config_flow.py:343`) with a two-item menu (`async_show_menu`):

| Menu option | Step | Effect |
| --- | --- | --- |
| `edit` | `async_step_edit` (`config_flow.py:765`) | Re-run the whole step chain pre-filled, save back to `entry.data` |
| `compare` | `async_step_compare` (`compare_flow.py:1556`) | One-off quote against another supplier; nothing saved |
...
2. Computes the new title via `_entry_title` (`config_flow.py:131`),
...
`_IMPACT_DEFAULT_CONTRACTS` gives (`flow_schemas.py:630`): that card states only
...
`_SweepStepsMixin` (`compare_flow.py:2402`) is a separate branch reached from a
...
the steps are separate. `_sweep_candidates` (`flow_schemas.py:337`) narrows to
...
The pricing itself is not in the flow. `_SweepEngine` (`compare_flow.py:512`)
holds only an entry, a hass and the dialog's what-if overrides, which is
everything `_resolve_household` (`compare_flow.py:834`), `_sweep_own_row`
(`compare_flow.py:1105`) and `_sweep_one` (`compare_flow.py:1421`) ever read
...
`build_sweep` (`compare_flow.py:782`) resolves the cell for both callers, so
...
step. `async_run_daily_compare` (`compare_flow.py:477`) drives
`run_full_sweep` (`compare_flow.py:719`) and parks the result on
...
| `compare_solar` | `compare_flow.py:397` | What-if solar regime via `_compare_solar_schema` (`flow_schemas.py:1159`), narrowed to the region by the shared `_regime_options` (`flow_schemas.py:1111`). Skipped for an entry with no solar. Reached from both exits of `compare_meter`, so a dynamic target gets it too |
| `compare_api_key` | `compare_flow.py:486` | Shown when `_after_compare_meter` (`compare_flow.py:1735`) finds the quote needs spot data the entry lacks: a spot-priced target (`SPOT_PRICED_CONTRACT_KINDS` - dynamic per slot, spot-monthly on the delivery month's mean), or (injection regime) a spot-indexed-injection contract on *either* side. Key used only for the quote, not saved. Skippable like `injection_api_key`: a blank submission asks ENTSO-E nothing and goes straight on, since a quote is a one-off and every reader of the key falls back to the entry's own with `or` |
| `compare_result` | `compare_flow.py:514` | Renders a side-by-side annual + YTD estimate via `_build_compare_placeholders` (`compare_flow.py:1830`); submit aborts `compare_done`. Each side is priced on the spot its own energy shape bills: a dynamic leg on the mean of the fetched day-ahead window (linear in spot, so the yearly average is that mean), a spot-monthly leg on the DELIVERY MONTH's mean, which is the flat rate it actually bills and does not move with the day the dialog opened |
...
historical spot cache go through `_borrowed_spot_cache` (`compare_flow.py:143`),
...
The builder is in two halves: `_resolve_household` (`compare_flow.py:834`) resolves everything that does not depend on which contract is being quoted -- the meter reads, the recorder walk, the measured hour shapes, the day-ahead window -- and returns a `_HouseholdQuote` (`compare_flow.py:425`); the rest of `_build_compare_placeholders` is the target side, recomputed per contract. The household half is O(1) in the number of contracts compared, which is what makes quoting more than one affordable. Placeholder
