...
| Class | Base | Role |
| --- | --- | --- |
| `_WizardStepsMixin` | - | The shared step chain (`async_step_contract` through `async_step_meters`) plus the branch helpers (`config_flow.py:869`) |
| `BePricesConfigFlow` | `_WizardStepsMixin, ConfigFlow` | Install-time flow; entry step `async_step_user`, finalizes with `async_create_entry` (`config_flow.py:1087`) |
| `BePricesOptionsFlow` | `_WizardStepsMixin, OptionsFlow` | Post-install; menu -> `edit` (re-runs the chain pre-filled) or `compare` (throwaway quote) (`config_flow.py:1177`) |
...
mixin's docstring at `config_flow.py:869` states the invariant: `_after_meter` is
overridden in `BePricesConfigFlow` to add the install-time unique-id reject, and
`_finalize` is abstract (`config_flow.py:1080` raises `NotImplementedError`).
...
(`config_flow.py:26` module docstring, `config_flow.py:1481`).
...
| --- | --- | --- | --- | --- |
| `user` | `async_step_user` (`config_flow.py:1415`) | Supplier, region | `CONF_SUPPLIER`, `CONF_REGION` | Always (install entry step) |
| `contract` | `async_step_contract` (`config_flow.py:885`) | Contract (region-filtered) | `CONF_CONTRACT` | Always; aborts `supplier_region_unavailable` if none |
| `dso` | `async_step_dso` (`config_flow.py:1157`) | Distribution operator | `CONF_DSO` | Always |
| `meter` | `async_step_meter` (`config_flow.py:1168`) | Meter type | `CONF_METER` | Always; option list narrows by contract kind |
| `dso_tariff_mode` | `async_step_dso_tariff_mode` (`config_flow.py:1293`) | DSO billing mode (simple/bi/impact) | `CONF_DSO_TARIFF_MODE` | Region == Wallonia (`config_flow.py:1304`) |
| `api_key` | `async_step_api_key` (`config_flow.py:924`) | ENTSO-E token (required) | `CONF_API_KEY` | Contract kind == `dynamic` or `spot_monthly` (both are spot-indexed) |
...
| `capacity` | `async_step_capacity` (`config_flow.py:942`) | Peak source (sensor/fixed) + value | `CONF_CAPACITY_MODE`, `CONF_CAPACITY_PEAK_SENSOR`, `CONF_CAPACITY_FIXED_KW` | Region == Flanders (`config_flow.py:1071`, `:968`) |
| `connection_power` | `async_step_connection_power` (`config_flow.py:1046`) | Brussels connection-power tier | `CONF_CONNECTION_KVA_TIER` | Region == Brussels (`config_flow.py:949`) |
| `solar` | `async_step_solar` (`config_flow.py:954`) | Inverter kVA + regime | `CONF_SOLAR_KVA`, `CONF_SOLAR_REGIME` | Always |
| `injection_api_key` | `async_step_injection_api_key` (`config_flow.py:883`) | ENTSO-E token (optional) | `CONF_API_KEY` | `_needs_injection_api_key` true (`config_flow.py:964`) |
...
| `meters` | `async_step_meters` (`config_flow.py:1281`) | kWh sensors (registers or totals) | 6 `CONF_*_KWH` keys | Always (final step, then `_finalize`) |
...
in the mixin: `_after_meter` (`config_flow.py:1304`), `_after_dso_tariff_mode`
(`config_flow.py:1065`), `_after_api_key` (`config_flow.py:976`), `_before_solar`
(`config_flow.py:949`), and `_after_solar` (`config_flow.py:1234`).
...
`async_step_user` seeds `self._data = {}` on first entry (`config_flow.py:1418`).
The OptionsFlow's `edit` step seeds instead from `{**entry.data, **entry.options}`
(`config_flow.py:1472`), which is why every later step can pre-fill.
...
filtered list is empty (`config_flow.py:890`), for example a Flanders-only supplier
...
Schema `_dso_schema` (`config_flow.py:410`). Options come from `DSO_CHOICES[region]`
(`const.py:101`) via `_region_dso_options` (`config_flow.py:149`): 8 Fluvius
...
Schema `_meter_schema` (`config_flow.py:299`). The key rule (`config_flow.py:418`):
...
billed energy by TOU slot, two billing modes that do not mix (`config_flow.py:412`
...
Reached only when region is Wallonia (`_after_meter`, `config_flow.py:1304`). Tarif
...
capacity tariff; `config_flow.py:1304` comment).

### `api_key`: ENTSO-E token for spot-indexed energy (required)

Schema `_api_key_schema` (`config_flow.py:440`), a `PASSWORD` text field. Reached
...
ENTSO-E day-ahead endpoint by `_validate_entsoe_key` (`config_flow.py:659`) before
the flow proceeds:
...
The validator queries a 24h window anchored on yesterday (`config_flow.py:676`): a
...
Schema `_capacity_schema` (`config_flow.py:690`). Reached from `_after_api_key` or
`_after_dso_tariff_mode` when region is Flanders (`config_flow.py:1071`, `:968`).
Fields:
...
- `CONF_CAPACITY_PEAK_SENSOR`: an `EntitySelector` restricted to
  `device_class=["power","apparent_power"]` (`config_flow.py:500`). The restriction
...
forces a deliberate choice (issue #19 again, `config_flow.py:799`).

### `connection_power`: Brussels connection-power tier

Schema `_connection_power_schema` (`config_flow.py:438`), default
...
Schema `_solar_schema` (`config_flow.py:820`). Fields:
...
The region filter (`config_flow.py:825`): `SOLAR_REGIME_COMPENSATION` is offered
...
falls back to `SOLAR_REGIME_NONE` (`config_flow.py:831`).

### `injection_api_key`: optional ENTSO-E token for spot-indexed injection

Schema is inline (`config_flow.py:1004`), an *optional* `PASSWORD` field. The gate is
`_needs_injection_api_key` (`config_flow.py:964`), which is true when all of:
...
`_contract_has_spot_injection` (`config_flow.py:217`) reads the registry's
...
step, this one is skippable (`config_flow.py:883` docstring): submitting blank pops
...
`_validate_entsoe_key` the same way as the dynamic step (`config_flow.py:998`).

### `meters`: cumulative kWh sensors (current-year cost)

Schema `_meters_schema` (`config_flow.py:729`). All six fields are optional
`EntitySelector`s restricted to `device_class="energy"` (`config_flow.py:746`) so a
...
| Single cumulative totals | `CONF_CONSUMPTION_KWH`, `CONF_INJECTION_KWH` | Coordinator splits deltas into day/night via `is_offpeak(now)` and persists them (`config_flow.py:729` docstring; `const.py:197`) |

When both are filled for the same side, the day/night registers win (more accurate;
`config_flow.py:530`). Each side (consumption, injection) is resolved independently,
...
`_apply_energy_manager_defaults` (`config_flow.py:668`) before rendering, but only
when *none* of the six keys is already set (`config_flow.py:682`). It reads the
...
`EntitySelector` default; `config_flow.py:923`). For each side it then tries
`_utility_meter_day_night_children` (`config_flow.py:589`) to also pre-fill the
...
- classifies each tariff name via `_classify_tariff` (`config_flow.py:772`), which
...
- bails to `{}` on any ambiguity (a name carrying both a day and a night token, or
  ...
  the year cost (`config_flow.py:607` comment).
...
| Rule | Where | Reason |
| --- | --- | --- |
| Supplier has no contract in region -> abort `supplier_region_unavailable` | `config_flow.py:890` | Region filtering deferred from the supplier step to here |
...
| ENTSO-E key validated live before finalize | `config_flow.py:659` | Prevents finalizing an entry that fails on first refresh |
| Duplicate (supplier, contract, region, dso) tuple rejected | `config_flow.py:1113`, `:1050` | Two coordinators on the same tuple double-poll the supplier |
...
`BePricesConfigFlow._after_meter` (`config_flow.py:1113`) sets it after the meter
...
OptionsFlow enforces the same at finalize (`config_flow.py:1481`): if the edited
...
- `_meter_schema` clears the default when the stored meter is not in the
  kind-narrowed option list (`config_flow.py:633`).
- `_solar_schema` falls back to `none` when the stored regime is filtered out
  (`config_flow.py:831`).

## Options flow

`BePricesOptionsFlow` (`config_flow.py:1177`) opens on `async_step_init`
(`config_flow.py:1461`) with a two-item menu (`async_show_menu`):

| Menu option | Step | Effect |
| --- | --- | --- |
| `edit` | `async_step_edit` (`config_flow.py:1195`) | Re-run the whole step chain pre-filled, save back to `entry.data` |
| `compare` | `async_step_compare` (`config_flow.py:1531`) | One-off quote against another supplier; nothing saved |
...
(`config_flow.py:1472`) so every downstream schema pre-fills from the live entry,
...
`_finalize` (`config_flow.py:1481`):

1. Recomputes the unique id from the edited tuple and aborts `already_configured`
   on collision with another entry (`config_flow.py:1489`).
2. Computes the new title via `_entry_title` (`config_flow.py:857`),
   `"<supplier label> - <contract label> (<Region>)"`.
3. Skips the write entirely when nothing changed. The no-op check compares against
   the *merged* `{**data, **options}` (`config_flow.py:1234`), not `entry.data`
   ...
   entities and the warmed snapshot for no benefit (`config_flow.py:1222` comment).
...
The compare branch (`config_flow.py:1524` onward) walks `compare -> compare_contract
...
| `compare_result` | `config_flow.py:1695` | Renders a side-by-side annual + YTD estimate via `_build_compare_placeholders` (`config_flow.py:1708`); submit aborts `compare_done` |

The compare-meter narrowing mirrors the install `_meter_schema` exactly (dynamic/
tou/tou_impact all require a smart meter; `config_flow.py:1341` comment). The
...
`coord._historical_spots` around the fetch (`config_flow.py:1751`). Placeholder
...
populates every token (even the reloading-entry fallback at `config_flow.py:1731`)
