...
Every sensor is one `BePriceSensor` (`sensor.py:547`) instance driven by a
...
`native_value` (`sensor.py:604`) calls `value_fn(coordinator.data)` and then
...
`async_setup_entry` (`sensor.py:514`) assembles the entity list conditionally:

| Group | Source | Created when |
| --- | --- | --- |
| `SENSORS` (11 core price sensors) | `sensor.py:383` | always |
| `FEE_SENSORS` (4 fee/cost sensors) | `sensor.py:415` | always |
| `CAPACITY_SENSORS` (2) | `sensor.py:480` | `CONF_REGION == REGION_FLANDERS` |
| `PROSUMER_SENSORS` (1) | `sensor.py:400` | `solar_kva > 0` and `CONF_SOLAR_REGIME == SOLAR_REGIME_COMPENSATION` |
| `INJECTION_SENSORS` (1) | `sensor.py:411` | `CONF_SOLAR_REGIME == SOLAR_REGIME_INJECTION` |
| `ContractEndDateSensor` (1) | `sensor.py:687` | `CONF_CONTRACT_END_DATE` is set |
...
Two sensors read the clock through it: `_current` (`sensor.py:116`) over
...
`_next_hour` (`sensor.py:141`) targets `slot_start(now) + 1h`. On a 15-minute
...
The today/tomorrow scalar sensors (`_bucket`, `sensor.py:151`) reduce over every
slot whose local date matches, so on a quarter-hourly contract they operate at
native 15-minute resolution.

The three tomorrow sensors go through `_tomorrow_bucket` (`sensor.py:213`),
...
`today`/`tomorrow` arrays when its injection varies intra-day (`sensor.py:302`);
...
`_today_ranked` (`sensor.py:248`) guarantees the cheapest and dearest lists are
...
- `last_reset` (`sensor.py:599`) is pinned to Jan 1 00:00 local via
