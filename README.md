...
- **Dynamic contracts** — `factor × spot + base`, where `spot` is the Belgian day-ahead price from ENTSO-E. Priced per hour by default; suppliers that bill per quarter-hour (the Bolt Dynamisch cards, Cociter, EBEM, Ecofix, Ecopower Dynamische Burgerstroom, energie.be, Energy Knights Agilior Online, EnergyVision, Engie and OCTA+, after the SDAC 15-minute market switch of Oct 2025) keep the native 15-minute slots for the live price, next slot and cheapest-window service. **Frank Energie** lets you pick: its cards price per hour and its app switches the account to quarter-hourly prices from the start of any month, so the **Bill per quarter-hour** box on the meter step says which side you are on. The formula is the same either way, only the market price it reads changes. The `today` / `tomorrow` **list attributes** carry those same 15-minute slots, so an automation can plan a day against the grid it is billed on; only the cheapest / most-expensive lists stay hourly, since they are counted in hours (see `current_price` below). Year-to-date billing stays hourly too, since Home Assistant only retains hourly long-term statistics.
...
| --- | --- | --- |
| **Bolt** | Bolt Fixe · Bolt Plenty Fixe · Bolt Variable · Bolt Plenty Variable · Bolt Online · Bolt Plenty Online · each of those four variable cards also as **Dynamisch** *(quarter-hourly Belpex)* · all ten as **pro** contracts | [`providers/bolt.py`](./custom_components/be_electricity_prices/providers/bolt.py) — card URLs under `files.boltenergie.be/pricelists/<fix\|var>/`: fixed cards roll monthly by `<YYYYMM>`, variable and dynamic cards carry a version suffix Bolt bumps in place, read off the `boltenergie.be/fr/listes-des-prix` listing on every fetch (superseded versions stay served and still parse). Parsed via `pdfplumber` (rotated columns + Unicode line-separators). Every variable card is sold on either settlement and the pair shares one document: the **Dynamisch** contract applies the card's printed `Belpex × factor + base` formula to the 15-minute spot, while the variable one redistributes the month RLP-weighted. Pick the Dynamisch row matching your own product, not Bolt Dynamisch: the four cards carry different coefficients and different standing charges (Plenty Online is `× 1,145 + 16,45` at €0,99/month against the base card's `× 1,168 + 16,90` at €8,99). |
...
quarter-hour (the Bolt Dynamisch cards, Cociter, EBEM, Ecofix, Ecopower Dynamische Burgerstroom, energie.be, Energy Knights Agilior Online, EnergyVision, Engie and OCTA+, plus Frank Energie with the quarter-hour box ticked), which keep the
...
| --- | --- | --- |
| `duration_hours` | _required_ | Window length in whole hours (1-48). On a 15-minute contract (the Bolt Dynamisch cards / Cociter / EBEM / Ecofix / Ecopower Dynamische Burgerstroom / energie.be / Energy Knights Agilior Online / EnergyVision / Engie / OCTA+, and Frank Energie with the quarter-hour box ticked) the window aligns to quarter-hour boundaries. |
