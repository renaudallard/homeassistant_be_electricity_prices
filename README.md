...
- **Dynamic contracts** — `factor × spot + base`, where `spot` is the Belgian day-ahead price from ENTSO-E. Priced per hour by default; suppliers that bill per quarter-hour (Cociter, EBEM, Ecofix, Ecopower Dynamische Burgerstroom, energie.be, Energy Knights Agilior Online, EnergyVision, Engie, OCTA+ and Trevion, after the SDAC 15-minute market switch of Oct 2025) keep the native 15-minute slots for the live price, next slot and cheapest-window service. **Bolt and Frank Energie** let you pick, so a **settlement** step in the setup flow asks which side you are on. Frank's cards price per hour and its app switches the account to quarter-hourly prices from the start of any month; Bolt settles the same printed formula either per quarter-hour or against the month redistributed RLP-weighted. The formula is the same either way, only the market price it reads changes. The `today` / `tomorrow` **list attributes** carry those same 15-minute slots, so an automation can plan a day against the grid it is billed on; only the cheapest / most-expensive lists stay hourly, since they are counted in hours (see `current_price` below). Year-to-date billing stays hourly too, since Home Assistant only retains hourly long-term statistics.
- **Monthly-indexed contracts** — `factor × this month's mean spot + base`, for cards that index the commodity to a monthly average rather than the live hourly price (energie.be Variabel, Energy Knights Essentia Online, and Trevion Groene Stroom Flex and LifePowr on Belpex_RLP, and Eneco Zon & Wind Flex and Flex One on Belpex-RLP-M). For all four suppliers that mean is the RLP-weighted one, each hour's price weighted by Synergrid's residential load profile, which the integration downloads and blends the way each supplier defines it: the mean of the three regional DSO curves for Eneco, the Fluvius curve for Energy Knights and Trevion, which both sell in Flanders only, and every DSO column counting once for energie.be. Each reproduces the values its own supplier published for 2026, which the plain monthly mean missed by a few percent, always low. The rate is flat for the whole delivery month and firms up as the month fills in, so an ENTSO-E API key is required just as for a dynamic contract. When such a card indexes its injection on a *different* parameter, the credit is resolved on that one: energie.be and Trevion Flex/LifePowr pay on the solar-weighted Belpex_SPP, which runs far below the consumption index in sunny months, so the integration downloads Synergrid's solar production profile and weights the month's spot prices by it. Until that profile is available the card's own printed indicative is credited instead — never the consumption index, which would roughly double the credit.
- **Time-of-Use contracts** — Engie Empower Flextime and Luminus SmartFlex: 3 hour-of-day bands (peak / transition / offpeak) with the supplier's published rates per slot. Luminus SmartFlex is billed on its *seasonal* schedule: peak 07:00-11:00 + 17:00-22:00 all year, the cheapest super-creuses band 11:00-17:00 only in spring/summer (21/03-20/09), and 22:00-07:00 always at the middle creuses rate (the "free electricity on Sundays" first-year promo is not modelled). Both cards index every band on the delivery month and print last month's figures, so with an **ENTSO-E API key** each band is re-priced on the running month mean; on Flextime the per-slot feed-in credit is re-indexed the same way.
...
- **Year-to-date cost** — `current_year_cost` sensor reports your running bill in EUR since Jan 1 (or since your contract start date, if you tick that option), computed day by day (or hour by hour for TOU, dynamic and monthly-indexed contracts) from HA's recorder (consumption × the tariff of the month that day/hour belongs to). Each day is billed at its own month's published rate when the supplier archives historical cards (Bolt fix / Cociter / DATS 24 / EBEM / Ecopower / Eneco / energie.be / Energy Knights / EnergyVision / Engie / Frank / Luminus / Mega / OCTA+ / Trevion); a closed month is read first from this repository's own card archive (every supplier's card, stored daily since September 2026 with the supplier archives mirrored onto it and kept three years, see *The card archive* under Development), then from the supplier's own archive for a month the branch does not hold, and only a month neither holds falls back to the current rate as a proxy. An Eneco Flex month is settled at the Belpex-RLP-M Eneco itself publishes for it on the following month's card, so a closed month bills the realised index and not the previous month's estimate its own card printed; until that card is out the estimate stands. The annual fees, the Walloon prosumer charge and the Flemish capacity tariff accrue into it too, each pro-rated over the elapsed year, so the figure is the whole bill rather than the energy side alone. **TOU contracts** (Engie Empower Flextime, Luminus SmartFlex) use the per-hour path so each kWh hits its actual peak / transition / offpeak rate. **Dynamic contracts** replay historical hourly ENTSO-E day-ahead spots from a persistent cache so each past kWh is billed at its actual `factor × spot + base` rate; an hour the cache cannot price still bills its network and tax legs and forfeits only the energy term, since those two are known from the month's card whatever the day-ahead price did; a spot-indexed feed-in credit for such an hour stands in at the card's printed indicative where the card prints one, and is forfeited where it does not, until the spot arrives. Compensation regime is billed the way a yearly-read meter is settled: each register's net for the year (consumption minus injection, clamped at zero per register since surplus is forfeited) is spread over the elapsed months by Synergrid's residential load profile and each slice priced at its month's tariff, which is how the supplier allocates the single net figure the DSO reports; until the profile is downloaded the metered hours are netted as they happen. Annual fees are pro-rated to the elapsed fraction of the year so the figure grows day by day instead of jumping to the full annual on Jan 1.
- **Month-to-date cost** — `current_month_cost` is that same bill accumulated from the 1st instead of from 1 January, which is the period most people budget in and the one an invoice covers. It resets on the 1st. It is priced as its own window rather than sliced off the year, so on the **compensation regime** it nets that month's registers on their own and twelve monthly figures will not add up to `current_year_cost`; on every other regime they do, and the yearly sensor stays the authority since annual netting is what the supplier actually settles. If you only want the *kWh* for the month rather than the cost, Home Assistant's own **Utility Meter** helper with a monthly cycle does that without this integration.
...
- **Statistics backfill** — on first install (or after a database reset) the integration populates the recorder's long-term statistics for the price sensors and `current_year_cost` from Jan 1 of the current year up to "now", so the Energy dashboard shows price history immediately. This runs in the background after setup, as do the year's spot-price fetch on a dynamic or monthly-indexed contract, the archived tariff cards the year-to-date bills each past month with, and the Synergrid load profile a compensation or index-weighted contract is settled on: **neither finishing the wizard nor starting Home Assistant waits on any of them**, so the last step returns in seconds rather than minutes and a restart cannot spend the whole of Home Assistant's startup budget reading tariff PDFs (one Frank Energie card takes about 25 seconds to read on a Raspberry Pi, and the year-to-date walks one per month). Those archived cards are kept on disk once a month has closed, so a restart re-reads at most the current month's rather than the whole year's. The year-to-date fills itself in over the following minute or two, and until it does the past hours it has not priced yet bill their network and tax legs and forfeit only the energy term, while a month whose card is not in hand yet is billed off the current card the way a supplier without an archive is billed all year, and a contract awaiting its load profile is priced on the plain average of the month's day-ahead prices. A day-ahead source that hangs rather than answers no longer holds startup either: the first fetch is given 45 seconds and the rest is left to the same background pass. The load profile is downloaded once per Home Assistant rather than once per configured contract. A `backfill_statistics` service is exposed for re-runs after a tariff change.
...
- **Signing-cohort pricing** — set an optional **contract start date** and a fixed or dynamic contract is priced at the rate you locked in that month instead of today's new-customer card, for suppliers that archive past cards (Bolt fix / Cociter / DATS 24 / EBEM / Ecopower / Eneco / energie.be / Energy Knights / EnergyVision / Engie / Frank / Luminus / Mega / OCTA+ / Trevion), on their fixed and dynamic products. A separate optional **tariff card month** says which card that is when it is not the month your supply started, which is the usual case after a switch: the contract is locked to the card in force when it was signed, and the switch takes about a month to go through, so energie.be supplies an *Elektriciteit dynamisch particulier online 06/26* contract from 1 July. Leave it blank and the start date's month is used, as before. The card lookup is the only thing that reads it, so the year-to-date window, the pro-rated fees and the welcome-credit window keep counting from the start date. Only the contract's own terms are frozen to the signing month, both the commodity price and the feed-in coefficients, since a contract that locks one locks the other; the regulated network tariffs and taxes still track the current month, and the year-to-date cost bills every past month at the same locked rate. The setup flow also offers an optional **signing-rate** step where you type the rate you actually locked in — energy (single, day, night, exclusive-night circuit) or spot factor / base (for a dynamic contract and for a monthly-indexed one, both of which sign a coefficient pair), plus the yearly fee. What you type wins, field by field: the published card only ever knew the new-customer rate, so a promotional, brokered or negotiated rate has to come from you. Every box left blank falls back to the signing-month card, read from this repository's own archive first and from the supplier's archive for a month the branch does not hold, and to the current card when neither has it. Leave the whole step blank to price purely off the card. Which card the entry ended up on is published as the `current_price` sensor's `signing_card` attribute, so a signing month no archive can serve says so instead of looking like the date was ignored. **Variable** contracts whose card exposes a numeric index formula (Cociter Variable, EBEM Groen Variabel / B@sic+, Eneco Flex and Flex One, Mega Flex) re-price differently: the signing month's *formula coefficients* are frozen and re-applied to the current month's mean spot, since a variable rate re-indexes monthly. This is exact for Cociter (its BELIX index is the arithmetic monthly mean) and for Eneco (its Belpex-RLP-M is computed on Synergrid's residential load profile, which the integration applies); for the other RLP-weighted cards (EBEM / Mega) the arithmetic mean is a close approximation of that weighting, so their re-price runs a few percent off. Where a card prints one formula per meter, each meter is billed its own — including a dedicated exclusive-night circuit, which is a separate contractual formula rather than the off-peak one. Cociter Tarif Variable Trihoraire re-prices the same way with one BELIX formula per CWaPE band, since its card prints last month's index too. Resolving that mean needs spot data, so the variable re-price only runs when an **ENTSO-E API key** is configured; without one the entry keeps the current card and prices off its published rate. Time-of-use contracts whose card prints one formula per band (Engie Empower Flextime, Luminus SmartFlex) re-price the same way, slot by slot.
...
- **Self-healing** — last-known prices keep serving on outage, and the day-ahead curve is persisted, so an ENTSO-E outage spanning a Home Assistant restart no longer blanks a dynamic entry. A stored curve is only ever used for the day it actually prices: once it is outlived it is discarded rather than served as the current one. Repair issues surface under **Settings → System → Repairs**: snapshot older than 7 days, a supplier extractor parse failure (layout drift), a card that downloads fine but carries no readable text layer, a card being priced off the archive's reading of its pixels, the supplier being unreachable after repeated fetch failures, ENTSO-E rejecting the API key, a supplier that has announced it is leaving the residential market, and four cards for a value the supplier never printed (the exclusive-night distribution rate, the Walloon Tarif Impact bands, the Walloon connection fee, the Walloon prosumer tariff). A single transient fetch timeout no longer raises an issue. The fetch and staleness ones auto-clear on the next successful refresh; the deprecation and missing-row ones clear when the underlying situation changes.
...
- **Expert custom formula** — an escape hatch for suppliers that publish no public tariff card (group-purchase deals, B2B-flavoured products). You type the commodity formula (`factor × spot + base`, a monthly-averaged spot rate, or a flat rate) and all regulated DSO + tax values; there is no live card, so it's a static snapshot with none of the auto-update or drift-check safety net. Listed last in the supplier dropdown and clearly labelled as expert. If your supplier does publish a card, or is already listed but priced differently from your card, open an issue with the card instead of rebuilding it here: the form says so on its first step, and a fix reaches everyone where a workaround reaches one entry.

## Supported providers

| Supplier | Contracts | Source |
| --- | --- | --- |
| **Bolt** | Bolt Fixe · Bolt Plenty Fixe · Bolt Variable · Bolt Plenty Variable · Bolt Online · Bolt Plenty Online · all six as **pro** contracts | [`providers/bolt.py`](./custom_components/be_electricity_prices/providers/bolt.py) — card URLs under `files.boltenergie.be/pricelists/<fix\|var>/`: fixed cards roll monthly by `<YYYYMM>`, variable and dynamic cards carry a version suffix Bolt bumps in place, read off the `boltenergie.be/fr/listes-des-prix` listing on every fetch (superseded versions stay served and still parse). Parsed via `pdfplumber` (rotated columns + Unicode line-separators). Every variable card is sold on **either settlement**, so tick **Bill per quarter-hour** on the setup flow's settlement step to be billed each quarter-hour at its own Belpex price instead of the month redistributed RLP-weighted. Same card, same printed `Belpex × factor + base` formula, same standing charge: only the index it reads changes. In Wallonia note that the CWaPE *Tarif Impact* energy bands belong to the monthly settlement, so ticking the box prices every quarter at its own Belpex instead of banding the day; the network leg still follows your DSO tariff mode. The four cards are not interchangeable, though (Plenty Online is `× 1,145 + 16,45` at €0,99/month against the base card's `× 1,168 + 16,90` at €8,99), so pick your own product. |
...
| **Ecofix** ⚠️ *(cards are page images since August 2026, read in CI)* | Motion (quarter-hourly Belpex 15M) · Motion Online (same index, own coefficients and a 10 €/yr standing charge, online-only) · Flexy (BELPEX-RLP-M monthly variable) · Flexy Online (same index, own coefficients and the same 10 €/yr charge, online-only) | [`providers/ecofix.py`](./custom_components/be_electricity_prices/providers/ecofix.py) — stable URLs at `portal.ecofixgp.be/docs/prices/current/EL_Ecofix_<PRODUCT>_NL.pdf`, overwrite-in-place each month; the catalogue is read from the public tariefkaarten listing, which is the only surface that names a product the registry does not already carry. One PDF carries Flanders + Wallonia overlays (no Brussels). Parsed via `pdfplumber` for the column-major Wallonia DSO table. **Published as page images since the August 2026 card**, so nothing here can parse them; the repository's daily card archive reads them with a dedicated engine and entries price off that row — see the note below. |
...
| **EnergyVision** | Dynamisch *(quarter-hourly Belpex)* · 3 jaar vast · 1 an fixe *(Wallonia)* · 1.800 kWh vast · vaste injectieprijs 3 jaar · Laadpunt | [`providers/energyvision.py`](./custom_components/be_electricity_prices/providers/energyvision.py) — monthly `Goedkope stroom` cards resolved from the `energyvision.be/nl-be/tariefkaart` listing (filenames carry the pricing month, e.g. `EV-0726-GSDYN-nl.pdf`), parsed via `pdfplumber`. Each product is published for one region in one language: the two Flemish cards in Dutch, the Walloon `1 an fixe` in French (`-WAL-fr`). The dynamic card prints Belpex in EUR/MWh (Bolt axis, factor not scaled by 10); both fixed cards index injection on the monthly Belpex-SPP-M and are credited at the delivery month's own solar-weighted mean, with the card's printed figure kept only as the fallback while that mean is unpublished; the card's 1 c€/kWh guarantee is applied on top. The three tiered cards bill a first tranche of the year at a flat rate (1.800 or 1.000 kWh) and the remainder on `factor × Belpex-RLP-M + 20 EUR/MWh`, weighted by Synergrid's Flemish residential load profile; the tranche is folded into that formula against your measured yearly consumption (the same figure the comparison page quotes its rows from, falling back to the 3.500 kWh default under 90 days of meter history), which is exact for the year because the cards' *Voordeelzekerheid* clause settles it so the full tranche is charged at the flat rate whatever the months did. An exclusive-night entry is billed the formula alone, since the footnote that grants the tranche withholds it from that register. `vaste injectieprijs 3 jaar` fixes its feed-in price instead of indexing it. Four of the six cards print a one-off **welkomstkorting**, which is credited against `current_year_cost` when you set a contract start date: pro rata per day over the first subscription year, capped at what that period charged for energy, the standing charge and the green electricity/WKK contribution, and never off network tariffs, taxes or levies. It expires by itself a year after the start date and is read off your **signing month's** card rather than today's (EnergyVision moved the figure four times between March and September 2026). The comparison page credits both sides: a card being quoted is credited what a customer signing it today is granted over the coming year, and in the year-to-date column what it would have granted had it been signed on your own start date, while your own row carries whatever is left of your first year. Past cards are read by their plain monthly filename, so a signing month back to each product's own horizon resolves. Gas and the two tiered cards that also price self-consumed solar (`eigen zon`) are out of scope. Since the August 2026 card the Walloon `1 an fixe` no longer prints the `Redevance de raccordement`, a charge Wallonia still levies; prices exclude it (about €2.60/yr at 3500 kWh) and the entry raises a Repairs card saying so, which clears when EnergyVision prints the row again. |
| **Engie** | Easy Fixed · Easy Variable · Direct Online · Basic Online · Dynamic · Empower Fixed · Empower Variable · Empower Flextime *(TOU)* · Flow · Empty House · the same eight as **pro** contracts, minus Direct Online and Basic Online | [`providers/engie.py`](./custom_components/be_electricity_prices/providers/engie.py) — Engie's public REST endpoint at `engie.be/api/engie/be/ms/pricing/v1/public/pricesAndConditionsPDF`, one PDF per (contract, region). The professional editions are the same endpoint with `segment=P` and `_P_` in the document slug: identical layout, priced excluding 21% VAT, with the degressive excise schedule and the professional energy-fund row. The same endpoint serves past months through its `monthOffset` parameter, back to late 2024 in the layout the parser reads, so a contract start date pins the signing month's card and the year-to-date bills each past month on its own. |
| **Frank Energie** | Dynamisch · Dynamisch HV · Dynamisch Korting · Dynamisch JN · Dynamisch Slim | [`providers/frank.py`](./custom_components/be_electricity_prices/providers/frank.py) — monthly tariff card PDFs discovered via the public Sanity CMS file-asset API (`8navd656.api.sanity.io`), parsed via `pdfplumber`. Flanders only, five dynamic contract tiers with different factor/base/fee combinations. Every card prices `BELPEX per uur` and carries a footnote offering `kwartierprijzen [Quarter Hourly BELPEX]` through Frank's app instead, switchable from one month to the next; the coefficients do not change, so the card cannot say which grid an account is on and the meter step asks. Three tiers print a **cashback** (`Dynamisch Korting` 120 EUR, `HV` 115, `JN` 35), granted as a lump on the invoice after a full year rather than accrued across it; with a contract start date set it is credited against `current_year_cost` in the year the first anniversary falls in, and the comparison page credits it to a tier being quoted over the coming year, which is what makes `Korting` rank on the year it would actually cost. The cards' conditions are assumed to hold. All three require a year of uninterrupted supply, which an entry that left the contract satisfies by no longer being priced on that card. `HV` and `JN` attach three more that nothing here can check: that every invoice was paid on time, that the discount is not combined with another promotion, and that the household is not on a social tariff (none is modelled anywhere in this integration). `Dynamisch Korting` attaches none of those three. |
| **Luminus** | Comfy · Comfy+ · ComfyFlex · ComfyFlex+ · MaxxFix · MaxxFlex · BasicFix · BasicFlex · SmartFlex *(TOU)* · Dynamic | [`providers/luminus.py`](./custom_components/be_electricity_prices/providers/luminus.py) — Luminus's public REST endpoint at `luminus.be/api-next/get-pricelist/`, V/W only (no Brussels for market products). Past months come from the price-list archive behind the site's "Archives de listes de prix" page (`api/pricelist/products` names a month's products, `api/pricelist/pdf` serves one), back to 2021, so a contract start date pins the signing month's card and the year-to-date bills each past month on its own. |
| **Mega** | Smart Fixed/Flex · Zen Fixed · Online Fixed/Flex · Cosy Fixed/Flex · Off-peak Fixed · Off-peak Flex · Off-peak Impact *(Wallonia, CWaPE 3-band)* · Dynamic · **pro**: SME Fixed/Flex · Smart Fixed/Flex · Online Fixed · Cosy Fixed/Flex · Off-peak Fixed · Dynamic · Zen Fixed | [`providers/mega.py`](./custom_components/be_electricity_prices/providers/mega.py) — scrapes the public listing at `mega.be/fr/energie/cartes-tarifaires` to resolve each `(product, region)` to its current PDF on `my.mega.be`. Every Flex and the Off-peak Impact card is month-indexed: it prints its tariff formula per meter (or per CWaPE band) beside "les derniers prix constatés … pour le mois de <MONTH>", and the month it names is the one before the card's own — the April card settles March. With an ENTSO-E key the delivery month is re-priced off the formula; without one that printed figure, a month behind, stands |
| **OCTA+** | Fixed · Fixed Impact *(Wallonia, CWaPE 3-band)* · Eco Fixed · Smart Variable · Flux · Eco Flux · Dynamic · Eco Dynamic | [`providers/octaplus.py`](./custom_components/be_electricity_prices/providers/octaplus.py) — stable URLs at `files.octaplus.be/tariffs/E_OCTA_<PRODUCT>_RE_<VL\|WL>_FR.pdf`, parsed via word-coordinate alignment (heavy character spacing in the tax block) — Flanders + Wallonia only. Every non-dynamic card indexes its feed-in credit on the monthly Epex SPP and prints only an estimate of it, so the credit is resolved against the delivery month's own solar-weighted mean. Past months come from the archive behind the site's "archive fiches tarifaires" page (`getTarifArchive` lists a month's cards, `getTariffSheet` serves one), so a contract start date pins the signing month's card and the year-to-date bills each past month on its own. |
...
| **Trevion** | Groene Energie Vast · Groene Stroom Flex *(monthly Belpex_RLP_VL)* · Groene Energie Dynamisch · Groene Energie Dynamisch Plus · LifePowr *(monthly Belpex_RLP_VL since June 2026, quarter-hourly Belpex 15 MTU before)* · Energreen | [`providers/trevion.py`](./custom_components/be_electricity_prices/providers/trevion.py) — monthly residential cards discovered from `trevion.be/tariefkaarten/`, with a public month-stamped archive. Flanders only. The three dynamic products bill on quarter-hourly Belpex 15 MTU; Flex and LifePowr settle consumption on the Fluvius RLP-weighted monthly mean and injection on the Belgian SPP-weighted mean; Vast publishes separate peak/off-peak consumption and feed-in rates. |
| **Expert: custom formula** *(no public card)* | Dynamic (`factor × spot + base`) · Monthly average (`factor × monthly-mean spot + base`) · Fixed / manual rate | [`providers/custom.py`](./custom_components/be_electricity_prices/providers/custom.py) — an escape hatch for suppliers with **no public, machine-resolvable tariff card** (see below). Not scraped: you type the commodity formula and all regulated DSO + tax values, and the coordinator builds the snapshot from your config entry. The tax step shows only the boxes your region bills: the green levy box takes GSC + WKK together in Flanders and the green-energy contribution in Wallonia and Brussels, and the connection-fee box (the Walloon redevance de raccordement, VAT-exempt) appears on Walloon entries only. |
...
1.800 kWh vast, vaste injectieprijs 3 jaar and Laadpunt, Trevion Groene Stroom
...
- **Spot prices** *(dynamic and monthly-indexed contracts)* — fetched from ENTSO-E at hourly resolution, or at the native 15-minute resolution for suppliers that bill per quarter-hour (Cociter, EBEM, Ecofix, Ecopower Dynamische Burgerstroom, energie.be, Energy Knights Agilior Online, EnergyVision, Engie, OCTA+ and Trevion, plus Bolt and Frank Energie with the quarter-hour box ticked); tomorrow's curve picked up on the first hourly tick after publication (~13:00 CET), so up to an hour after it, since the coordinator's tick is not clock-aligned. Historical spots are backfilled lazily into a per-entry persistent cache, requested on the same grid the contract settles on (widened per month: a closed month whose archived card billed per quarter-hour while the current card does not, which is how Trevion LifePowr was billed until May 2026, is fetched on the 15-minute grid for that month) and stored as one price per clock hour (the mean of that hour's slots), so `current_year_cost` replays each past hour at its actual rate (the live spot for a dynamic contract, the delivery month's mean for a monthly-indexed one) without re-fetching the same window every tick. A window neither source could answer is retried three hours later rather than on the next tick, so a platform outage no longer has every hourly tick re-requesting the whole year behind it; the missing hours forfeit only their energy term in the meantime, and fill in once a source answers again. An hour is the finest a replay can price because the recorder only keeps hourly consumption, and the mean is exact there for every formula that is linear in the spot. A floored feed-in formula is the one that is not, whether the floor is zero (a never-negative guarantee) or a stated minimum (EnergyVision's 1 c€/kWh): clamping makes it convex, so an hour whose spot crossed the floor inside it is worth more than flooring its mean says, and an entry on such a card keeps that hour's own 15-minute slots beside the mean and replays the credit off them, matching what its `injection_price` sensor shows.
