...
| `yearly_cost` | Running annual bill in EUR. Configure once in the **Energy meters** step, two ways: (a) point at the four day/night register sensors directly (preferred — exact from setup); or (b) point at single cumulative consumption / injection sensors (the integration splits deltas into day/night buckets via the bi-hourly schedule and persists them across restarts; running cost accurate from the moment of setup). Goes negative under Walloon compensation when injection > consumption (uncapped — surplus is theoretically credited at the consumption rate, even though most suppliers floor the actual bill at zero). Always unavailable on dynamic / TOU contracts (no stable rate). |
