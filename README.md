...
- **Self-healing** — last-known prices keep serving on outage, and the day-ahead curve is persisted, so an ENTSO-E outage spanning a Home Assistant restart no longer blanks a dynamic entry. A stored curve is only ever used for the day it actually prices: once it is outlived it is discarded rather than served as the current one. Repair issues surface under **Settings → System → Repairs**: snapshot older than 7 days, a supplier extractor parse failure (layout drift), a card that downloads fine but carries no readable text layer, the supplier being unreachable after repeated fetch failures, ENTSO-E rejecting the API key, a supplier that has announced it is leaving the residential market, and four cards for a value the supplier never printed (the exclusive-night distribution rate, the Walloon Tarif Impact bands, the Walloon connection fee, the Walloon prosumer tariff). A single transient fetch timeout no longer raises an issue. The fetch and staleness ones auto-clear on the next successful refresh; the deprecation and missing-row ones clear when the underlying situation changes.
...
TimeoutError` rather than trailing off after the colon. Eleven repair issues surface
