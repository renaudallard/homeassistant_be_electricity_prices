...
| **Mega** | Smart Fixed/Flex · Online Fixed/Flex · Cosy Fixed/Flex · Off-peak Flex · Off-peak Impact *(Wallonia, CWaPE 3-band)* · Dynamic · Cap · **pro**: Smart Fixed/Flex · Online Fixed · Cosy Fixed/Flex · Dynamic · Cap · Zen Fixed | [`providers/mega.py`](./custom_components/be_electricity_prices/providers/mega.py) — scrapes the public listing at `mega.be/fr/energie/cartes-tarifaires` to resolve each `(product, region)` to its current PDF on `my.mega.be` |
