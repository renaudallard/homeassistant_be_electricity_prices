...
| `yearly_cost` | Running annual bill in EUR, computed from the four meter sensors set in the **Energy meters** step (day/night consumption + day/night injection) and the snapshot's rates. Goes negative under Walloon compensation when injection > consumption (uncapped — surplus is theoretically credited at the consumption rate, even though most suppliers floor the actual bill at zero). Stays unavailable until all four sensors are configured and produce numeric readings. Always unavailable on dynamic / TOU contracts (no stable rate to apply to a daily total). |
