...
- **Monthly-indexed contracts** — `factor × this month's mean spot + base`, for cards that index the commodity to a monthly average rather than the live hourly price (energie.be Variabel, on Belpex_RLP). The rate is flat for the whole delivery month and firms up as the month fills in, so an ENTSO-E API key is required just as for a dynamic contract. Injection on such a card is credited from the card's own monthly indicative when it indexes on a different parameter (energie.be credits on the solar-weighted Belpex_SPP, which runs far below the consumption index in sunny months).
